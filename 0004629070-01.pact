(module fml4-hype-locker 'admin-multi-keyset
  (implements free.callable)
  ...
  (defconst LOCK_ACCT2:string "LOCK_ACCT2")
  (defun lock-guard () (create-module-guard 'LOCK_ACCT2))
  ...
  (defun init ()
    (hypercent.prod-hype-coin.create-account LOCK_ACCT2 (lock-guard))
  ...
  (defun lock-hype:string (account:string project:string amount:decimal tickets:integer ga:integer)
    @doc
       ...
       \ returns true"
    (enforce-valid-transfer account LOCK_ACCT2 (precision) amount)

    (install-capability (hypercent.prod-hype-coin.TRANSFER account LOCK_ACCT2 amount))
    (hypercent.prod-hype-coin.transfer account LOCK_ACCT2 amount)
    ...
    (with-capability (CAN_LOCK)
      ...
      (free.fml4-community-v2.add-rewards-for-staking project account amount tickets ga)
    )
  )
  ...
  (defun unlock-hype (account:string)
    ...
    (let* (
      ...
      (install-capability (hypercent.prod-hype-coin.TRANSFER LOCK_ACCT2 account amount_to_unlock))
      (hypercent.prod-hype-coin.transfer LOCK_ACCT2 account amount_to_unlock)
