(module kedao GOVERNANCE
  ...
  (defun migrate-point-snapshots:[string] (begin_date:string end_date:string)
    (let*
      (
        (snapshots (query-point-snapshots-with-date begin_date))
        (add-snapshots (lambda (record)
          (let*
            ...
            (write point-snapshots (format "{}_{}" [(at 'uid record) end_date]) {
              "points": {
                "droplets": (+ (at 'droplets pre_snapshot) (diff-for-type "droplets")),
                "flames": (+ (at 'flames pre_snapshot) (diff-for-type "flames")),
                "goldstone": (+ (at 'goldstone pre_snapshot) (diff-for-type "goldstone")),
                "leaves": (+ (at 'leaves pre_snapshot) (diff-for-type "leaves"))
              }
            })
          )
        ))
