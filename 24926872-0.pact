(module radio02 GOVERNANCE
  ...
  (defun add-affiliate (affCode address)
      (with-read radmin "admin"
        { "aguard" := aguard }
        (enforce-guard aguard)
...
;     hashedPw: string
  ...
  (defun get-licenses () 
    (select licenses1 (constantly true)) 
  )
