      ...
      (with-capability (OWNER nft-id)
        (let*
          (
            ...
            (weapons-count-mult (if (!= "attack-three" "") 1.5 (if (!= "attack-two" "") 1.25 (if (!= "attack-one" "") 1 1))))
            (owner (at 'owner nft))
            (battle (read battles-table battle-id))
            (amount (at 'amount battle))
            ...
            (damage (* weapons-count-mult (if (= env-type nft-type) (* (at "damage" attack) mult) (at "damage" attack))))
    ...
    (defun set-battle-env (type:string mult:decimal)
      ...
      (with-capability (API)
        (let ((curr-env (get-curr-battle-env))) 
          (update environments (at "type" (try (at 0 curr-env) { "type": "" })) {
            "type": type,
            "mult": mult,
            "active": false
          })
