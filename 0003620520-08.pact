(module prod-marketplace GOVERNANCE
    ...
    (defcap CLOSE_SALE () "Internal cap" true)
    (defcap IS_ADMIN ()
        (enforce-keyset "hypercent.hyper-api-admin-prod"))
    
    (defcap OPEN_SALE (type:string id:string owner:string buy_now_price:decimal sale_start_days:integer sale_end_days:integer) @event true)
    (defcap CANCEL_BID (id:string account:string) @event true)
    (defcap BID (id:string buyer:string amount:decimal bid_days:integer)  @event true)
    (defcap OUTBID (id:string account:string last_account:string) @event true)
    (defcap BUY (id:string buyer:string) @event true)
    ...
    (defun nft-bank-guard () (create-module-guard "hypercent-marketplace-bank"))
    ...
    (defun close-sale (id:string)
        (with-read sale id {"owner":=sale_owner, "closed":=sale_closed, "last_offer_account":=last_offer_account, "last_offer":=last_offer}
            ...
            (with-capability (CLOSE_SALE)
                (internal-close-sale id sale_owner)
            )
            
        )
    )
    (defun internal-close-sale (id:string sale_owner:string)
        (require-capability (CLOSE_SALE))
    ...
    (defun accept-last-bid (id:string)
        (with-read sale id {"closed":=closed, "owner":=sale_owner, "last_offer":=last_offer, "last_offer_account":=last_offer_account}
    ...
    (defun buy (id:string buyer:string)
            (with-read sale id {"closed":=closed, "can_buy_now":=can_buy_now, "buy_now_price":=buy_now_price, "owner":=owner, "last_offer":=last_offer, "last_offer_account":=last_offer_account}
    ...
    (defun bid (id:string buyer:string amount:decimal bid_days:integer)            
        (with-read sale id {"closed":=closed, "last_offer":=last_offer, "type":=type, "last_offer_account":=last_offer_account, "sale_end_date":=sale_end_date, "owner":=sale_owner}
    ...
    (defun cancel-bid (id:string)
        (with-read sale id {"closed":=closed, "last_offer":=last_offer, "type":=type, "last_offer_account":=last_offer_account, "offer_end_date":=offer_end_date}
