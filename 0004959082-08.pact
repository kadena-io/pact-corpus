...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (defun claim-weapon-reserved (
    ...
    (with-capability (MINT-RESERVED-NFTS collection-id)
      (with-capability (MINT-BULK owner)
        (let*
            (
            (random (get-random owner))
            (token-ids-to-mint:list (get-token-ids 1 collection-id owner random true))
            (collection (get-collection collection-id))
            (reserved-tokenList (at 'reserved-tokens collection))
            (reserved-tokenList-length (length reserved-tokenList))
   ...
   (defun get-first-non-empty-collection-from-weapons ()
        (let* 
            (
                (collection-suffix-list:list (enumerate 1 32)) 
                (collection-names-list:list (map (lambda (x) (x + "battle-heroes-weapons-policy-level-1-col-")) collection-suffix-list))
                (non-minted-cols-list-list:list  (map (lambda (x) (visible-for-test-get-non-minted-tokens x)) collection-names-list))
               (has-next-non-minted-cols-list-list:list (filter (compose (length) (< 1)) non-minted-cols-list-list))
            )
    ...
    (defun visible-for-test-get-non-minted-tokens:list (col-id:string )
        (if (= col-id "battle-heroes-weapons-policy-level-1-col-1")
            [
                {"non-minted-token-ids": [], "collection-id" : "battle-heroes-weapons-policy-level-1-col-1"}
            ]
            [
                {"non-minted-token-ids": ["1"], "collection-id" : "battle-heroes-weapons-policy-level-1-col-2"}
                {"non-minted-token-ids": ["1" "2"], "collection-id" : "battle-heroes-weapons-policy-level-1-col-3"}
            ]
        )
    )

  (defun get-non-minted-tokens-col (col-id:string)
        (read collections col-id ['non-minted-token-ids 'collection-id])
  )
