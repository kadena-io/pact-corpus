(module test2-lottery 'hyper-admin-multi-keyset
  (defcap IS_ADMIN ()
  ...
  (hypercent.test2-community.has-ga account project)
  )
  ...
  (enforce-guard (at 'guard (test2-hype-coin.details account)))
    (let* (
      ...
      (if (constantly is_winner)
        ...
        (let* (
            ...
            (debit-tickets (test2-community.debit-tickets account tickets))
  ...
  (defun pre-process-participants-local (batch_length:integer project:string winners_per_round:integer)
      ...
      (with-read projects-table project
        ...
        (let* (
              ...
              (winners_with_kyc (filter (hypercent.test2-community.has-kyc) total_winners))
