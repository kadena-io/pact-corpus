(module staging11-community-v2 'hype-admin-local-test-staging
  (defcap IS_ADMIN ()
  ...
  (defun set-ga-wins (project:string account:string ga_wins:integer)
      (with-capability (IS_ADMIN)
  ...
  (defun get-participant-with-tickets (participant_key:string)
    (read users-in-projects-table participant_key)
  )
  ...
  (defun is-nft-round-winner (project:string account:string)
     (> (get-nft-participant-ga project account) 0)
  )
  ...
  (defun get-participant-lottery-tickets (project account:string tickets:integer )
        (let* (
                (empty_tickets_arr (map (wrap-list) (if (= tickets 0) [] (enumerate 1 tickets))))
  ...
  (defun get-participants (project:string winners_amount:integer)
            (let*  (
                    ...
                    (participants_processed (map (lambda (participant) {
                        ...
                        "winning_tickets": (map (at "ticket") (filter (where "account" (= (at "account" participant) )) participants_lottery_tickets_sorted)),
  ...
  (defconst TICKETS_PRICE [200.00,230.00,265.00,305.00,350.00,405.00,465.00])
  (defconst GA_PRICE [1000.00,1220.00,1490.00,1820.00,2220.00])
  ...
  (defun add-rewards-for-staking (project:string account:string amount:decimal tickets:integer ga:integer)
        ...
        (let* (
              (locking-contract:module{staging11-callable} (free.staging11-callable-contracts.get "hype-locker"))
              (total_price (get-hype-amount-for-rewards tickets ga))
              (has_kyc (if (= (has-kyc project account) true) true false))
              (enforce_kyc (enforce (= true has_kyc) "no kyc"))
            )
  ...
  (defun get-participant-ga (project:string account:string)
      (with-read projects-table project {"round":=round}
        (with-default-read users-in-projects-table (get-participant-key project account) {"ga": 0, "ga_wins": 0, "tickets": 0} {"ga":=ga, "ga_wins":=ga_wins, "tickets":=tickets}
            ;round 1 = total ga from locking or won
            ;round 2 = unlucky tickets, total of tickets - whatever he won
            ;round 3 = 10 ga
            (if (= round 1) (+ ga ga_wins) (if (= round 2) (- tickets ga_wins) 10))
        )
      )
      
  )

  (defun get-nft-participant-ga (project:string account:string)
      (with-read projects-table project {"round":=round}
        (with-default-read users-in-projects-table (get-participant-key project account) {"ga": 0, "ga_wins": 0, "tickets": 0} {"ga":=ga, "ga_wins":=ga_wins, "tickets":=tickets}
            ;round 1 = total ga from locking or won
            ;round 2 = unlucky tickets, total of tickets - whatever he won
            ;round 3 = 10 ga
            ;round not in range = 0 //probably the rounds not started
            (if (= round 1) ga (if (= round 2) ga_wins (if (= round 3) 1 0)))
        )
      )
