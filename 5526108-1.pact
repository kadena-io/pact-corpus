(module tokensale GOVERNANCE
  ...
  (defun fetch-total-fundedamount (projectId:string fundOwner:string)
    (let
      ((investedAmount (fold (lambda (fundAmount1 fundAmount2)
        { "amount": (+ (at "amount" fundAmount1) (at "amount" fundAmount2)) }
        )
        {"amount": 0.0}
        (select funds-table ['amount] (and? (and? (where 'fundOwner (= fundOwner)) (where 'projectId (= projectId))) (where 'status (!= CANCELLED))))
      )))
      (at "amount" investedAmount)
    )
  )
  ...
  (defun claim-tokens (projectId funder funder-guard)
    ...
    (let ((claimableAmount (fetch-claimable-tokens projectId funder)))
        ...
        (with-read projects-table projectId {
          ...
          (let ((tokenAmount (* ratio claimableAmount)))
