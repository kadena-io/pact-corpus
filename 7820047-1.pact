(module mjc-casino GOVERNANCE
    ...
    (defconst BANK_KDA_ACCT "mjc-bank")
    ...
    (defun withdraw-staked-rewards (account:string)
        ...
        (with-capability (ACCOUNT_GUARD account)
            (let*
                ...
                (install-capability (coin.TRANSFER BANK_KDA_ACCT account (get-rewards account)))
                (coin.transfer BANK_KDA_ACCT account (get-rewards account))

                (update rewards-table account {"amount": 0.0})
    
                (format "You successfully made a withdrawal of {} KDA" [(get-rewards account)])
            )
        ))
    ...
    (defun withdraw-bet-rewards (account:string)
        ...
        (with-capability (ACCOUNT_GUARD account)
            (let*
                ...
                (coin.transfer BANK_KDA_ACCT account rewards)
    ...
    (defcap GOVERNANCE ()
        ...
        (enforce-guard (keyset-ref-guard "free.mj-keyset"))
