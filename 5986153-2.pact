...
; ============================================
    ...
    (defun place-nft-offer (account:string offer:decimal duration:integer nft-id:string )
        ...
        (with-capability (PRIVATE)
            (with-read mledger nft-id
                ...
                (if (!= old-offer-id -1) (return-kda-for-id (int-to-str 10 old-offer-id)) "no action necessary")
                (if (!= old-offer-id -1) 
                    (with-read nft-offer-table (int-to-str 10 old-offer-id) ; if true
                        { 'amount := old-offer }
                        (enforce (> offer old-offer) "Your offer must be greater than the current offer") 
                    )
                    "no action necessary") ;if false
            )
    ...
    (defun select-valid-offers-for-id (nft-id:string)
        ...
        (select nft-offer-table (where "valid-until" (< (at "block-time" (chain-data)))))
    )

    (defun select-offers-made-by-account (account:string)
        @doc "selects all nft-specific-offers made by an account"
        (select nft-offer-table (where "address" (= account)))
    )
