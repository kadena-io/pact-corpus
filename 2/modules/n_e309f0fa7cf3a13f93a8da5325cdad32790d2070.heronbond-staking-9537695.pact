(module heronbond-staking GOVERNANCE "Heronbond Staking Pool - Distributes 2% of HERON supply to Heronbond Stakers"
    ...
    (defun create-pool (id:string
        ...
        (with-capability (ACCOUNT_GUARD account)
            ...
            (reward-token::create-account (get-account-principal) (create-pool-guard))
            ;Transfer reward token
            (reward-token::transfer-create account (get-account-principal) (create-pool-guard) balance)
            ;If reward-token != stake-token then create a token account for stake-token too
            (if (= (get-token-key reward-token) (get-token-key stake-token)) true (stake-token::create-account (get-account-principal) (create-pool-guard)) )
            ;Insert pool
            (insert pools id
                {
                    "id": id,
                      ...
                      (let
                          ...
                          (if (or (or (= (at "active" pool-data) false) (= claim-stake true) ) (<= (- available to-pay-max ) 0.0))
                            (let
                                ...
                                (if (= (reward-token::details (get-account-principal)) (stake-token::details (get-account-principal)))
                                  (if (= (get-token-key reward-token) (get-token-key stake-token))
                                    (if (>= (- (at "balance" pool-data) to-pay ) 0.0)
                                      (install-capability (reward-token::TRANSFER (get-account-principal) account (+ to-pay-stake to-pay)))
                                      (install-capability (reward-token::TRANSFER (get-account-principal) account (+ to-pay-stake (at "balance" pool-data))))
                                    )
                                    (if (and (>= (- (at "balance" pool-data) to-pay ) 0.0)  (> to-pay 0.0))
                                      (install-capability (reward-token::TRANSFER (get-account-principal) account to-pay))
                                      (if (> (at "balance" pool-data) 0.0) (install-capability (reward-token::TRANSFER (get-account-principal) account (at "balance" pool-data))) true)
                                    )
                                  )
                                  (if (and (>= (- (at "balance" pool-data) to-pay ) 0.0)  (> to-pay 0.0))
                                      (install-capability (reward-token::TRANSFER (get-account-principal) account to-pay))
                                ...
                                (if (= (reward-token::details (get-account-principal)) (stake-token::details (get-account-principal)))
                                  (if (= (get-token-key reward-token) (get-token-key stake-token))
                                    (if (>= (- (at "balance" pool-data) to-pay ) 0.0)
                                      (with-capability (PRIVATE_RESERVE) (reward-token::transfer (get-account-principal) account (+ to-pay-stake to-pay))  )
                                      (with-capability (PRIVATE_RESERVE) (reward-token::transfer (get-account-principal) account (+ to-pay-stake (at "balance" pool-data)))  )
                                    )
                                    (if (and (>= (- (at "balance" pool-data) to-pay ) 0.0)  (> to-pay 0.0))
                                      (with-capability (PRIVATE_RESERVE) (reward-token::transfer-create (get-account-principal) account (at "guard" (coin.details account)) to-pay )  )
                                      (if (> (at "balance" pool-data) 0.0) (with-capability (PRIVATE_RESERVE) (reward-token::transfer-create (get-account-principal) account (at "guard" (coin.details account)) (at "balance" pool-data) )  ) true)
                                    )
                                  )
                                  (if (and (>= (- (at "balance" pool-data) to-pay ) 0.0)  (> to-pay 0.0))
                                    (with-capability (PRIVATE_RESERVE) (reward-token::transfer-create (get-account-principal) account (at "guard" (coin.details account)) to-pay )  )
                                ...
                                (if (= (reward-token::details (get-account-principal)) (stake-token::details (get-account-principal)))
                                  ...
                                  (if (= (get-token-key reward-token) (get-token-key stake-token))
                                    true
                                    (install-capability (stake-token::TRANSFER (get-account-principal) account to-pay-stake))
                                  )
                                )
                                ...
                                (if (= (reward-token::details (get-account-principal)) (stake-token::details (get-account-principal)))
                                  ...
                                  (if (= (get-token-key reward-token) (get-token-key stake-token))
                                    true
                                    (with-capability (PRIVATE_RESERVE) (stake-token::transfer (get-account-principal) account to-pay-stake)  )
                                  )
                                )
