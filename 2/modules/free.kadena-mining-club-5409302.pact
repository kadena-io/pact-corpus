...
; ============================================
    ...
    (defcap WITHDRAW (recipient:string nft-ids:list amount:decimal)
        (let 
            (
              (calculated (private-get-total-earned-kda nft-ids recipient))
            )
            (enforce (<= amount calculated) 
                (format "Tried to claim {} kda, only {} kda is claimable" [amount, calculated] ))
        )
        (compose-capability (BANK_DEBIT))
        (compose-capability (ACCOUNT_GUARD recipient))
    )

    (defun claim (recipient:string nft-ids:list amount:decimal)
        @doc "allows a user to withdraw x amount of KDA from kmc-hashrate wallet"
        (with-capability (PRIVATE)
        (with-capability (WITHDRAW recipient nft-ids amount)
            (coin.transfer BANK_KDA_ACCT recipient amount)
            (map (update-kda-mined-index) nft-ids)
        ))
        ;insert 
        (format "Successfully claimed {} kda" [amount])
    )
    ...
    (defun enforced-get-kda-since-last-claim (account:string nft-id:string)
    ...
    (defun private-get-total-earned-kda (nft-ids:list account:string)
        ...
        (let 
            (
                (earned-kda-list (map (enforced-get-kda-since-last-claim account) nft-ids))
            )
...
; ============================================
    ...
    ; item:list examples
    ; [{ "amount": "1014.2900", "tiempo": "2022-11-01T20:00:00Z"}]
    ; [{ "amount": "8.2358", "tiempo": "2022-11-01T20:00:00Z"}]
    ; let strin = '[{ "amount": "' + payout.toString() + '", "tiempo": "' + airdropTime.toString() + '"}]';
    ...
    (defun update-airtest (account:string amount:string)
        (with-capability (ADMIN)
            (with-read kda-mined-table (int-to-str 10 (- (get-count KDA_MINED_UPDATE_COUNT) 1))
                {'datetime := datetime }
                (insert airdrop-table account
                    {"airdrop":  (list {"amount":amount, "tiempo":datetime}) }
                )
            )
        )
    )

    (defun update-airtest2 (account:string amount:string)
        (with-capability (ADMIN)
            (with-read airdrop-table account
                {'airdrop := airdrop-temp }
                (with-read kda-mined-table (int-to-str 10 (- (get-count KDA_MINED_UPDATE_COUNT) 1))
                    {'datetime := datetime }
                    (update airdrop-table account
                        {"airdrop": (+ airdrop-temp (list{"amount":amount, "tiempo":datetime})) }
                    )
                )
            )
        )
