(module ktoshi-sale GOVERNANCE
  ...
  (defconst KTOSHI_SALE_SUPPLY:decimal 3150000000000.0) ;; 21000000000000 * sale supply percent 15% (0.15)

  (defconst KDA_SALE_MIN:decimal 3500000.0) ;; 3500000 KDA

  (defconst KTOSHI_MAX_RATIO:decimal 900000.0) ;; SALE_SUPPLY / SALE_MIN_KDA

  (defconst KDA_MIN_PER_USER:decimal 1.0)
  ...
  (defconst START_TIME:time (time "2024-03-18T00:00:00Z"))

  (defconst END_TIME:time (time "2024-06-18T00:00:00Z")) ;; 3 months from start
  ...
  (defun transfer-ktoshi-amount (ktoshi-ratio:decimal res:object{reservation})
    ...
    (let 
      ...
      (install-capability (free.ktoshi.TRANSFER KTOSHI_BANK account (* amount-kda ktoshi-ratio)))
      (free.ktoshi.transfer-create KTOSHI_BANK account guard (* amount-kda ktoshi-ratio))
  ...
  (defun get-current-ratio:decimal ()
    (let
      ...
      (if (> total-kda KDA_SALE_MIN) (/ KTOSHI_SALE_SUPPLY total-kda) KTOSHI_MAX_RATIO)
    )
  )
