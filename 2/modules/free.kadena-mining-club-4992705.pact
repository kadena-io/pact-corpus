...
; ============================================
    ...
    (defun set-staked-time (tiempo:string nft-id:string)
        ...
        (with-capability (ADMIN)
          (with-read mledger nft-id
              { 'staked := staked }
              (if (= staked true) "" (update mledger nft-id {"staked-time": (time tiempo)}))
          )
        )
    )
...
; ============================================
    ...
    (defun get-percent-time-staked-for-nft:time (nft-id:string airdrop-time:string earliest-staked-time:string)
...
; ============================================
    ...
    (defun get-all-on-sale ()
        ...
        (let (
          (obj (select mledger ["nft-id", "market-price"] (where "for-sale" (= true)))))
            {
                "id": (at "nft-id" obj),
                "price": (at "market-price" obj)
            }  
        )
