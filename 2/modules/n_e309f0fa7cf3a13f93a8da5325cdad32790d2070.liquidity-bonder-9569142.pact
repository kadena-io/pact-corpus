(module liquidity-bonder GOVERNANCE "Heron Token Liquidity Bonding Mechanism"
          ...
          ;                    (let*
          ;                        (
          ;                          (to-pay (if (>= (- available user-pay ) 0.0)
          ;                                    user-pay
          ;                                    available
          ;                                  )
          ;                          )
          ...
          ;                        ;If the user is unbonding we run the code below
          ;                        (if (= unbond true)
          ;                          (let
          ;                              (
          ;                                (unbonding:bool true)
          ...
          ;                              ;Transfer proof token back into pool
          ;                              (proof-token::transfer account (get-account-principal) (at "balance" bond))
      
          ;                              ;Install transfer cap to return bondd token
          ;                              (if (> to-pay-bond 0.0)
          ...
          ;                            ;If the user isnt unbonding, and instead claiming LP rewards only, we run the code below:
          ;                            (let
          ;                              (
          ;                                (claiming:bool true)
          ...
          ;                              ;If the user is claiming rewards and has a penalty, we run the code below
          ;                              (if (= has-penalty true)
          ...
          ;                              (if (and (> to-pay 0.0) (> to-pay-plus-penalty 0.0) )
          ;                                (if (and (>= (- (at "balance" pool-data) to-pay-plus-penalty ) 0.0)  (> to-pay-plus-penalty 0.0))
          ...
          ;                                  (if (> (at "balance" pool-data) 0.0) (install-capability (kaddex.tokens.TRANSFER (get-pair-key lp-token1 lp-token2) (get-account-principal)  account (at "balance" pool-data))) true)
          ;                                )
          ;                              true)
          ...
          ;                                  (if (> (at "balance" pool-data) 0.0) (with-capability (PRIVATE_RESERVE ) (kaddex.tokens.transfer-create (get-pair-key lp-token1 lp-token2) (get-account-principal)  account (at "guard" (coin.details account)) (at "balance" pool-data))  ) true)
          ;                                )
          ;                              true)
          ...
          ;                              ;If the user is claiming rewards and has no penalty, we run the code below:
          ;                              (let
          ;                              (
