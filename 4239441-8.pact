(module isoko-orchestrator GOVERNANCE
  ...
  (defcap RESERVE-NFTS (collection-id:string)
    (let*
      (
          (collection (get-collection collection-id))
          (collection-guard (at 'collection-guard collection))
      )
      (enforce-one "Any Guard passes" [(enforce-guard (keyset-ref-guard "free.isoko-admin")) (enforce-guard collection-guard)])
    )
  )
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;; TODO : update SGK repl and remove
  ...
  (defun reserve-tokens (amount-to-reserve:integer owner:string collection-id:string)
    (with-capability (RESERVE-NFTS collection-id owner)
      (let*
          ...
          (enforce (<= (+ amount-to-reserve token-ids-to-reserve-length) created-tokenList-length) "Not enough tokens to reserve!")
          (update collections collection-id
    ...
    (with-capability (MINT-RESERVED-NFTS collection-id)
      (with-capability (MINT-BULK owner)
        (let*
            ...
            ;;TODO: Can update filter to just take and drop around index start and end if more gas effecient
            (enforce (<= (+ total-to-mint minted-total) reserved-tokenList-length) "Not enough tokens to mint!")
            (update collections collection-id
                  {'non-minted-tokens: new-created-token-list,
