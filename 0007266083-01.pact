(module gen-1-kitty-kad-kitties 'kitty-kad
  "Kitty Kad Kitties Gen1s for the NFT game"
    ...
    (defconst BREED_FEE 1.0)
    (defconst MAX_GEN_1 5000)
    ...
    (defun mint-kitty (parent-1-id:string parent-2-id:string owner:string)
        ...
        (with-capability (PRIVATE) 
            (let* 
                (
                    (p1-data (get-kitty-data-by-id parent-1-id))
                    (p2-data (get-kitty-data-by-id parent-2-id))
                    (new-id (id-for-new-kitty))
                    (curr-count (get-count KITTIES_MINTED_COUNT_KEY))
                    (new-kitty-data (free.kitty-kad-kitties-helper-functions.create-new-kitty-data p1-data p2-data new-id owner))
                )
                (enforce (< curr-count MAX_GEN_1) "All Gen 1 have been made" )
                (enforce-at-least-one-gen-0 p1-data p2-data)
                (insert nfts new-id new-kitty-data)
                (coin.transfer owner ADMIN_ADDRESS BREED_FEE)
                (increase-count KITTIES_MINTED_COUNT_KEY)
                ; Mark original kitties as bred
                (mark-as-bred  parent-1-id)
                (mark-as-bred  parent-2-id)
                (emit-event (MINTED new-id owner))
            )
        )
    )

    (defun enforce-at-least-one-gen-0 (p1-data:object p2-data: object)
        (enforce 
            (
                ;; Ensures at least one gen 1 there
                <=
                (+ 
                    (at "generation" p1-data) 
                    (at "generation" p2-data)
                )
                1
            )
            "Need at lest one gen 0"
        )
    )

    (defun mark-as-bred (id:string)
        (if
            (contains ":" id) 
            (free.kitty-kad-kitties.bred id)
            (bred id)
        )
    )

    (defun bred (id:string)
        (with-capability (OWNER id)
            (update nfts id 
                {"next-breed-time": (
                    free.kitty-kad-kitties-helper-functions.next-breed-time  
                    (at "gene-pairs" (get-nft-fields-for-id ["gene-pairs"] id))
                    1
                )}
            )
            (emit-event (BRED id))
    ...
    ;;;; Getter functions
    (defun get-kitty-data-by-id (id:string)
        (if
            (contains ":" id) 
            (free.kitty-kad-kitties.get-nft-fields-for-id [] id)
            (get-nft-fields-for-id [] id)
        )
    )
