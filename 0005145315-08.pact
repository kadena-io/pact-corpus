(module kmc-your-miner GOVERNANCE
  @doc "Kadena Mining Club NFT Your Miner policy. NFTs representing physical ASIC miners"
    ;shoutout to @crypto_pascal for his util libraries: free.util-lists
    ...
    (defun get-all-project-details ()
        (map (get-project-details) (map (at 'project-name) (get-all-projects-with-stock)))
    )
    ...
    (defun add-new-project (project:string payout-address:string associated-asics:list)
        ...
        (insert projects-table project 
            ...
            , "asics-remaining": (length associated-asics)
    ...
    (defun update-project-details (project:string field-item:object) ;item intentionally has no type so it can be anything
    	@doc "allows the admin to update project details such as payout address"
    	(with-capability (GOVERNANCE)
    		(update projects-table project field-item))
    )

    (defun deactivate-project (project:string active-promotion:bool) ;item intentionally has no type so it can be anything
    	@doc "allows the admin to update project details such as payout address"
    	(with-capability (GOVERNANCE)
    		(update projects-table project 
    ...
    (defun decrease-remaining-asics (project:string)
        (enforce-kmc)
        (with-read projects-table project
            { "asics-remaining" := asics-remaining }
                (update projects-table project
                { "asics-remaining" : (- asics-remaining 1) }))
    )

    (defun disassociate-project (project:string id:string)
        (enforce-kmc)
        (with-read kmc-tokens id
            { "asic-details" := asic-details }
                (let 
                    (
                        (hashrate (at 'Hashrate asic-details))
                        (wattage (at 'Wattage asic-details))
                        (location (at 'Location asic-details))
                    )
                    (update kmc-tokens id
                    { "asic-details" : { "Hashrate": hashrate, "Wattage": wattage, "Affiliation": "kmc", "Location": location }})))
        (with-read projects-table project
            { "associated-asics" := associated-asics }
                (update projects-table project 
                  { "associated-asics" : (filter (!= id) associated-asics) })
        )
    )
