...
; ============================================
    ...
    (defun init-init-multiple (old-nft-ids:[string])
        (with-capability (CALL-POLICY-MODULES)
            (map (init-init) old-nft-ids)
        )
    )
    
    (defun init-init (old-nft-id:string)
        
        (with-read nft-token-pairs old-nft-id
            { "token-id":= token-string }
            (kmc-policy.init-token-id-status-table (at 'token-id token-string))
        )
    )
