(module kor-create-nft "free.kornft-projectadmin-keyset"
    ...
    (defun set-owner(owner-address:string nft-id:string)
        @doc "Set the owner of an NFT - only available for admin"
            ...
            (enforce-keyset "free.kornft-projectadmin-keyset")
    ...
    (defun set-values(owner-address:string nft-id:string nft-value:string created-date:integer hash-rate:string rarityModel:string)
        @doc "Set the values for NFT"
            (enforce-keyset "free.kornft-projectadmin-keyset")
            (insert nft-kordata nft-id {"owner-address": owner-address,"nft-value": nft-value,"created-date": created-date, "hash-rate": hash-rate, "rarityModel":rarityModel, "nftid" : nft-id, "phone-number": "", "og-badge" : ""})
    )
