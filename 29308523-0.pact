(module radio02 GOVERNANCE
  ...
  (defun add-received-with-check (gatewayId:string mic:string)
    "Add received"
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (let ((received [{"address": (at "sender" (chain-data)), "mic": mic}]))
        (with-read nodes7 (at "sender" (chain-data))
  ...
  (defun award-receivers (amount:decimal receiver:object{receive})
    ...
    (bind receiver {"address":= address, "gatewayId":= gatewayId}
      (with-read nodes7 address 
        ...
        if (= (gatewayIdDb gatewayId)) ;Making sure address belongs to gateway
  ...
  (defun update-sent (mic:string)
    ...
    (with-read nodes7 (at "sender" (chain-data))
        {"send":= send,
         "sent":= sent}
        (enforce (= send true) "Has not been directed to send")
        (enforce (= sent "") "Has already updated what it sent")
        (update nodes7 (at "sender" (chain-data))
            {"sent": mic,
             "send": false,
             "lastAction": (get-time)}
...
;     hashedPw: string
  ...
  (defun mitigate-license ()
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
      (let* 
        ((licBalance (coin.get-balance RADIO01_LICENSE_BANK))
         (admAddress (at 'admin (read radmin "admin" ['admin])))
         (over (- licBalance 3000.0))
        )
        (if (> licBalance 3000.0) 
            (with-read nodeExtras9 (at "sender" (chain-data))
              {"oracle":= oracle}
              (enforce (= oracle true) "Needs to be an oracle node")
              (install-capability (coin.TRANSFER RADIO01_LICENSE_BANK admAddress over))
              (coin.transfer RADIO01_LICENSE_BANK admAddress over)
            )
            "Nothing to do"
        )
      )
