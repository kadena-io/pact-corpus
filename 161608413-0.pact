...
;     hashedPw: string
  ...
  (defun buy-license-mock (affCode)
    ...
    ;     {"address" := affAddress,
    ;      "percentage":= affPerc}
        (bind (free.crankkAffs.get-affiliate affCode) {"address":= affAddress, "percentage":= affPerc}
          (insert licenses5 (at "sender" (chain-data)) 
            {"address": (at "sender" (chain-data)),
            "guard": (read-keyset "keyset"),
            "usedForGw": "",
            "used": false,
...
;   )
  ...
  (defun get-licenses-mock () 
    (select licenses5 (constantly true)) 
  )
