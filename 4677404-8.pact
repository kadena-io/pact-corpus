(module default-coa-wr-policy GOVERNANCE

  @doc "Policy for fixed issuance with royalty and quoted sale in specified fungible. Allows burn and transfer"

  (defcap GOVERNANCE ()
    (enforce-guard (keyset-ref-guard 'thedna-default-coa-wr-policy-ks-12ABD5A6CF74 )))
  ...
  (defun enforce-init:bool
    ...
    (enforce-ledger)
    ;; Only admin can init
    (enforce-keyset 'thedna-only-admin-init-ks-12ABD5A6CF74)
    (let* ( (spec:object (read-msg TOKEN_SPEC))
            (fungible:module{fungible-v2} (at 'fungible spec))
            (creator:string (at 'creator spec))
            (creator-guard:guard (keyset-ref-guard (at 'creator-guard spec)))
