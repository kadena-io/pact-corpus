(module noxkd-whitelist-v1 GOVERNANCE

  @doc "Collection whitelist."

  (defcap GOVERNANCE ()
    (enforce-guard (keyset-ref-guard "noxkd_admin" )))
  ...
  (defun read-whitelist:object{whitelist-info} ()
  (let ((whitelist-info:object{whitelist-info} { "collection-id": (read-msg "collection-id"), "index":(read-integer "index"), "account":(read-msg "account"), "guard":(read-keyset "guard")}))
     whitelist-info ))


  (defcap CREATE_TOKEN (token-id:string account:string)
    (let* ( (mint-guard:guard  ( read-keyset "guard" )) )
  ...
  (defun reveal-tokens:[string] (collection-id:string token-ids:[string])
    (with-read collections collection-id {
      ...
      (with-capability (OPERATOR collection-id)
        (enforce (= (+ collection-size 1) (length slots)) "Pre-sale has not ended")
        (enforce (= (length token-ids) (+ collection-size 1)) "Token list is invalid")
        (update collections collection-id {
          "tokens": token-ids
  ...
  (defun enforce-init:bool (token:object{token-info})
    ...
    (let* ( (token-id:string  (at "id" token))
            ...
            (whitelist-info:object{whitelist-info} ( read-whitelist ))
