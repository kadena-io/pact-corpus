(module brawler-bears-attacks GOV
    ...
    (defconst PUBLIC_MINT_START_TIME "2023-05-20T19:00:00Z")
    (defconst MINT_END_TIME "3023-03-08T19:00:00Z")
    (defconst TRADING_START_TIME "2023-05-08T19:00:00Z")
    ...
    (defconst IMAGES_URI "https://arkade-prod.s3.us-east-1.amazonaws.com/brawler-bears-attacks/")
     ...
     (defun mint-nfts-bulk (owner:string amount:integer)
         @doc "Mints NFTs bulk"
          (enforce-mint-live)
          (arkade.token.transfer owner CREATOR_ADDRESS (* 0.90 (* (get-mint-price) amount)))
          (arkade.token.transfer owner ADMIN_ADDRESS (* 0.10 (* (get-mint-price) amount)))
    ...
    (defun equip:string (bear-id:string id:string)
        ...
        ; (enforce-marketplace-live)
        (enforce (= (at "bear-id" (read nfts id ["bear-id"])) "") "Already equiped to a Bear")
        (with-capability (OWNER id)
    ...
    (defun unequip:string (bear-id:string id:string)
        @doc "Unequip an attack to an NFT."
        ; (enforce-marketplace-live)
        (enforce (!= (at "bear-id" (read nfts id ["bear-id"])) "") "Not equiped to a Bear")
        (with-capability (OWNER id)
    ...
    (defun perform-attack:string (id:string)
        @doc "Equip an attack to an NFT."
        (with-capability (OWNER id)
            (update nfts id {"power": (- (at "power" (read nfts id ["power"])) 1)})
        )
    )
