(module time-lock GOVERNANCE
  ...
  (defconst WITHDRAW_AVAILABLE_FROM:time (time "2022-08-01T15:00:00Z"))
  ...
  (defun withdraw:string(reservation-account:string)
    (with-read lockups reservation-account
      ...
      (enforce (> kdx-locked 0.0) "No withdraw amount available")
        ...
        ;; Payout KDX
        (install-capability (kaddex.kdx.TRANSFER KDX_BANK payout-account kdx-locked))
        (kaddex.kdx.transfer-create KDX_BANK payout-account payout-guard kdx-locked)
