...
;   (use fungible-util)
  ...
  (defun update-affiliate-ora (aff:object{affiliateInput})
    ...
    (with-default-read affiliates4 (compound-key2 address category)
        {"address": ""}
        {"address":= addressDb}
        (if (= addressDb "")
            (insert affiliates4 (compound-key2 address category)
                {"category": category,
                 ...
                 "denied": denied,
                 "company": company,
                 "appliedAt": (time appliedAt),
                 "decidedAt": (time decidedAt),
  ...
  (defun add-affCode-ora (c:object{code})
    (enforce-guard (at 'guard (coin.details (at "sender" (chain-data)))))
    (is-oracle)
    (bind c {
      "category":= category,
