...
;   (use fungible-util)
  ...
  (defconst RADIO01_SUBSCR_BANK:string 'radio01-subscr01-bank)
  ...
  (bless "zPYJ2bKjgcguLrguZK1WlOts77pJjrMAhBDrk8kZYgU")
  ...
  (deftable gatewayGPSs2:{gatewayGPS})

  (defschema gatewayLocSet
     gatewayId: string
     set: bool
     rankScore:decimal
     lastUpdated: time
  ...
  (deftable gatewayLocSet3:{gatewayLocSet})

  (defschema gatewayLocSetNew
     gatewayId: string
     set: bool
     rankScore:decimal
     lastUpdated: time
  ...
  (deftable gatewayLocSet5:{gatewayLocSetNew})
  ...
  (defun insert-base ()
    (with-read radmin "admin"
      {"aguard" := aguard }
      (enforce-guard aguard)
      (insert rewardBase1 "base" {"base": 0.9008})
    )
  )
  ...
  (defun decrement-base ()
    (with-read radmin "admin"
      ...
      (with-read rewardBase1 "base"
        {"base":= baseDb}
        (update rewardBase1 "base" {"base": (- baseDb 0.001)})
      )
    )
  )

  (defun get-base ()
    (round (* 1.5 (at 'base (read rewardBase1 "base" ['base]))) 5)
  )
  ...
  (defschema exchangeRate
     curr1: string 
     curr2: string
     rate: decimal
  )    

  (deftable exchangeRates1:{exchangeRate})
  ...
  (defun update-USDtoCRKK (code:string USDtoCRKK)
    ...
    (with-read nodeExtras9 (at "sender" (chain-data))
        ...
        (with-default-read exchangeRates1 "USD/CRKK"
            {"rate": 0.0}
            {"rate":= rate}
            (if (= rate 0.0)
                (insert exchangeRates1 "USD/CRKK"
                    {"curr1": "USD",
                    "curr2": "CRKK",
                    "rate": USDtoCRKK}
                )
                (update exchangeRates1 "USD/CRKK"
                    {"rate": USDtoCRKK}
                )
            )
        )
  ...
  (defun is-locSet:bool (address)
    (with-default-read nodes7 address
        {"gatewayId": "e45f01fffe1744ab"}
        {"gatewayId":= gatewayId}
        (with-default-read gatewayLocSet3 gatewayId
          {"set": false}
          {"set":= set}
          set
        )
  ...
  (defun is-locSet-address:bool (address)
    (with-default-read gatewayLocSet5 address
      {"set": true}
      {"set":= set}
      set
    )
  ...
  (defun get-gatewayGPSs ()
    "Get all gatewayGPSs"
    (select gatewayGPSs2 (constantly true))
  )

  (defun get-gatewayLocSet ()
    "Get all gatewayLocSet"
    (select gatewayLocSet3 (constantly true))
  )

  (defun get-gatewayLocSet-address ()
    "Get all gatewayLocSet"
    (select gatewayLocSet5 (constantly true))
  )

  (defun get-currentGPSs ()
    "Get current gatewayGPSs"
    (select gatewayGPSs2 (where 'lastDetected (< (add-time (get-time) (days -1)))))
  )
  ...
  (defun update-gateway-gps (gatewayId)
    ...
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"address":= sender, "oracle":= oracle}
    (enforce (= oracle true) "Must be an oracle node")
    (with-default-read gatewayGPSs2 gatewayId 
      {"gatewayId": ""}
      {"gatewayId":= gatewayIdDb}
      (if (= gatewayIdDb "")
      (insert gatewayGPSs2 gatewayId 
          {"gatewayId": gatewayId, "lastDetected": (get-time)}
      )
      (update gatewayGPSs2 gatewayId
          {"gatewayId": gatewayId, "lastDetected": (get-time)}
      )
      )    
    )
  ...
  (defun update-gateway-locSet (gatewayId:string set:bool rankScore:decimal)
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"address":= sender, "oracle":= oracle}
    (enforce (= oracle true) "Must be an oracle node")
    (with-default-read gatewayLocSet3 gatewayId 
      {"gatewayId": "", "set": false, "rankScore": 0.0, "lastUpdated":(get-time)}
      {"gatewayId":= gatewayIdDb, "set":= setDb, "rankScore":= rankScoreDb, "lastUpdated":=lastUpdatedDb}
      (if (= gatewayIdDb "")
      (insert gatewayLocSet3 gatewayId 
         {"gatewayId": gatewayId, "set": set, "rankScore": rankScore, "lastUpdated": (get-time)}
      )
      (update gatewayLocSet3 gatewayId
         {"gatewayId": gatewayId, "set": set, "rankScore": rankScore, "lastUpdated": (get-time)}
      )
      )    
    )
  ...
  (defun update-gateway-locSet-address (gatewayId:string set:bool rankScore:decimal address:string)
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"address":= sender, "oracle":= oracle}
    (enforce (= oracle true) "Must be an oracle node")
    (with-default-read gatewayLocSet5 address 
      {"gatewayId": "", "set": false, "rankScore": 0.0, "lastUpdated":(get-time), "address":""}
      {"gatewayId":= gatewayIdDb, "set":= setDb, "rankScore":= rankScoreDb, "lastUpdated":=lastUpdatedDb, "address":=addressDb}
      (if (= gatewayIdDb "")
      (insert gatewayLocSet5 address
         {"gatewayId": gatewayId, "set": set, "rankScore": rankScore, "lastUpdated": (get-time), "address": address}
      )
      (update gatewayLocSet5 address
         {"gatewayId": gatewayId, "set": set, "rankScore": rankScore, "lastUpdated": (get-time), "address": address}
      )
      )    
    )
...
;   )
    ...
    (with-read nodes7 (at "sender" (chain-data))
        ...
        (with-read nodes7 sender
            ...
            (update nodes7 sender
                {"sent": "",
                 "net": (add-time (get-time) (+ 21200 (get-rand))),
            ...
            (if (> (length receivers) 0)
                (let*
                  ((base (get-base))
                   ...
                   (if (has-gps sender)    
                       (award sender (round (* (final-amount sender base) 1.01) 5))
                       (award sender (final-amount sender base))
                   )    
    ...
    "Award"
      ...
      (if (account-exists-crkk address)
          (with-capability (AWARD_DEBIT) 
              ...
              (free.crankk01.transfer RADIO01_AWARD_BANK address amount)
          ) 
          ""        
      )
  )
  ...
  (defun set-net-ahead (address)
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"oracle":= oracle}
        (enforce (= oracle true) "Must be an oracle node")
        (update nodes7 address
            {"net": (add-time (get-time) (days 1))}
        )
    )
  )
  ...
  (deftable licensePackTypes1:{licensePackType})
  ...
  (defun add-licensePackType (code:string units:integer priceUSD:decimal priceCRKK:decimal USDtoCRKK:decimal)
      (with-read radmin "admin"
        { "aguard" := aguard }
        (enforce-guard aguard)
        (insert licensePackTypes1 code
            {"units": units,
             "priceUSD": priceUSD,
             "priceCRKK": priceCRKK,
             "USDtoCRKK": USDtoCRKK}
  ...
  (defun update-packUSDtoCRKK (code:string USDtoCRKK:decimal)
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"oracle":= oracle}
        (enforce (= oracle true) "Must be an oracle node")
        (with-read licensePackTypes1 code
            {"priceUSD":= priceUSD}
            (update licensePackTypes1 code
                {"priceCRKK": (round (* priceUSD USDtoCRKK) 2), 
                 "USDtoCRKK": USDtoCRKK}
            )
        )
    )
...
;     hashedPw: string
  ...
  (deftable licensePacks1:{licensePack})

  (defschema licenseHash
     hashCode:string
     used:bool
     usedBy:string
  )    
  (deftable licenseHashes1:{licenseHash})

  (defun buy-licensePack (code:string hashCodes:[string])
      "Buy license pack"
      (with-read licensePackTypes1 code
        {"priceCRKK" := priceCRKK,
         "units":= units}
          (enforce (= units (length hashCodes)) "Provide the exact number of hashes")
          (insert licensePacks1 (compound-key2 (at "sender" (chain-data)) (get-formattedTime (get-time)))  
            {"address": (at "sender" (chain-data)),
            "guard": (read-keyset "keyset"),
            "boughtAt": (get-time),
            "CRKKPaid": priceCRKK,
          ...
          (with-capability (INTERNAL)
              (fold (insert-hashCode) 0 hashCodes)
            ;   (free.crankk01.transfer (at "sender" (chain-data)) NEW_LIC_BANK priceCRKK)
          )    
      )
  )
  (defun insert-hashCode (dummy:integer hashCode:string)
    (require-capability (INTERNAL))
      (insert licenseHashes1 hashCode
        {"hashCode": hashCode,
         "used": false,
         "usedBy": ""}
      )
  ...
  (defun reverse-license (address)
      "Reverse license - admin use"
    (with-read radmin "admin"
    { "aguard" := aguard }
      (enforce-guard aguard)
      (with-read licenses6 address
        {"kdaPaid":= kdaPaid,
         ...
         "refunded":= refunded}
          (enforce (= used false) "Already used")
          (enforce (= refunded false) "Already refunded")
          (install-capability (coin.TRANSFER RADIO01_LICENSE_BANK address kdaPaid))
          (coin.transfer RADIO01_LICENSE_BANK address kdaPaid)
          (update licenses6 address 
            {"refunded": true,
            "refundedAt": (get-time)}
          )
  ...
  (deftable subscriptionMenu3:{subscriptionMenu})

  (defun insert-subscription-menu (   
    subscriptionId:string
    periodDays:integer
    value:decimal
    valueUnit:string
    ...
    (with-read radmin "admin"
        { "aguard" := aguard }
        (enforce-guard aguard)
        (insert subscriptionMenu3 subscriptionId
        {
  ...
  (deftable subscriptions2:{subscription})

 (defun buy-subscription (   
    subscriptionId:string)
    (enforce-guard (at 'guard (coin.details (at "sender" (chain-data)))))
    (with-read subscriptionMenu3 subscriptionId
        {
        "periodDays":= periodDays,
        "value":= value,
        "valueUnit":= valueUnit,
        ...
        (insert subscriptions2 (compound-key2 (at "sender" (chain-data)) subscriptionId)
            {
            "address": (at "sender" (chain-data)),
            "subscriptionId": subscriptionId,
            "purchaseDate": (get-time),
            "validUntil": (add-time (get-time) (days periodDays)),
            "active":true
            }
        )
        (if (= paidIn "KDA")
            (coin.transfer (at "sender" (chain-data)) RADIO01_SUBSCR_BANK
             (get-subscription-price subscriptionId))
            (if (= paidIn "CRKK")
                (free.crankk01.transfer (at "sender" (chain-data)) RADIO01_SUBSCR_BANK
                (get-subscription-price subscriptionId))
                (enforce false "Not supported payment token")
            )
        )
    )
  )    

  (defun get-subscription-price (subscriptionId)
    ...
    (with-read subscriptionMenu3 subscriptionId
        ...
        (with-read exchangeRates1 (compound-key2 valueUnit paidIn)
            {"rate":= rate}
            (round (* value rate) 2)
        )
    )    
  ...
  (defun get-subscription-menu ()
    "Get subscription-menu"
    (select subscriptionMenu3 (constantly true))
  )

  (defun get-subscriptions ()
    "Get subscriptions"
    (select subscriptions2 (constantly true))
  )
  ...
  (defun get-exchangeRates () 
    (select exchangeRates1 (constantly true)) 
  )
  ...
  (defun get-licensePackTypes () 
    (select licensePackTypes1 (constantly true)) 
  )
...
(defun init-subscr-bank ()
      (free.crankk01.transfer-create (at "sender" (chain-data)) RADIO01_SUBSCR_BANK 
...
; create-table
; (create-table subscriptionMenu3)
; (create-table subscriptions2)
; (create-table exchangeRates1)
;
;
