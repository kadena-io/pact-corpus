...
; ============================================
    ...
    (defun mint (account:string amount:integer)
        ...
        (enforce (> 0 amount) "you must mint at least 1 NFT")
        (let*
            (
                (mint-status (enforce-mint-status false))
                (mints-remaining (get-whitelist-details account))
                (tiempo (get-time))
                (isWhitelistTime (if (and (> tiempo (time "2022-06-01T12:00:00Z")) 
                                (< tiempo (add-time (time "2022-06-01T12:00:00Z") 86400)))
                                true false))
                (price (if (= true isWhitelistTime) (get-price WHITELIST_PRICE_KEY) (get-price PRICE_KEY)))
            )
            (if (= true isWhitelistTime) 
                (enforce (<= amount mints-remaining) "Cannot mint more than your allocated whitelist mints. You have {} remaining. Come back during the public sale in < 24 hours")
                (coin.transfer account ADMIN_ADDRESS price)
                (with-capability (PRIVATE)
                    (coin.transfer account ADMIN_ADDRESS (* price amount))
                    (map (marmalade-mint account (coin-account-guard account)) (make-list amount))
                )
            )
        )
    )
    ...
    (defun marmalade-mint (account:string account-guard:guard)
    ...
    (defun enforce-mint-status (wl:bool) ; wl is set to true if checking for whitelist
        ...
        (with-read mint-status MINT_STATUS
            ...
            (if (= true wl)
                ...
                (enforce (or (= status "started")) "mint is not running, so no write functions will work")
            )
        status
        )
    )
