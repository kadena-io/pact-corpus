(module kda-skellies GOV
              ...
              (let* 
                ...
                ;   (coin.transfer curr-user minted-by minter-fee)
                  (if 
                      ...
                      (coin.transfer curr-user minted-by (+ minter-fee to-seller-amount)) ; combined
                      true
                  )
                  (if 
                      (!= original-owner minted-by)
                      (coin.transfer curr-user minted-by minter-fee) ; minted-by != seller
                      true
                  )
                  (if 
                      (!= original-owner minted-by)
                      (coin.transfer curr-user original-owner to-seller-amount) ; seller != minted-by
                      true
                  )
                  (coin.transfer curr-user CREATOR_ADDRESS fee)
                  (write marketplace id {
    ...
    ;   (if 
    ;       (> (length fields) 0)
    ;       (+ {"id": id, "minted-by": (at "minted-by" (read nfts id ["minted-by"]))} (read marketplace id fields))
    ;       (read marketplace id)
    ;   )
          (+ {"id": id, "minted-by": (at "minted-by" (read nfts id ["minted-by"]))} (read marketplace id fields))
    )
    ...
    (defun get-owner-items-on-sale (owner:string)
      ...
      (defun get-nft-fields-for-id (fields:list id:string )
          @doc "Return the fieldsxx for a given id"
