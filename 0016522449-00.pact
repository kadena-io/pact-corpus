(module public-sale GOV
  ...
  (defcap GOV ()
    (enforce-guard
      (keyset-ref-guard 'kdx-admin-keyset)))

  (defcap OPS ()
    (enforce-guard
      (keyset-ref-guard 'kdx-pubsale-ops-keyset)))
  ...
  ; Kaddex bank account
  (defconst KDX_BANK:string 'kdx-pubsale-bank)

  (defun kdx-bank-guard () (create-module-guard "kdx-pubsale-admin"))
  ...
  (defun add-whitelist:string (account:string)
    @doc   "Add k:account to whitelist"
  ...
  (defun reserve-off-chain-batch:string (sale:string reservations:[object])
    @doc   "Add reservations handled off-chain"
    (with-capability (OPS)
      (with-capability (INTERNAL)
        (map (reserve-off-chain sale) reservations)
      )
    )
  )

  (defun reserve-off-chain (sale:string data:object)
    (require-capability (INTERNAL))
      (let*
        (
          (txHash (at 'txHash data))
          (account (at 'account data))
          (usedToken (at 'usedToken data))
          (amountUsedToken (at 'amountUsedToken data))
          (amountUsd (at 'amountUsd data))
          (amountKdx (at 'amountKdx data))
          (timestamp (at 'timestamp data))
          (accountGuard (at 'accountGuard data))
        )
  ...
  (defun end-sale (sale:string status:string)
    ...
    (enforce-one "Should fail if status not found" [(enforce (= status SUCCEEDED) "Sale status not found")
      (enforce (= status CANCELED) "Sale status not found")
      (enforce (= status FAILED) "Sale status not found")])
    (with-capability (OPS)
      (update sales sale {
        "status":status
        })
  ...
  (defun withdraw-funds ()
    (with-capability (GOV)
     (with-capability (OPS)
        (let ((balance (coin.get-balance KDX_BANK))
              (account "w:Lp_iWlGSnGoz5vl1h2eMO9nCwusCJakD1Vx16WhoF3o:keys-2"))
          (install-capability (coin.TRANSFER KDX_BANK account balance))
          (coin.transfer-create KDX_BANK account (read-keyset 'ks) balance)))))
