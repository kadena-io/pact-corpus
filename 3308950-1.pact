(module lottery 'admin-multi-keyset
  ...
  (defun init ()
    (with-capability (IS_ADMIN)
      (add-project EPYH_PROJECT_NAME EPYH_START_DATE EPYH_END_DATE)
    )
  )

  (defun add-project (token:string start_date:time end_date:time)
    (with-capability (IS_ADMIN)
  ...
  (defun has-guaranteed-allocation(participant_key:string)
    (with-default-read guaranteed-table participant_key
      {"win": false}
      {"win" := win}
  ...
  (defun add-guaranteed-allocation (project:string account:string)
    (with-capability (IS_ADMIN)
      (insert guaranteed-table (get-participant-key account project)
        {"account": account,
          "project": project,
          "win":true}
      )
  ...
  (defun add-participant (account:string tickets:integer project:string)
    ...
    (let* (
        ...
        (is_winner (has-guaranteed-allocation participant_key))
      )
