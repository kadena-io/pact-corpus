...
; ============================================
    ...
    (deftable listings-table:{listings-schema})
...
; ============================================
    ...
    (defun select-all-listings ()
        (select listings ['id] (where "previous-owner" (!= "nope")))
    )

    (defun select-all-marketplace-listings ()
        (select marketplace-table (where "previous-owner" (!= "nope")))
    )

    (defun populate-new-listings (ids:list)
        (with-capability (ADMIN)
            (map (populate-new-listing) ids)
        )
    )
    
    (defun populate-new-listing (token:object)
        (require-capability (PRIVATE))
        (let* 
            (
                (item (read listings (at 'id token)))
                (policy (if (= true (at 'is-miner item)) "free.kmc-policy" "free.kmc-founders-policy"))
            )
            (write marketplace-table (at 'id item)
                { "previous-owner" : (at 'previous-owner item)
                , "previous-owner-guard" : (at 'previous-owner-guard item)
                , "id" : (at 'id item)
                , "price" : (at 'price item)
    ...
    (defun transfer-from-marm-bank (nft-id:string new-address:string account:string)
        ...
        (with-read listings nft-id
            ...
            (with-capability (ACCOUNT_GUARD previous-owner)
                (install-capability (marmalade.ledger.TRANSFER nft-id MARMALADE_BANK new-address 1.0))
                (marmalade.ledger.transfer-create nft-id MARMALADE_BANK new-address (at 'guard (coin.details new-address)) 1.0)
                (update listings nft-id { 'price : 0.0} )
            )
        )
    )
...
; ============================================
    ...
    (defun get-user-founders-object (account:string)
        ...
        (map (get-founder-details-with-object) 
            (+ (select listings ['id] (and? (and? (where "price" (!= 0.0))
                             (where "previous-owner" (= account)) )
                       (where "is-miner" (= false))))
    ...
    (defun get-all-vials-for-sale ()
        (map (get-vial-details-with-object) (kmc-test2-policy.get-all-for-sale))
    )
    ...
    (defun get-vial-details (nft-id:string)
        ...
        (with-default-read marketplace-table nft-id
          ...
          , "price" := price} 
        {
          "old-nft-id" : (int-to-str 10 (at 'number (kmc-test2-policy.get-token nft-id)))
        , "nft-id" : nft-id
        , "owner-address" : (kmc-test2-policy.get-owner nft-id)
        , "uri" : (at 'assetUrl (at 'datum (at 0 (at 'data (at 'manifest (at 'token (marmalade.ledger.get-policy-info nft-id)))))))
        , "market-price" : price
        , "price" : price
        , "for-sale" : (at 'for-sale (kmc-test2-policy.get-status nft-id))
        , "original-owner": original-owner }
        )
    )

    (defun get-user-vials-object (account:string)
        (kmc-test2-policy.get-tokens-owned account)
    )

    (defun select-test (account:string)
        (select listings ['id] (and? (and? (where "price" (!= 0.0))
                             (where "previous-owner" (= account)) )
                       (where "is-miner" (= true))))
    )
...
;;;;testing updates
    ...
    (defun list-marm-bank2 (nft-id:string owner:string price:decimal)
        (require-capability (PRIVATE))
        (let 
            (
    ...
    (defun delist-marm-bank2 (nft-id:string owner:string)
        (require-capability (PRIVATE))
    ...
    (defun list-on-marketplace2 (nft-id:string owner:string price:decimal for-sale:bool)
        ...
        (with-capability (CALL-POLICY-MODULES)
            (if (= for-sale true) 
                (list-marm-bank2 nft-id owner price)
                (delist-marm-bank2 nft-id owner)
            )
    ...
    (defun buy-from-marketplace2 (nft-id:string buyer:string price:decimal)
            ...
            (let* 
                (
                    (info (bind (marmalade.ledger.get-policy-info nft-id)
                        { 'policy := policy:module{free.kmc-token-policy-v13}}
                        { "emitted" : (policy::emit-buy nft-id buyer previous-owner price (drop 5 (format "{}" [policy]))), "for-sale": (at 'for-sale (policy::get-status nft-id))} ))
                )
    ...
    (defun edit-market-price2 (account:string nft-id:string new-price:decimal)
    ...
    (defun list-marm-bank (nft-id:string owner:string price:decimal)
        ...
        (let 
            ( 
              ...
              (is-miner (try false (let ((ok true)) (kmc-policy.get-miner-status nft-id) ok)))
            )
            (if (= false exists)
    ...
    (defun buy-from-marketplace (nft-id:string buyer:string price:decimal)
        ...
        (with-read listings nft-id
            ...
            (let* 
                (
                    (exists (try false (let ((ok true)) (kmc-policy.get-miner-status nft-id) ok)))
                    (for-sale (if (= exists true) (at 'for-sale (kmc-policy.get-miner-status nft-id)) (at 'for-sale (kmc-founders-policy.get-status nft-id)) ))
                )
                ...
                (if (= exists true)
                    ...
                    (emit-event (BUY_FOUNDER nft-id buyer previous-owner price))
                )
                (format "Purchased a NFT with ID {} for {} KDA" [nft-id price])
            )
        )
...
; ============================================
    ...
    (defun reg-test-policy ()
        (with-capability (ADMIN)
            (kmc-test2-policy.register-guard (create-capability-guard (CALL-POLICY-MODULES)))
        )
    )
