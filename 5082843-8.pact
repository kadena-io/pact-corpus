...
; ============================================
    ...
    (defun add-new-affiliation (project:string asics-remaining:integer payout-address:string associated-asics:list price:decimal)
        ...
        (with-capability (DISCORD)
            (kmc-your-miner.add-new-project project asics-remaining payout-address associated-asics)
    ...
    (defun get-asic-details (nft-id:string)
        ...
        (with-default-read marketplace-table nft-id
          ...
          (let*  (
                    ...
                    (contract-cost-in-kda (round (/ (* (* contract-days-remaining (* electric-rate 24)) (/ wattage 1000.0)) kda-price) 1))
                    (asic-cost-in-kda (/ price kda-price))
                )
            {
        ...
        (with-read marketplace-table nft-id
            ...
            (let* 
                (
                    (info (bind (marmalade.ledger.get-policy-info nft-id)
                        { 'policy := policy:module{free.kmc-token-policy-v13}}
                        { "emitted" : (policy::emit-buy nft-id buyer previous-owner price (format "{}" [policy])), "for-sale": (at 'for-sale (policy::get-status nft-id)), "nft-your-miner-protection" : (enforce (!= (format "{}" [policy]) "free.kmc-your-miner") "Cannot buy kmc-your-miner with this function") }                   
                        ))
                )
        ...
        (with-read marketplace-table nft-id
            ...
            (let*
                (
                    ...
                    (electric-rate (at 'electric-rate asic-details))
                    (wattage (str-to-int (at 'Wattage (at 'asic-details asic-details))))
                    (kda-price (kmc-oracle.get-price "kda-price-key"))
                    (contract-days-remaining (/ (diff-time (at 'electric-expiry asic-details)) (at "block-time" (chain-data))) 86400.0)
                    (contract-cost-in-kda (/ (* contract-days-remaining (* (* electric-rate 24) wattage))) kda-price)
                    (asic-cost-in-kda (/ ledger-price kda-price))
                    (info (bind (marmalade.ledger.get-policy-info nft-id)
                        { 'policy := policy:module{free.kmc-token-policy-v13}}
                        { "emitted" : (policy::emit-buy nft-id buyer previous-owner price (format "{}" [policy])), "for-sale": (at 'for-sale (policy::get-status nft-id))} ))
                    (total-cost (+ asic-cost-in-kda contract-cost-in-kda))
                )
                (enforce (= (at 'for-sale info) true)  "You can only purchase an NFT that is for sale." )
                ...
                (install-capability (marmalade.ledger.TRANSFER nft-id MARMALADE_BANK buyer 1.0))
                (marmalade.ledger.transfer-create nft-id MARMALADE_BANK buyer (at 'guard (coin.details buyer)) 1.0)
                (format "Purchased a NFT with ID {} for {} KDA and contact info of {}" [nft-id total-cost contact-info])
            ))
        )
    )
