(module kmc-vial-policy GOVERNANCE
  @doc "Kadena Mining Club Immersion Vial policy"
    ...
    (deftable legendary-vials:{legendary-schema})
    ...
    (defun initialize ()
        ...
        (insert legendary-vials "list" {"token-ids" : 
            ["t:C1Oeetqgeuq3UTVu3x23uwF4dG7-AdQq_zst7mfwck4",
             "t:FIEfIBUV0MtXYZE2nqnGvnuT88JT_ZvuTNx72PqBfGk",
             "t:VTBSs3r_DpPu2meMd_0IT4QCUnvgUK9KhgaBY7vMyR8"]})
    )
    
    (defun update-legendary-vials (ids:list)
        (with-capability (ADMIN)
            (update legendary-vials "list" 
                {"token-ids" : ids }))
    )

    (defun update-whitelist-multiple (number-of-mints:integer accounts:list account:string)
    ...
    (defun add-legendary (token-id:string)
        (require-capability (PRIVATE))
        (with-read legendary-vials "list"
            { "token-ids" := token-ids }
            (update legendary-vials "list"
    ...
    (defun get-all-minted-legendary-vials ()
        (at 'token-ids (read legendary-vials "list"))
    )
    ...
    (defun enforce-mint:bool
        ...
        (if (=  "Legendary" 
                (at 'value (at 0 (at 'attributes (at 'datum (at 0 (at 'data 
                    (at 'manifest (at 'token (marmalade.ledger.get-policy-info (at 'id token))))))))))
                )
            (with-capability (PRIVATE)
                (add-legendary (at 'id token)))
            "")
        (update kmc-tokens (at 'id token) 
            {"supply": 1.0
            ,"owner": account }
...
; ============================================
    ...
    (defun internal-mint (account:string)
        (require-capability (PRIVATE))
        (let*
            (
                ...
                (isWhitelistTime (if (and (> tiempo (time "2023-06-31T16:00:00Z")) 
                                (< tiempo (add-time (time "2023-07-01T16:52:00Z") 86400)))
                                true false))
                (price (get-current-nft-price))
            )
            (if (= account MINT_ADDRESS)
                (coin.transfer MINT_ADDRESS ADMIN_ADDRESS 0.001)
        ...
        (let*
            (
                (isWhitelistTime (if (and (> (get-time) (time "2023-06-31T16:00:00Z")) 
                                (< (get-time) (add-time (time "2023-07-01T16:52:00Z") 86400)))
                                true false))         
