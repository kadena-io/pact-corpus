(module kapepe-nft-test GOVERNANCE
  ...
  (deftable nft-collection:{collection-schema})

  ; Creating

  (defun create-collection ()
    (with-capability (GOVERNANCE)
      (insert nft-collection COLLECTION_ID
        {'id: COLLECTION_ID
         ,'non-minted-tokens: []
         ,'minted-total: 0
    ...
    (defun create-nfts-bulk (items:list)
      ...
      (let*
        ...
        (with-capability (GOVERNANCE)
          (let*
            (
              (token-ids (map (lambda (item) (create-nft-token collection COLLECTION_ID)) items))
    ...
    (defun create-nft-token (datum:object)
      ...
      (let*
        ...
        (marmalade.ledger.create-token token-id 0 manifest kip.token-policy-v1)
        token-id
        )
    )
  ...
  (defun update-non-minted-collection (token-ids:list collection:object{collection-schema})
    ...
    (bind collection
      ...
      (update nft-collection COLLECTION_ID
  ...
  (defun get-collection:object ()
    (read nft-collection COLLECTION_ID)
  )
