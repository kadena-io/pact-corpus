(module mainnetmodulemakagyn1 GOV ; ---- CHANGE ON NEW
  ...
  (defun buy-one (pixel-id:integer account:string color:string request-price:decimal is-new-pixel:bool)
    "buy a pixel"
    ...
    (let
      ...
      (with-capability (INTERNAL "buy")
        (if is-new-pixel
          (buy-new pixel-id-str account color place-price request-price true)
          (buy-used pixel-id-str account color place-price request-price true)
        )
      )
    )
  )
            ...
            (lambda
              ...
              (let
                ...
                (let
                  ...
                  (with-capability (INTERNAL "buy")
                    (if is-new-pixel
                      (buy-new pixel-id-str account color place-price request-price false)
                      (buy-used pixel-id-str account color place-price request-price false)
                    )
                  )
                )
              )
  ...
  (defun get-pixel-data:object (pixel-id:integer)
    ...
    (let
      ...
      (+ (read pixel-table pixel-id-str) (read color-table pixel-id-str))
    )
  )
