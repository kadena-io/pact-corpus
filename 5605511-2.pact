...
; ============================================
    ...
    (defun initialize ()
        ...
        (insert counts-table CURRENT_OFFER_ID {"count": 0})
    ...
    (defun place-collection-offer (account:string offer:decimal duration:integer) ;expiry is number of seconds
        ...
        (with-capability (PRIVATE)
            (let 
                ...
                (insert collection-offer-table (int-to-str 10 (get-count CURRENT_OFFER_ID))
                    { "address": account
                    , "valid-until": valid-until-time
                    , "amount": offer
                    , "offer-id": (get-count CURRENT_OFFER_ID) })
                (increase-count CURRENT_OFFER_ID)
                (format "Your offer of {} KDA has been submitted with an expiration time of {} UTC. You can remove your offer at any time. If your offer expires, you must come back and collect your KDA"
    ...
    (defun accept-collection-offer (account:string nft-id:string offer-id:string)
        ...
        (with-read collection-offer-table offer-id
            ...
            (update collection-offer-table offer-id
                { "valid-until" :  (time "2022-08-27T19:00:00Z") }
            )
            (format "Offer accepted successfully. NFT #{} has been transferred to {} and {} KDA has been sent to {}, less a 5% marketplace fee"
    ...
    (defun remove-collection-offer (account:string offer-id:string)
        ...
        (with-capability (ACCOUNT_GUARD account)
            (with-read collection-offer-table offer-id
                ...
                (enforce (!= valid-until (time "2022-08-27T19:00:00Z")) "Your offer has already been withdrawn")
                (update collection-offer-table offer-id
                    { "valid-until" : (time "2022-08-27T19:00:00Z")} 
                )
...
; ============================================
    ...
    (defun create-simple-user-guard (funder:string amount:decimal)
        (with-capability (ADMIN)
            (coin.transfer-create funder COLLECTION_OFFER_ACCOUNT 
