(module eighties-bulls 'bulls-keyset
  "80s Bulls"
    ...
    (defun sell-my-bull ( id:string owner:string price:decimal forsale:string)
        @doc "Put a Bull on the Market or update one already for sale "
        (with-read minted-nfts (key id owner)
          { 'id := l_id, 
          'owner := l_owner }
          ;Enforce rules
          (with-capability (ACCOUNT_GUARD id l_owner)
            ...
            (if (= forsale true) (format "Bull with ID {} is now for sale for {}" [id price]) (format "Bull with ID {} is no longer for sale" [id]))
          )
        )
    )
    ...
    (defun buy-bull-off-market
        ...
        (with-read minted-nfts (key id owner)
          ...
          'forsale := m_forsale,
          'price := m_price }
            (enforce (= m_forsale true)  "You can only purchase a Bull that is for sale." )
