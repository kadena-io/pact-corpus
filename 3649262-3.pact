(module corona-token GOVERNANCE

  @doc " '$CRNA' is a decentralised meme token supported by community on Kadena Blockchain. Saviour of the #EARTH. "
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset "free.corona-inu-admin")
  )
  ...
  (defconst ADMIN_ACCOUNT_ID "k:955ffc803b23f1332da6b974b36a08b827afd347b18ec04842e6f4260064e061"
    " ID for admin accont" )

  (defconst INITIAL_SUPPLY:decimal 50000000000.0
    " Initial supply of 50 billion $CRNA tokens.")
 ...
 (defconst PUBLIC_WL_KEY "public_wl_role"
  ...
  (defconst PREMINE_SALE_AMOUNT:decimal 15000000000.0
  ...
  (defconst CRNAS_PER_KDA 300000.0
    " How much $CRNA tokens 1 KDA can buy. ")
  ...
  (defun ido-sale:string
    ...
    @doc " Function to let people buy the token. "
      (with-capability (INTERNAL)
        (let
          (
              (amount-crnas (* amount-kda CRNAS_PER_KDA ) )
          )
          (enforce-passes-wl account amount-crnas)
          (increase-premined-amount amount-crnas)
          (coin.transfer account ADMIN_ACCOUNT_ID amount-kda)
          (install-capability (TRANSFER ROOT_ACCOUNT_ID account amount-crnas) )
          (transfer-create ROOT_ACCOUNT_ID account guard amount-crnas )
        )
      )
  )
  ...
  (defun enforce-wl-buy-conditions (user-id:string buy-amount:decimal user-wl-role:string)
    ...
    (let
        ...
        (enforce (>= new-balance (min-buy-crnas)) " Must buy above minimum amount" )
        (enforce (<= new-balance (max-buy-crnas)) " Cannot buy above maximum amount" )
    )
  )
  ...
  (defun min-buy-crnas ()
    @doc " Minimum amount in $CRNA's a user can buy "
    (* MIN_BUY_KDA CRNAS_PER_KDA)
  )

  (defun max-buy-crnas ()
    @doc " Minimum amount in $CRNA'S a user can buy "
    (* MAX_BUY_KDA CRNAS_PER_KDA)
  )
