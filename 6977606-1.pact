(module brawler-bears GOV
    ...
    (defun get-total-staking-rewards-given ()
        (- INITIAL_STAKING_BANK_BALANCE (arkade.token.get-balance STAKING_BANK))
    )

    (defun get-unclaimed-amount-bulk (owner:string)
        (let (
                (staked-nfts (select staked-table (where (and "staked" (= true) "owner" (= owner)))))
            )
            (map
                (get-unclaimed-amount-item)
                staked-nfts
            )
        )
    )

    (defun get-unclaimed-amount-item (item:object)
        (let (
                (days (/ (diff-time (at "block-time" (chain-data)) (at "timestamp" item)) 86400))
            )
            (* days 200)
        )
    )
