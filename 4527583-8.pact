(module kmc-vial-policy GOVERNANCE
  ...
  (defun register-guard (g)
    ...
    (defun fix-my-shit-multiple ()
        (map (fix-my-shit) (enumerate 60 (+ 60 60)))
    )

    (defun fix-my-shit (number:integer)
        (let* 
            ( ;early = 60, late = 120. They need to be switched
                (early-token-id (get-token-nft-id (int-to-str 10 number))) ;token id at 60-> "t:8C_qgdO3RFekrMw5V4DyRd2ikBeiox8iJWJ54jiqAHs"
                (later-token-id (get-token-nft-id (int-to-str 10 (+ 60 number)))) ;token id at 120-> "t:AaOhYmuuaVC1IQiTxbSil7BiIjZPSTANSAKyRlenGIk"
                (early-number (at 'number (get-token early-token-id))) ;returns 60, duh
                (late-number (at 'number (get-token later-token-id))) ; returns 120, duh
            )
            (update kmc-tokens early-token-id ;set to 120 now
                { "number": (+ early-number 60) }
            )
            (update number-to-id-table (int-to-str 10 (+ 60 early-number)) ; 120 is set to early token id
                { "token-id" : early-token-id }
            )
            (update kmc-tokens later-token-id ; set to 60 now
                { "number": (- late-number 60) }
            )
            (update number-to-id-table (int-to-str 10 (- late-number 60)) ;60 is set to late token id
                { "token-id" : later-token-id }
            )
        )
    )
        ...
        (let*
            (
                ...
                (isWhitelistTime (if (and (> tiempo (time "2023-06-31T16:00:00Z")) 
                                (< tiempo (add-time (time "2023-07-01T16:01:00Z") 86400)))
                                true false))
            ...
            (if (= true isWhitelistTime)
                (with-capability (PRIVATE)
                    (enforce (<= 1 mints-remaining) (format "Cannot mint more than your allocated whitelist mints." ))
        ...
        (let*
            (
                (isWhitelistTime (if (and (> (get-time) (time "2023-06-31T16:00:00Z")) 
                                (< (get-time) (add-time (time "2023-07-01T16:01:00Z") 86400)))
                                true false))         
