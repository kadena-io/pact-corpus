(module kapepe-coin GOVERNANCE

  @doc " Making Kadena Great Again"
  ...
  (bless "fBHmiQIkvXCn7xSH54TlEHrwMF9RwU3lLMld8ckSP9o")
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset "free.admin-kapepe")
  )
  ...
  (defconst INITIAL_SUPPLY:decimal 7700000000.0
    " Initial supply of 7.7 billion")
  ...
  (defun debit-swap
    ...
    (enforce (> amount 0.0) "Debit amount must be positive.")
    (enforce-unit amount)
    (require-capability (n_5a7ccd559b245b7dcbd5259e1ee43d04fbf93eab.kapepe.DEBIT accountId))
  ...
  (defpact transfer-crosschain:string
    ...
    (step
      (with-capability (DEBIT sender)
        ...
        (enforce (= (at 'chain-id (chain-data)) "1") "Transfers are only allowed from chain 1")
        (enforce (!= "" target-chain) "empty target-chain")
        (enforce (!= "1" target-chain) "empty target-chain")
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
      (create-account ROOT_ACCOUNT_ID (keyset-ref-guard "free.admin-kapepe"))
      (update token-table ROOT_ACCOUNT_ID { "balance" : INITIAL_SUPPLY })
    )
  )

  (defun get-current-holders ()
    @doc " Return all token holders. "
    (keys token-table))

  (defun get-current-holders-balances (accounts:list) 
