(module looney-bulls-airdrop GOV
  "Looney Bulls Airdrop for $ARKD IDO Participants"

    (defconst NFTS_TO_MINT_COUNT 340.0)  
    (defconst MAX_WL_MINT 1)
    (defconst MINT_START_TIME "2022-09-22T19:00:00Z")
    (defconst MINT_END_TIME "2022-10-01T19:00:00Z")
    ...
    (defconst OFFERS_BANK:string "looney-bulls-offers-bank")
    (defconst URI_KEY "uri-key")
    (defconst IMAGES_URI "https://arkade-prod.s3.us-east-1.amazonaws.com/looney-bulls")
    ...
    (defcap OWNER (id:string)
        ...
        (let 
            (
                (nft-owner (at "owner" (read airdrop-nfts id ["owner"])))
            )
    ...
    (defun initialize ()
        ...
        (insert price PRICE_KEY {"price": 0.0})
    ...
    (deftable airdrop-nfts:{nft-main-schema})
    ...
    (defun mint-nfts-bulk (owner:string amount:integer)
       @doc "Mints NFTs bulk"
        ...
        (enforce (= 1 amount) "Can only mint 1 max")
        (enforce-mint-live)
    ...
    (defun mint-nft-id (owner:string id:string)
        @doc "Mints nft as admin for giveaways"
        (with-capability (ADMIN)
            (insert airdrop-nfts id {
                "id": id,
                "item": { "edition": id },
    ...
    (defun transfer:string (receiver:string id:string)
        ...
        (with-capability (OWNER id)
            ...
            (update airdrop-nfts id {"owner": receiver})
        )
    )
    ...
    (defun buy-id-on-sale (id:string curr-user:string)
        ...
        (with-capability (ACCOUNT_GUARD curr-user)
            ...
            (let* 
                ...
                (update airdrop-nfts id (+ {"owner": curr-user} nft-data ))
                (with-capability (PRIVATE)
    ...
    (defun make-offer (id:string buyer:string duration:integer amount:decimal)
        ...
        (let (
            (original-owner (at "owner" (read airdrop-nfts id ["owner"])))
        ...
        (with-read offers id
        {
            ...
            (with-capability (OWNER nft-id)
                ...
                (let* 
                    ...
                    (update airdrop-nfts nft-id (+ {"owner": buyer} nft-data ))
                    (with-capability (PRIVATE)
    ...
    (defun get-all-on-sale ()
        ...
        (select marketplace ["id", "price", "updated-at", "owner"] (where "for-sale" (= true)))
    )
    ...
    (defun enforce-mint-live ()
        ...
        (enforce (>= (at "block-time" (chain-data)) (time MINT_START_TIME)) "Mint is not live.")
    ...
    (defun ids-owned-by (owner:string)
        ...
        (select airdrop-nfts ["id"] (where "owner" (= owner)))
    )
    ...
    (defun get-nft-fields-for-id (fields:list id:string )
        @doc "Return the fields for a given id"
        (+ {"id": id} (read airdrop-nfts id fields))
    )
    ...
    (defun patch-count (num:decimal key:string)
        (with-capability (ADMIN)
            (update counts key {"count": num})
        )
    )

    (defun backfill-nft-items (items:list)
        @doc "Backfills nft table from old contract"
        (with-capability (ADMIN)
            (map 
                (backfill-nft-item)
                items
    ...
    (defun backfill-nft-item (item:object)
        (require-capability (ADMIN))
        (write airdrop-nfts (at "id" item) 
            {
                "date-minted": (time (at "date-minted" item)),
                "id": (at "id" item),
                "item": {"edition": (at "id" item)},
                "owner": (at "owner" item)
            }
        )
    )

    (defun backfill-marketplace-items (items:list)
        @doc "Backfills marketplace table from old contract"
        (with-capability (ADMIN)
            (map 
                (backfill-marketplace-item)
                items
            )
        )
    )

    (defun backfill-marketplace-item (item:object)
        (require-capability (ADMIN))
        (write marketplace (at "id" item) 
            {
                "updated-at": (time (at "updated-at" item)),
                "id": (at "id" item),
