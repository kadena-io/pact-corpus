(module sgk-rewards GOVERNANCE
    ...
    (defun claim-rewards (account:string tokens:[string] collection:string)
        @doc "Allows a user to claim multiple rewards"
        (enforce (or (= collection BATTLE_HEROES) (= collection GEN_ZERO)) 
            (format "Collection must either be {} or {}" [BATTLE_HEROES GEN_ZERO]))
        (with-capability (PRIVATE)
        (with-capability (ACCOUNT_GUARD account)
            (if (= collection BATTLE_HEROES)
                (lambda-distribute-rewards-battle-heroes tokens)
                (lambda-distribute-rewards-gen-zero tokens)
            )
        ...
        (with-capability (OWNER token-id)
            (let*
                ...
                (if (= claim-status false)
                    [
                        (map (n_f1c962776331c4773136dc1587a8355c9957eae1.isoko-orchestrator.TEST_bulk-claim-weapons-staking account) (make-list claim-count guard))
        ...
        (let*
            ...
            (if (and (= stake-status true) (= rewardable true))
                [
                    ...
                    (n_7d47538766e6f80008f253dccd30451f4d483c38.battle-heroes-nft-policy.update-lock-time token-id)
                    (increase-count REWARD_COUNT)
                ]
    ...
    (defun testing (token-id:string)
        ...
        (update counts-table key {"count": 0})
    )
