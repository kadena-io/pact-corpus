...
(defcap GOVERNANCE ()
    ...
    (defschema policy-ref-schema 
        policy:module{n_f1c962776331c4773136dc1587a8355c9957eae1.queryable-collections-policy-v2}
    )
    (deftable policy-ref-table:{policy-ref-schema})
...
(defun get-minted-supply-prod:integer ()
  (let* 
      (
        (supply-list:list (map (get-total-minted-supply-from-contract) SGK_WEAPONS_LEVEL_ONES_PROD))
        (sum (fold (+) 0 supply-list))
      )
      sum
  )
)

(defun get-public-minted-supply-prod:integer ()
  (let* 
      (
        (supply-list:list (map (get-total-public-minted-supply-from-contract) SGK_WEAPONS_LEVEL_ONES_PROD))
        (sum (fold (+) 0 supply-list))
      )
      sum
  )
...
(defun get-minted-supply-test:integer ()
  (let* 
      (
        (supply-list:list (map (get-total-minted-supply-from-contract) SGK_WEAPONS_LEVEL_ONES_DEVO))
        (sum (fold (+) 0 supply-list))
      )
      sum
  )
...
(defun get-public-minted-supply-test:integer ()
  (let* 
      (
        (supply-list:list (map (get-total-public-minted-supply-from-contract) SGK_WEAPONS_LEVEL_ONES_DEVO))
        (sum (fold (+) 0 supply-list))
      )
      sum
  )
...
(defun get-total-minted-supply-from-contract (policy-name:string)
    (let* 
      (
        (policy:module{n_f1c962776331c4773136dc1587a8355c9957eae1.queryable-collections-policy-v2} (get-policy policy-name))
        (supply:integer (policy::get-minted-supply))
      )
      supply
    )
)

(defun get-total-public-minted-supply-from-contract (policy-name:string)
    (let* 
      (
        (policy:module{n_f1c962776331c4773136dc1587a8355c9957eae1.queryable-collections-policy-v2} (get-policy policy-name))
        (supply:integer (policy::get-public-minted-supply))
      )
      supply
    )
)
...
(defun get-policy (policy-name:string)
  (at "policy" (read policy-ref-table policy-name))
)
