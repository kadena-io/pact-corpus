...
; ============================================
    ...
    (defun initialize ()
        ...
        (insert counts-table CURRENT_MINER_ID_COUNT {"count": 1})
...
; ============================================
    ...
    (defun mint-miner-multiple (account:string number-to-mint:integer usdc:bool)
        ...
        (with-default-read counts-table CURRENT_MINER_ID_COUNT ;starts at 01
            { 'count: 0.0 }
            { 'count := current-count }
            (enforce (>= WHITELIST_MAX_MINT current-count ) (format "Max supply reached, current supply is {}" [current-count]))
        )
;;remove after whitelist below
        (let 
            ( 
              (is-on-whitelist (try false (let ((ok true)) (get-wl-mints-remaining account) ok)))
            )
            (enforce (= is-on-whitelist true) "The first hour of minting is reserved for whitelisted members only")
            (let 
                ( 
                  (wl-mints-remaining (get-wl-mints-remaining account))
                )
                (enforce (>= wl-mints-remaining number-to-mint) (format "You only have {} whitelist mints remaining, please try again" [wl-mints-remaining]))
                (update wl-remaining-per-account account {"wl-mints-remaining":(- wl-mints-remaining number-to-mint)})
            )
        )
; remove after whitelist above
        (with-capability (ACCOUNT_GUARD account)
            (with-default-read price-table WHITELIST_PRICE_KEY
                { 'price: 0.0 }
                { 'price := price_1 }
                (if (= true usdc) (lago.kwUSDC.transfer account MINT_WALLET (* 1.0 number-to-mint)) (coin.transfer account MINT_WALLET (* number-to-mint price_1)) )
            )
            (with-capability (PRIVATE)
                (map 
                    (mint-miner)
    ...
    (defun mint-miner (account:string)
        ...
        (update mledger (int-to-str 10 (get-count CURRENT_MINER_ID_COUNT))
            { "owner-address" : account
            , "mint-date": (at "block-time" (chain-data)) })
        (increase-count WHITELIST_MINTED_COUNT)
        (increase-count CURRENT_MINER_ID_COUNT)
    )

    (defun admin-mint-miner-multiple (account:string number-to-mint:integer)
