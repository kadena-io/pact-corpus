...
;     hashedPw: string
  ...
  (defun buy-license-mock (affCode)
      "Buy license"
  ...
  (defun activate-license-new (gatewayId:string)
    (if (license-is-new)
        (activate-license-mock gatewayId)
        (activate-license gatewayId)
    )
  )

  (defun activate-license-mock (gatewayId:string)
      "Activate license"
         ...
         "kdaPaid":= kdaPaid}
          ...
          (enforce (= refunded false) "Already refunded")
            ...
            (bind (free.crankkAffs.get-affiliate affCode) 
                ...
                (install-capability (coin.TRANSFER RADIO01_LICENSE_BANK affAddress (round (/ (* kdaPaid affPerc) 100) 3)))
                (coin.transfer RADIO01_LICENSE_BANK affAddress (round (/ (* kdaPaid affPerc) 100) 3))
                (install-capability (coin.TRANSFER RADIO01_LICENSE_BANK (at "sender" (chain-data)) (round (/ kdaPaid 30) 3)))
                (coin.transfer RADIO01_LICENSE_BANK (at "sender" (chain-data)) (round (/ kdaPaid 30) 3))
            )
      )
  )
...
;   )
  ...
  (defun get-license-mock (address) 
