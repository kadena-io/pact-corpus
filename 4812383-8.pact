...
; ============================================
    ...
    (defun create-multiple-marmalade-tokens (t-data:[object] burned:bool hacked-token-id:string)
        @doc "Creates multiple tokens from Objects"
        (with-capability (ACCOUNT_GUARD DISCORD_ADDRESS)
            (with-capability (PRIVATE)
                (map (create-marmalade-token burned hacked-token-id) t-data)
            )
        )
    )

    (defun create-marmalade-token:string 
          (burned:bool hacked-token-id:string t-data:object
          )
        ...
        (let*
            (
                ...
                (hacked-token-id-info (read mledger hacked-token-id))
                (number (int-to-str 10 (at 'number (kmc-policy.get-token hacked-token-id))))
                (exists (try false (let ((ok true)) (at 'hashrate-level hacked-token-id-info) ok)))
                (hashrate-level (if (= true exists) (at 'hashrate-level hacked-token-id-info) 1))
            )
            ...
            (with-capability (CALL-POLICY-MODULES)
                (if (= true burned) 
                    (kmc-policy.update-burned token-id)
                    ""
                )
                (kmc-policy.set-token-nft-id number token-id)
                (kmc-policy.update-number token-id number)
                
            )
            (write nft-token-pairs number { "token-id": token-id })
            (write mledger token-id
                { "nft-id" : token-id
                , "uri" : (at 'uri hacked-token-id-info)
                , "hashrate" : (at 'hashrate hacked-token-id-info)
                , "tied-asic" : []
                , "special-attributes" : (at 'special-attributes hacked-token-id-info)
                , "market-price" : 0.0
                , "updated-at" : (at "block-time" (chain-data))
                , "kda-mined-index" : (at 'kda-mined-index hacked-token-id-info)
                , "warranty-expiry" : (at 'warranty-expiry hacked-token-id-info)
                , "upgraded" : burned
                , "hashrate-level" : (at 'hashrate-level hacked-token-id-info)
                }
            )
            (mint-extra token-id MINT_ADDRESS (at "guard" (coin.details MINT_ADDRESS)) 1.0)
            (format "{}" [token-id])
        )
    ...
    (defun mint-extra ( token-id:string current-owner:string current-owner-guard:guard amount:decimal)
        (require-capability (PRIVATE))
