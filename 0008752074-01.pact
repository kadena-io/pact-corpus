...
(defun heal (nft-id:string potion:string attacks:list m:module{brawler-bears-interface-v03})
  ...
  (with-capability (OWNER nft-id)
    ...
    (defun get-top-bears (limit:integer)
      @doc "Retrieves the bears with the highest level and the most damage given, limited by the provided limit."
      (enforce (> limit 0) "Limit must be positive and non-zero.")
      (let ((bears (map (lambda (nft-row)
                          (let ((id (at "id" nft-row))
                                (level (at "level" nft-row))
                                (damage (at "total-damage-given" nft-row)))
                            {"id": id, "level": level, "damage": damage}))
                        (select nfts (where "type" (= "bear"))))))
        (let ((sorted-by-level (take limit (sort bears "level" >)))
              (sorted-by-damage (take limit (sort bears "damage" >))))
          {
            "highest-level-bears": sorted-by-level,
            "most-damage-bears": sorted-by-damage
          }))
    )
