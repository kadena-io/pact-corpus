(module brawler-bears GOV
    ...
    (defun perform-attack (nft-id:string battle-id:string attack-id:string default:string)
      ...
      (with-capability (OWNER nft-id)
        (let*
          (
            ...
            (default-attack (at "default-attack" nft))
            ; (enforce (!= (at "damage" default-attack) "") "No default attack obj")
            (print default-attack)
            (attack (if (= default "true") default-attack (free.brawler-bears-attacks.get-nft-fields-for-id [] attack-id)))
            (damage (if (= default "true") (at "damage" default-attack) (at "damage" (free.brawler-bears-attacks.get-nft-fields-for-id [] attack-id))))
            (power (if (= default "true") (at "power" default-attack) (at "power" (free.brawler-bears-attacks.get-nft-fields-for-id [] attack-id))))
          )
  ...
  (defun playable-owned-by (owner:string)
      @doc "All IDs owned by someone"
      (select nfts (and?
        (where "owner" (= owner))
        (where "current-battle-id" (= ""))
      ))
  )
