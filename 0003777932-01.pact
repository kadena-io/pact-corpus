(module test3-vesting 'hyper-admin-multi-keyset
  (use hypercent.test3-hype-coin)
  ...
  (defconst HYPE_BANK:string 'hype-bank-test3)
  (defun hype-bank-guard () (keyset-ref-guard "hyper-admin-multi-keyset" ))

  ;WARNING THIS SHOULD BE CALCULATED
  (defconst HYPE_TOKENS 4000000.0)
  (defconst MAX_HYPE_ALLOCATION 240.0)
  (defconst MIN_HYPE_ALLOCATION 0.0001)
  (defconst HYPE_PROJECT_NAME "hype")
  (defconst SWAP_RATE:decimal 0.08)
  ...
  (defun can-invest (account:string)
    (and (test3-community.has-kyc account) (test3-lottery.is-next-in-queue account HYPE_PROJECT_NAME))
  )
  ...
  (defun redeem (account:string)
    ...
    (let* (
      ...
      (install-capability (test3-hype-coin.TRANSFER GENESIS_ACCT account amount_to_redeem))
      (test3-hype-coin.transfer GENESIS_ACCT account amount_to_redeem)
