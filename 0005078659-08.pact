...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (defun TEST_claim-weapon-staking (
    owner:string
    guard:guard)
      ...
      (with-capability (MINT-BULK owner)
        (let*
            (
                ...
                (collection-id-to-reserve-from (TEST-get-and-decrement-random-staking-reserve-collection random))
    ...
    (defun TEST-get-and-decrement-random-staking-reserve-collection:string (rand:integer)
        ...
        (let* 
            (
                ...
                (collection-types-list:list (map (lambda (x) (+ "free.test-weapons-" x)) collection-suffix-strings-list))
                (collection-full-names-list:list (map (lambda (x) (+ x "1" )) collection-types-list))
                
                (reserved-cols-list:list  (map (lambda (x) (TEST_get-reserve-token-amounts-staking-for-col x)) collection-full-names-list))
                ...
                (enforce (> 0 number-of-non-empty-cols) "No NFTs left for staking rewards")
                ;;TESTTODO Test mod logic
    ...
    (defun TEST_get-reserve-token-amounts-staking-for-col (col-id:string )
        (if (= col-id "free.test-weapons-1-2")
                {"staking-reserves": 4, "collection-id" : "battle-heroes-weapons-policy-level-1-col-6"}
                {"staking-reserves": 0, "collection-id" : (+ "battle-heroes-weapons-policy-level-1-col-" col-id)}
        )
    )
  ...
  (defun get-reserved-tokens-col (col-id:string)
    ...
    (defun get-staking-reserve-amt:integer (full-obj)
        (let*
            (
                (staking-reserves:list (at 'staking-reserves full-obj))
            ) 
            staking-reserves
        )
    )
