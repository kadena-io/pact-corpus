(module inu-crew 'inu-boss
  ...
  @doc " Grab yours algo-gen Inu Gangster cause something serious Ahusharmota is about to happen  \
    ...
    (defun inu-club-fight (owner:string kdaAmount:decimal gangsterId:string)
        @doc "Inu Club Fight"
        (enforce (>= kdaAmount 5) "Minumun 5 KDA for play")
        (enforce (<= kdaAmount 60) "Maximum 60 KDA for paly")
        (let ((gangster-owner (is-gangster-owner gangsterId)))
            (enforce (= [{"owner": owner}] gangster-owner) "You are not the owner of this NFT")
        )
        (coin.transfer owner INU_BOSS_WALLET kdaAmount) "Deposit to the Inu Crew vault"
        (with-capability (ACCOUNT_GUARD owner)
            (with-capability (PRIVATE)
                (fight gangsterId)
            )
