...
; ============================================
    ...
    (defun populate-ids-activity (items)
        (map (populate-id-activity) items)
    )

    (defun populate-id-activity (item)
        (update activity-table (at 'sale-count item)
            { "id": (at 'id item) })
    )
            ...
            (with-read marketplace-table nft-id 
                ...
                (with-capability (CALL-POLICY-MODULES)
                    ...
                    (let* 
                        ...
                        (add-to-activity sale-count new-address orig-owner amount policy nft-id)
                    ))
    ...
    (defun add-to-activity (sale-count:integer buyer:string seller:string price:decimal policy:string id:string)
        ...
        (with-read collection-offer-table offer-id
            ...
            (with-capability (CALL-POLICY-MODULES)
                ...
                (let* (
                    ...
                    (add-to-activity sale-count new-address account amount policy nft-id)
                ))
            ))
            (format "Offer accepted successfully. NFT #{} has been transferred to {}, and {} KDA has been sent to {}, minus the 5% marketplace fee"
        ...
        (with-read marketplace-table nft-id
           ...
           (with-default-read mledger nft-id 
            ...
            (let* 
                ...
                (add-to-activity sale-count buyer previous-owner price policy nft-id)
