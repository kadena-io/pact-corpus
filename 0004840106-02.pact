...
; Error from (api.chainweb.com): : Failure: Invalid arguments, received ["(free.kadena-mining-club.PRIVATE)"
; "(native `at`  Index LIST at IDX, or get value with..."] for capability: -> bool -> bool
...
; ============================================
    ...
    (defun get-all-on-sale ()
        ...
        (select mledger ["id", "price"] (where "for-sale" (= true)))
    )
; ============================================
...
; ============================================
    ...
    (defun turn-off-miner-multiple (nft-ids:list)
        (with-capability (PRIVATE)
            (map 
                (turn-off-miner)
                nft-ids
        ...
        (format "Successfully turned off {} miners" {} [(length nft-ids)])
    )
    ...
    (defun turn-on-miner-multiple (nft-ids:list)
        (with-capability (PRIVATE)
            (map 
                (turn-on-miner)
                nft-ids
        ...
        (format "Successfully turned on {} miners" {} [(length nft-ids)])
    )

    (defun turn-on-miner (nft-id:string )
        (require-capability (PRIVATE))
        (with-read mledger nft-id
          { 'for-sale := for-sale }
