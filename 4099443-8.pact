      ...
      (with-capability (END_SALE)
        (with-capability (INTERNAL)
          (with-read collections collection-id {
            ...
            (coin.transfer account creator (floor (* (get-kda-price price) (- 1.0 dev-fee))4))
            (coin.transfer account dev (floor(* (get-kda-price price) dev-fee)4))
            ...
            (update collections collection-id 
              {
                ...
                ,"collection-hash": (hash shifted-token-ids)
...
;; -----------------------------------------------------------------------------
  ...
  (defun get-token-from-index:integer 
    (collection-id:string index:integer)
    (at index (at "sold-tokens" collection-id)))
