(module test-sgk-battle-heros-policy GOVERNANCE
  ...
  ; Constants
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (defconst PUBLIC_MINT_TIME "2023-10-15T16:30:00Z")
    ...
    (with-capability (MINT (at 'id token) account)
      ...
      (let*
          (
           ...
           (update-account-info account account-info hasFreeMintsLeft)
  ...
  (defun update-account-info (account:string account-info:object{account-wl-record-schema} role:string shouldDeductFreeMints:bool)
...
(defun bulk-add-wl-records (wl-list:list)
 ...
 (defun get-price-by-phase:decimal(current-phase:string hasFreeMintsLeft:bool)
    (cond
        ( (and (= FREE_MINT_PHASE current-phase) (> hasFreeMintsLeft 0)) 0.0)
        ( (and (= PUBLIC_PHASE current-phase) (> hasFreeMintsLeft 0)) 0.0)
  ...
  (defun get-current-phase:string()
    (let*
        ...
        (cond
            ...
            ((= true isFreeMintStarted) FREE_MINT_PHASE)
            "Mint is not Live")
    )
  )
