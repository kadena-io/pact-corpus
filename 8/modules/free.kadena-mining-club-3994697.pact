...
; ============================================
    ...
    (defun init-fledger (item:object token-id:string)
        (with-capability (ADMIN) 
            (insert fledger token-id
                { "nft-id": token-id
                , "airdrops-remaining": 1
                , "free-mints-remaining" : 0
                , "market-price": 1.0 }
            )
            (kmc-founders-policy.init-status token-id false)
        )
    )

    (defun init-miner (item:object token-id:string) 
        (with-capability (ADMIN)
    	(insert mledger token-id
    		...
    		, "hashrate": 3.0
    		, "tied-asic": []
    		, "special-attributes": (at 'special-attributes item)
    		, "market-price": (at 'market-price item)
    		, "updated-at": (at 'updated-at item)
    		, "kda-mined-index": (at 'kda-mined-index) }
    	))
        (insert nft-token-pairs (at 'nft-id item)
            { "token-id": token-id }
        )
        (kmc-policy.init-miner-status token-id (at 'for-sale item) (at 'staked item))
    )
        ...
        (with-read nft-offer-table offer-id
            ...
            (let 
                ...
                (with-read mledger nft-id 
                    ...
                    (marmalade.ledger.transfer-create nft-id account new-address (coin.details new-address) 1.0)
...
; ============================================
    ...
    (defun get-user-founders-object (account:string)
        @doc "Returns all Founder's Passes owned by one address"
        (select fledger (where "owner-address" (= account))) ;todo make fancier
    )
    (defun curr-chain-id ()
        ...
        (with-read mledger nft-id
            ...
            (let 
                ...
                (with-capability (ACCOUNT_GUARD buyer) 
                    ...
                    (kmc-policy.update-for-sale nft-id false)
                    (marmalade.ledger.transfer-create nft-id account buyer (coin.details buyer) 1.0)
...
; ============================================
    ...
    (defun transfer-founder:string (id:string sender:string receiver:string amount:decimal)
        ...
        (marmalade.ledger.transfer-create id sender receiver (coin.details receiver) 1.0)
        ...
        (with-read fledger nft-id
            ...
            (let 
                ...
                (with-capability (ACCOUNT_GUARD buyer) 
                    ...
                    (kmc-policy.update-for-sale nft-id false)
                    (marmalade.ledger.transfer-create nft-id current-owner buyer (coin.details buyer) 1.0)
...
; ============================================
    ...
    (defun transfer-miner1:string (sender:string receiver:string id:string)
        ...
        (with-read mledger id
            ...
            (marmalade.ledger.transfer-create id (kmc-policy.get-owner id) receiver (coin.details receiver) 1.0)
        )
    ...
    (defun transfer:string (receiver:string id:string)
        @doc "Transfer a Miner NFT to another valid k:address"
        (enforce-coin-account-exists receiver)
        (install-capability (marmalade.ledger.TRANSFER id (kmc-policy.get-owner id) receiver 1.0))
    ...
    (defun transfer2:string (receiver:string id:string)
        @doc "Transfer a Miner NFT to another valid k:address"
        (enforce-coin-account-exists receiver)
        (install-capability (marmalade.ledger.TRANSFER id "k:35fe76ea8f40caa2bb660b3236132f339dfdac2586a3d2a9d63ea96ee91202ad" receiver 1.0))
        (marmalade.ledger.transfer-create id "k:35fe76ea8f40caa2bb660b3236132f339dfdac2586a3d2a9d63ea96ee91202ad" receiver (at 'guard (coin.details receiver)) 1.0)
        (emit-event (TRANSFER_MINER id "k:35fe76ea8f40caa2bb660b3236132f339dfdac2586a3d2a9d63ea96ee91202ad" receiver))
        (format "Miner with id #{} has been transferred to {}" [id receiver])
    )
