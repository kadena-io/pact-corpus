    ...
    (defpact open-sale (token-id:string timeout:integer)
      (step-with-rollback
        (let* ((spec:object{quote-spec} (read-msg QUOTE-MSG-KEY))
              ...
              (sale-id (pact-id))
              ( token-sale 
        (marmalade.ledger.sale  token-id (kmc-policy.get-owner token-id) 1.0 timeout) ))
         (format "timed escrow: {}, id: {}" [token-sale sale-id]) )
        (let (( token-sale 
        (continue (marmalade.ledger.sale token-id (kmc-policy.get-owner token-id) 1.0 timeout))))
