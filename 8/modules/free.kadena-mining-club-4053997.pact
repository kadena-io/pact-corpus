...
; ============================================
    ...
    (defun list-miner-on-market (nft-id:string owner:string price:decimal for-sale:bool)
     ...
     (defun edit-miner-market-price-marmalade (account:string nft-id:string new-price:decimal)
        ...
        (with-capability (MINER_OWNER account nft-id) 
            (if (get-miner-for-sale-true-false nft-id) (update listings nft-id { "price": new-price } ) (format "NFT is not for sale, please list it on the marketplace before trying to edit the price"))
        )
    )
