(module bh-weapons-upgrade-orchestrator GOVERNANCE
    @doc "Weapons Upgrade smart contract."
    (use marmalade.ledger [ get-policy-info ])
    (use n_7d47538766e6f80008f253dccd30451f4d483c38.battle-heroes-nft-policy [get-nfts-details is-locked])
    ...
    (defcap enforce-upgrade(account:string tk-id-one:string tk-id-two:string)
        (require-capability (PRIVATE))
        (let*
            (
                (token-one (get-nfts-details tk-id-one))
                (token-two (get-nfts-details tk-id-two))
                (stake-status-one (is-locked tk-id-one))
                (stake-status-two (is-locked tk-id-two))
            ...
            (enforce (and (= account (at 'owner token-one)) (= account (at 'owner token-two))) "You must be the owner of the two tokens")
            (enforce (and (= stake-status-one false) (= stake-status-two false)) "Unstake tokens to upgrade.") 
    ...
    (defun upgrade (account:string tk-id-one:string tk-id-two:string)
        ...
        (with-capability (UPGRADE account tk-id-one tk-id-two)
    ...
    (defun get-next-col (token-id:string)
        (let*
            (
                (col-name (at 'name (at 'refName (at 'policy (get-policy-info token-id)))))
