(module tiki-mint GOV
  @doc "The smart contract that is used to mint TIKI nfts in Tiers."
  ...
  (defun mint:bool 
    ...
    (with-capability (MINT)
      (with-read collections collection
        ...
        (enforce 
          (<= (+ current-index amount) total-supply) 
  ...
  (defun reveal-token:string 
    ...
    (with-capability (OPS)
      (bind m-token
        ...
        (update minted-tokens (get-mint-token-id collection token-id)
          { "revealed": true }
        )
