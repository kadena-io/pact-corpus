    ...
    (defun immerse (account:string token:object)
        ...
        (let*
            (
                ...
                (miner-number (at 'number (kmc-policy.get-token miner-id))) ;2992
                (vial-number (at 'number (kmc-vial-policy.get-token vial-id))) ;668
                (upgraded (at 'upgraded (read mledger miner-id))) ;false
                (legendary-vials (kmc-vial-policy.get-all-minted-legendary-vials))  ;4 of them right now
                (legendary-count (get-count LEGENDARIES_MINTED_COUNT)) ;0
                (is-legendary-vial (contains vial-id legendary-vials)) ;false
                (is-legendary-miner (contains miner-id LEGENDARY_MINERS)) ;false 
                (new-miner-id (if (= true is-legendary-vial)
                                (with-capability (PRIVATE)
                                    (increase-count LEGENDARIES_MINTED_COUNT)               ;20001
                                    (kmc-policy.get-token-nft-id2 (int-to-str 10 (+ 20001 legendary-count)))
                                ) ;false 
                                (kmc-policy.get-token-nft-id2 (int-to-str 10 (+ 10000 miner-number))) ) 
                )
                ...
                (new-uri (at 'assetUrl (at 'datum (at 0 (at 'data (at 'manifest (at 'token (marmalade.ledger.get-policy-info miner-to-upgrade))))))))
            )
...
;Failure: Type error: expected string, found integer
                    ; (update nft-token-pairs (int-to-str 10 miner-number) { "token-id": new-miner-id })
                ...
                "") ;do not burn gen 1 legendary, do not mint new NFT
            ...
            ;     , "market-price" : (at 'market-price old-miner-info)
            ;     , "updated-at" : (at "block-time" (chain-data))
            ;     , "kda-mined-index" : (at 'kda-mined-index old-miner-info)
            ;     , "warranty-expiry" : (at 'warranty-expiry old-miner-info)
            ;     , "upgraded" : true
            ...
            , "new-token-number" : (format "{}" [miner-number])
            , "new-hashrate": (format "{}" [new-hashrate] ) }
        )
    )


    (defun immerse-multiple2 (account:string tokens:list) ; list of objects have format { "vial-id" : "t:xxxxxx" , miner-id "t:xxxxxx" }
        @doc "Allows a user to burn multiple gen 1 miners and a vial to create a gen 2 miner"
        (map (immerse2 account) tokens)
    )

    (defun immerse2 (account:string token:object)
        @doc "Immerses a single NFT. Usually not called directly"
        (let*
            (
                (vial-id (at 'vial-id token))
                (miner-id (at 'miner-id token))
                (miner-number (int-to-str 10 (at 'number (kmc-policy.get-token miner-id)))) ;2992
                (vial-number (at 'number (kmc-vial-policy.get-token vial-id))) ;668
                (upgraded (at 'upgraded (read mledger miner-id))) ;false
                (legendary-vials (kmc-vial-policy.get-all-minted-legendary-vials))  ;4 of them right now
                (legendary-count (get-count LEGENDARIES_MINTED_COUNT)) ;0
            ...
            (if (= is-legendary-miner false)
                (with-capability (PRIVATE)
                    ...
                    (install-capability (marmalade.ledger.MINT new-miner-id account 1.0))
                    (marmalade.ledger.mint new-miner-id account (coin-account-guard account) 1.0)
                    (update nft-token-pairs miner-number { "token-id": new-miner-id })
                    (insert preserved-info new-miner-id
                        { "token-id": new-miner-id
                        , "old-token-id" : miner-id
                        , "uri" : (at 'uri old-miner-info) }
    ...
    (defun immerse-multiple3 (account:string tokens:list) ; list of objects have format { "vial-id" : "t:xxxxxx" , miner-id "t:xxxxxx" }
        @doc "Allows a user to burn multiple gen 1 miners and a vial to create a gen 2 miner"
        (map (immerse account) tokens)
    )

    (defun immerse3 (account:string token:object)
        @doc "Immerses a single NFT. Usually not called directly"
        (let*
            (
                (vial-id (at 'vial-id token))
                        ...
                        (
                            (info (bind (marmalade.ledger.get-policy-info nft-id)
                                { 'policy := policy:module{free.kmc-token-policy-v13}}
                                    {"emitted" : (policy::emit-buy nft-id new-address orig-owner amount (format "{}" [policy]))})
                                    )
