...
; ============================================
    ...
    (defun set-warranty-multiple (begin:integer end:integer)
        (with-capability (ADMIN)
        (with-capability (PRIVATE)
            (map (set-warranty) (enumerate begin end))
        ))
    )
    
    (defun set-warranty (id:integer)
        (require-capability (PRIVATE))
        (let*
            (
                (token-id (at 'token-id (get-token-id (int-to-str 10 id))))
                (num-days (if (and (> id 1500) (< id 3001)) 1461 1096))
            )
            (update mledger token-id
                { "warranty-expiry": (add-time (time "2023-03-14T23:59:59Z") (days num-days))}
            )
        )
    )
