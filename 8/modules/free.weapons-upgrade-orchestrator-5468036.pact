(module weapons-upgrade-orchestrator GOVERNANCE
    ...
    (defconst LEVEL_TWO_COUNT "level-2")
    (defconst LEVEL_THREE_COUNT "level-3")
    (defconst LEVEL_FOUR_COUNT "level-4")
    ...
    (defun initialize ()
        ...
        (with-capability (GOVERNANCE)
            ...
            (insert counts-table LEVEL_TWO_COUNT { "count": 0 })
            (insert counts-table LEVEL_THREE_COUNT { "count": 0 })
            (insert counts-table LEVEL_FOUR_COUNT { "count": 0 })
        )
    )
    ...
    (defun upgrade (account:string tk-id-one:string tk-id-two:string)
        ...
        (with-capability (PRIVATE)
            ...
            (let*
                ...
                (install-capability (marmalade.ledger.MINT next-mint-token account 1.0))
    ...
    (defun get-next-mint-token (token-id:string)
        (let*
            (
                ...
                (next-mint-id (+ 1 (get-count (concat ["level-" next-id]))))
                (next-mint-token (concat [(int-to-str 10 next-col-id) ":" (int-to-str 10 next-mint-id)]))
            )   
            
            {"next-mint-token":next-mint-token,"next-col-id":next-col-id, "id":id, "col-name":col-name, "next-id":next-id, "next-col-id":next-col-id, "next-mint-id":next-mint-id}
        )
    )
