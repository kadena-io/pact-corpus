...
; ============================================
    ...
    (defun populate-upgrade (token-ids:list)
        (with-capability (PRIVATE)
        (with-capability (DISCORD DISCORD_ADDRESS)
        (with-capability (CALL-POLICY-MODULES)
            (map (populate-new-upgrade) token-ids)
        )))
    )
    
    (defun populate-new-upgrade (token-id:string)
        (require-capability (PRIVATE))

        (update mledger token-id
            { "warranty-expiry" : (time "2026-03-14T12:00:00Z") }
        )
    )
