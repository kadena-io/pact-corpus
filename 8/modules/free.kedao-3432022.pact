(module kedao GOVERNANCE
  ...
  (defun filter-uid:[object] (rows:[object])
    (let*
      (
        ...
        (filter-row (lambda (obj)
          (and
            (>= (+ prefix_len 5) (length (at 'key obj)))
            (contains prefix (take prefix_len (at 'key obj)))
          )
        ))
      )
  ...
  (defun get-all-users:[object] ()
    (let
      ...
      (filter-uid (fold-db users (constantly true) (consumer)))
    )
  )
  ...
  (defun get-users-with-tg-uids:[object] (tg_uids:[string])
    ...
    (let*
      (
        (filter-tg (lambda (key obj) (contains (at 'telegram (at 'social_ids obj)) tg_uids)))
        (consumer (lambda (key obj) (+ {"key": key} obj)))
      )
      (filter-uid (fold-db users (filter-tg) (consumer)))
    )
  )
  (defun get-user-with-tg-uid:object (telegram_user_id:string)
    ...
    (let*
      (
        (matched (get-users-with-tg-uids [telegram_user_id]))
      )
  ...
  (defun get-dates-in-week:[string] (begin_date:string end_date:string)
    @doc "Get date list from begin_date to end_date, cnt = days "
    (enforce
      ( =
    ...
    (let
      ((add (
        lambda 
        (day_cnt) 
        (take 10 (add-time (time (format "{}T00:00:00Z" [begin_date])) (days day_cnt)))
      )))
      (map (add) (enumerate 0 6))
    )
  )
  ...
  (defun get-score-records:[object] (account:string)
    ...
    (select score-records (where 'uid (= (at 'uid (read users account)))))
  )
  (defun get-score-records-by-date:[object] (date:string)
    @doc "Query score records by certain date"
    (let*
      (
        (filter-date (lambda (key obj) (= date (at 'date obj)) ) )
        (consumer (lambda (key obj) obj))
      )
      (fold-db score-records (filter-date) (consumer))
    )
  )
  ;(defun get-score-records-by-date-range:[object] (uids:[string] begin_date:string end_date:string)
  (defun get-score-records-by-date-range:[object] (begin_date:string end_date:string)
    ...
    (let*
      (
        (dates (get-dates-in-week begin_date end_date))
        ;(dates_and_types (map (lambda (x) ) dates))
        ;(dates_and_type (zip (lambda (x y) { "source": x, "date": y }) SCORE_SOURCE_TYPES dates))
      )
      ;(fold-db score-records (filter-date) (consumer))
      [{}]
    )
  ...
  (defun migrate-score-records:[string] (all-users:[object] records:[object])
    ; modify score-records
    (with-capability (IS_KEDAO_BOT)
      (let*
        (
          (matched-uid (lambda (record) 
            (at 'key (at 0 (filter (lambda (user) (= (at 'main (at 'accounts user)) (at 'account record))) all-users)))
          ))
          (create-new (
            lambda
            (old-record)
