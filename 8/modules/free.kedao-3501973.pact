(module kedao GOVERNANCE
  ...
  (defun migrate-droplets:[string] (begin_date:string end_date:string)
    (let
      ((records (filter (lambda (obj) (and (<= begin_date (at 'date obj)) (>= end_date (at 'date obj)))) (filter (compose (at 'score_inc ) (< 0.0)) (get-all-score-records)))))
    ...
    (let
      ...
      (map (lambda (record) 
          (let*
            ...
            (with-capability (UPDATE_POINTS uid date "goldstone" 0.0 comments)
              (write point-records record_key {
                "records": {
