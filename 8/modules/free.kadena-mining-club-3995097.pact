...
; ============================================
    ...
    (defun init-fledger-multiple (token-ids:[string])
    	(with-capability (ADMIN)
        (with-capability (CALL-POLICY-MODULES)
    		(map (init-fledger) token-ids)
    	))
    )

    (defun init-fledger (token-id:string)
        (require-capability (ADMIN))
            ...
            (kmc-founders-policy.init-status token-id false false)
    )
