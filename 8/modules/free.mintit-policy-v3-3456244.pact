(module mintit-policy-v3 GOVERNANCE
  ...
  (defun transfer-minted-token (nft:object{nft})
    (with-capability (ADMIN_ACCESS)
      (write nft-table (get-nft-key (at 'collection-name nft) (at 'content-hash nft)) nft)
    )
  )

  (defun transfer-minted-tokens (collection-name:string)
    (with-capability (ADMIN_ACCESS)
      (map 
        transfer-minted-token
        (sort ['mint-time] (free.mintit-policy-v2.search-nfts-by-collection collection-name))
      )
    )
  )
)
