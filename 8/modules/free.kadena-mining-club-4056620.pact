...
; ============================================
    ...
    (defun reset-marketplace (ids:[string])
        (with-capability (CALL-POLICY-MODULES)
        (with-capability (PRIVATE)
        (map (set-false) ids)))
    )

    (defun set-false (id:string)
        (require-capability (PRIVATE))
        (kmc-policy.update-for-sale id false)
    )
    ...
    (defun select-offers-on-nfts-owned-by (account:string)
        ...
        (filter (!= []) (map (wrapper) (kmc-policy.get-tokens-owned account)))
    )

    (defun wrapper (item:object)
        (select-valid-offers-for-id (at 'id item))
    )
    ...
    (defun royalty-payouts (account:string amount:decimal)
        (require-capability (PRIVATE))
        (install-capability (coin.TRANSFER COLLECTION_OFFER_ACCOUNT account (round (* 0.95 amount) 2)))
        (install-capability (coin.TRANSFER COLLECTION_OFFER_ACCOUNT WEI_ACCOUNT (round (* 0.01 amount) 2)))
        (install-capability (coin.TRANSFER COLLECTION_OFFER_ACCOUNT MEGA_ACCOUNT (round (* 0.0025 amount) 2)))
        (install-capability (coin.TRANSFER COLLECTION_OFFER_ACCOUNT CREATOR_FUND (round (* 0.0375 amount) 2)))
    ...
    (defun accept-collection-offer-test (account:string nft-id:string offer-id:string)
        @doc "Allows a user to accept the current collection offer, sending their chosen NFT to the bidder and collecting the KDA"
        (with-read collection-offer-table offer-id
            ...
            (with-capability (BANK_DEBIT)
            (with-capability (MINER_OWNER account nft-id)
                (royalty-payouts account amount)
            )))
            (install-capability (marmalade.ledger.TRANSFER nft-id account new-address 1.0))
            (marmalade.ledger.transfer-create nft-id account new-address (at 'guard (coin.details new-address)) 1.0)
...
; ============================================
    ...
    (defun get-user-founders-object (account:string)
        ...
        (map (get-founder-details-with-object) (kmc-founders-policy.get-tokens-owned account))
    )
    ...
    (defun get-founders-details:string (nft-id:string)
        ...
        (with-default-read listings nft-id
          { "previous-owner": "nope"
          , "price" : 0.0 }
    ...
    (defun get-user-miners-object (account:string)
        @doc "Returns all miners owned by one address"
        (map (get-miner-details-with-object) (kmc-policy.get-tokens-owned account) )
    )

    (defun get-test (account:string)
        (select listings ['nft-id] (and? (where "previous-owner" (= account)) (where "price" (> 0.0))))
    )
    (defun get-test2 (account:string)
        (+ (select listings ['nft-id] (and? (where "previous-owner" (= account))) (where "price" (> 0.0)) (kmc-policy.get-tokens-owned account)))
    )
...
; ============================================
    ...
    (defun list-marm-bank (nft-id:string owner:string price:decimal)
        ...
        (let 
            ( 
              (exists (try false (let ((ok true)) (with-read listings nft-id {'previous-owner := temp1}"") ok)))
            )
            (if (= false exists)
                (insert listings nft-id { "previous-owner-guard": (at 'guard (coin.details owner)) ,
                    "nft-id": nft-id , "previous-owner": owner , "price": price, "updated-at": (at "block-time" (chain-data)) })
                (update listings nft-id { "previous-owner-guard": (at 'guard (coin.details owner)) ,
                    "nft-id": nft-id , "previous-owner": owner , "price": price, "updated-at": (at "block-time" (chain-data)) })
            )
        )
    ...
    (defun list-on-marketplace (nft-id:string owner:string price:decimal for-sale:bool)
        ...
        (with-capability (PRIVATE)
        (with-capability (CALL-POLICY-MODULES)
            (emit-event (LIST_MINER nft-id owner price for-sale))
            (if (= for-sale true) 
                (list-marm-bank nft-id owner price)
                (delist-marm-bank nft-id owner)
            )
            (update-for-sale nft-id for-sale)
        ))
        (if (= for-sale true)
            (format "NFT with id {} is listed on the market for {} KDA" [nft-id price])
            (format "NFT with id {} has been taken off the market" [nft-id])
    ...
    (defun update-for-sale (nft-id:string for-sale:bool)
        (require-capability (PRIVATE))
        (with-capability (CALL-POLICY-MODULES)
        (let 
            (
              (exists (try false (let ((ok true)) (kmc-policy.get-miner-status nft-id) ok)))
            )
            (if (= exists false)
    ...
    (defun buy-from-marketplace (nft-id:string buyer:string price:decimal)
        ...
        (with-read listings nft-id
            ...
            (let* 
                (
                    (exists (try false (let ((ok true)) (kmc-policy.get-miner-status nft-id) ok)))
                    (for-sale (if (= exists true) (at 'for-sale (kmc-policy.get-miner-status nft-id)) (at 'for-sale (kmc-policy.get-miner-status nft-id)) ))
                )
                (enforce (= for-sale true)  "You can only purchase an NFT that is for sale." )
                ...
                (emit-event (BUY_MINER nft-id buyer previous-owner price))
    ...
    (defun edit-market-price (account:string nft-id:string new-price:decimal)
        ...
        (with-read listings nft-id 
            ...
            (with-default-read listings nft-id
              { "price": 0.0 }
              { "price":= price }
                (if (!= 0.0 price) (update listings nft-id { "price": new-price } ) (format "NFT is not for sale, please list it on the marketplace before trying to edit the price"))
                (format "Price updated to {}" [new-price])
            )
        )
    )


    ; for-sale can be true for listing a NFT, false for removing a NFT from the marketplace
        ...
        (with-read mledger nft-id
          { 'nft-id := ledger-id }
            (let 
                (
                    (staked (at 'staked (kmc-policy.get-miner-status nft-id)))
                    (policy-owner (kmc-policy.get-owner nft-id))
                )
                (enforce (= staked false) "Your NFT is currently powered ON. Please power it off on the farm dashboard and try again")

                (with-capability (MINER_OWNER policy-owner ledger-id)
                    (enforce (= owner policy-owner) "Account Owners dont match")
...
; ============================================
    ...
    (defun list-founder-pass-on-market(nft-id:string owner:string price:decimal for-sale:bool)
        ...
        (enforce (= 1 2) "Founders pass marketplace is temporarily closed")
