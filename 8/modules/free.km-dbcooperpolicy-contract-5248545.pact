(module km-dbcooperpolicy-contract COOPER

  (defcap COOPER ()
    (enforce-guard (keyset-ref-guard "free.km-dbcpolicy-ks")))
    ...
    (defconst ADMIN:string "k:6d3408f25312d5342ea5513707cc18dfd7b2a2c30decd0250d6f6491bbbfcf32")
    ...
    (defun enforce-mint:bool
      ( token:object{token-info} account:string guard:guard amount:decimal )
       ...
       (with-read mint-status MINT_STATUS { 'status:= status }
    ...
    (defun mint-for-price
      ( guard:guard token-id:string account:string available-tokens:[integer] current-length:integer minted:integer sha256:string )
        (let* (
      ...
      ( guard:guard token-id:string account:string available-tokens:[integer] current-length:integer minted:integer sha256:string )
        ...
        (if (= account ADMIN)
          ...
          [ (update priority account{"claimed": true})
           (free.km-pass-contract.burn account)
          ]
        )
    )
    ...
    (defun enforce-transfer:bool
      ...
      (enforce-ledger)
      (with-read policies (at 'id token) { "owner" := owner }
    ...
    (defun update-mint-time
      ( public-mint-time:time whitelist-mint-time:time mint-end-time:time )
      (with-capability (IS_ADMIN)
      (update collection-info COLLECTION_INFO
        {
          "public-mint-time": public-mint-time,
