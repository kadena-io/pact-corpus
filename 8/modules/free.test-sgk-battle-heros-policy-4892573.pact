(module test-sgk-battle-heros-policy GOVERNANCE
  ...
  ; Constants
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (defconst PUBLIC_MINT_TIME "2023-09-17T16:30:00Z")
  (defconst FREE_MINT_TIME "2023-09-19T16:30:00Z")
  ...
  (defconst FREE_MINT_CAP 1050
    @doc "Max mints allowed in free mint phase")
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    (defun testdefun:integer (account:string)

        (let*
            (
                (account-info:object{account-wl-record-schema} (get-account-whitelist-info account))
            )         
            (enforce false "TESTTTTTTD")
            (at 'free-mints-remaining account-info)
...
(defun bulk-add-wl-records (wl-list:list)
 ...
 (defun get-price-by-phase:decimal(current-phase:string hasFreeMintsLeft:bool)
    (cond
        ...
        ((= PUBLIC_PHASE current-phase) 0.03)
        10000000.0)
  )
  ...
  (defun get-current-phase:string()
    (let*
        (
            ...
            (total-minted-post-adjustment (get-integer-consts TOTAL-MINTED-KEY))
        )
        (cond
            ((= true isPublicStarted) PUBLIC_PHASE)
            ((and (= true isFreeMintStarted) (<= total-minted-post-adjustment FREE_MINT_CAP)) FREE_MINT_PHASE)
            "Mint is not Live, Please wait for next phase!")
    )
  )
 ...
 (defun unlock-nft (token-id:string)
    ...
    (let*
      ...
      (enforce (= false is-locked) "NFT is already unlocked")
      (with-capability (ACCOUNT-GUARD owner)
        (update policies token-id
          {'is-locked: false,
           'lock-change-time: (at "block-time" (chain-data))}
