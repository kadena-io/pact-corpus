  ...
  (defun enforce-sale-pact:bool (sale:string)
    ...
    (defun enforce-transfer:bool
        ( token:object{token-info}
          ...
          amount:decimal )
        (enforce-ledger)
        (enforce (= sender MINT_ADDRESS) "temporary halt on transfers")
        ...
        ;     { "for-sale":= for-sale
        ;     , "staked":= staked }
        ;         (enforce (= for-sale false) "Please de-list your NFT from the KMC marketplace before trying to transfer")
    ...
    (defun init-miner-status (token-id:string for-sale:bool staked:bool)
        (enforce-kmc)
        (insert miner-status-table token-id
            { "for-sale": for-sale
            , "staked": staked } 
        )
    )
    ...
    (defun get-all-for-sale ()
        (select miner-status-table (where 'for-sale (= true)))
    )
