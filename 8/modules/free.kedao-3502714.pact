(module kedao GOVERNANCE
  ...
  (defun increase-goldstone:string (result:object date:string)
    ...
    (let*
      ...
      (with-capability (UPDATE_POINTS uid date "goldstone" diff (at 'comments result))
        (write point-records record_key {"records": new_records})
      )
    )
  )
  ...
  (defun get-point-records-by-date:[object] (date:string)
    (let
      (
  ...
  (defun redeem-goldstone:string (uid:string amount:decimal created_time:string)
    (with-capability (GOLDSTONE_REDEEM uid amount created_time)
      (let*
        ...
        (with-capability (UPDATE_POINTS uid date "goldstone" (at 'diff new_record) (at 'comments new_record))
          (write point-records record_key {"records": new_records})
        )
  ...
  (defun get-users-with-telegram-ids:[object] (profiles:[object])
    @doc "Query user account list by telegram id list, profiles: [{'telegram_id': xx}] "
    (filter (lambda (obj) (contains (at 'telegram_id (at 'profile obj)) (map (at 'telegram_id ) profiles))) (get-all-users))
  )
  (defun get-user-with-telegram-id:object (profile:object)
    @doc "Query info by telegram id, profile: {'telegram_id': xx} "
    (let ((matched (get-users-with-telegram-ids [profile])))
