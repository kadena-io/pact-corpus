...
; ============================================
    ...
    ; [{"token-id": "t:-2uhIhGhDQJf_ckJ-o5LK9U_haZaaHzQAM_lU7gJk6o"}]
...
; ============================================
    ...
    (defun list-marm-miner-on-market (nft-id:string owner:string rotate:bool)
        @doc "Make a new listing or update a listing for a Miner on the marketplace"
        ; (with-capability (ADMIN)
            (if (= true rotate)
                (marmalade.ledger.rotate nft-id owner (marm-bank-guard))
    ...
    (defun remove-marm-miner-from-market (nft-id:string orig-owner:string rotate:bool)
        ; (with-capability (ADMIN)
            (if (= true rotate)
                (remove-listing-rotate nft-id orig-owner)
                (remove-listing-transfer nft-id orig-owner)
            )
        ; )
        "de-listed"
    )

    (defun remove-listing-rotate (id:string orig-owner:string)
        ; (with-capability (ADMIN)
            (install-capability (marmalade.ledger.ROTATE id orig-owner))
            (marmalade.ledger.rotate id orig-owner (at "guard" (coin.details orig-owner)))
        ; )
    )

    (defun remove-listing-transfer (id:string orig-owner:string)
        ; (with-capability (ADMIN)
            (install-capability (marmalade.ledger.TRANSFER id MARMALADE_BANK orig-owner 1.0))
            (marmalade.ledger.transfer-create id MARMALADE_BANK orig-owner (at 'guard (coin.details orig-owner)) 1.0)
        ; )
    )
    (defun list-marm-real (nft-id:string owner:string price:decimal for-sale:bool rotate:bool)
        ...
        (if (= for-sale true) 
            (list-marm-miner-on-market nft-id owner rotate)
            (remove-marm-miner-from-market nft-id owner rotate)
        )
;TODO add some extra code for removing from the marketplace?
