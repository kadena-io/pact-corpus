(module sgk-rewards GOVERNANCE
        ...
        (with-capability (OWNER id)
            (let*
                ...
                (if (and (= (claim-status false)) (= mint-status true))
                    [
                        ;ToDo: Implement minting on the weapon/gear contract
    ...
    (defun get-owner-rewards (account:string)
        ...
        (select reward-table ["token","reward","claim-status"] (where 'account (= account)))
    )
    ...
    (defun distribute-rewards-gen-zero (token:object)
        ...
        (let*
            ...
            (if (and (= stake-status true) (= rewardable true))
                [
                    (insert reward-table (at 'token-id token) ;This will be changed to the ID of the reward token
                        {
                            "account": (at 'owner token)
                            ,"claim-status": false
                            ,"token": (at 'token-id token)
                            ,"reward": (at 'token-id token);This will be changed to the ID of the reward token
                            ,"collection": GEN_ZERO
                        }
                    )
                    ;(mint account guard token-id)
