(module kedao GOVERNANCE
  ...
  (defun filter-uid:[object] (_users:[object])
    (filter (lambda (obj) (contains UID_PREFIX (at 'key obj))) _users)
  )
  ...
  (defun migrate-score-records:[string] ()
    ...
    (with-capability (IS_KEDAO_BOT)
      (let*
        ...
        (map (create-new) (get-all-score-records))
      )
    )
  )
