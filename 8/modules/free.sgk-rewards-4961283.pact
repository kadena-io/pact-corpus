(module sgk-rewards GOVERNANCE
    ...
    ; Constants
    ; --------------------------------------------------------------------------
    (defconst LAMBDA_ADDRESS "k:3a665827b7414a6bef01b434bf27b2911266569e9d0bd2b2420ee28df8a5a9c0")
    ...
    (defun distribute-rewards-battle-heroes (token-id:string)
        ...
        (let*
            (
                ...
                (check-count (try false (with-read rewards-table token-id {'rewards := rewards}rewards)))
            )    
            (if (and (= stake-status true) (= rewardable true))
                [
                    (if (> check-count 0)
                ...
                (format "Token #{} is not staked" [token-id])
            )
        )
    ...
    (defun testing (token-id:string)
        (try false (with-read rewards-table token-id {'rewards := rewards}rewards))
    )
