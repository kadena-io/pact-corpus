(module mintit-policy-staging-v10 GOVERNANCE
  ...
  (defcap GOVERNANCE 
    () 
    (enforce-guard (read-keyset "mintit-admin-abd"))
  )
...
(defun count-nfts-by-owner-in-collection:decimal
    (collection-name:string creator:string)
    (length 
        (select nft-table 
            (and? 
  ...
  (defun sync-with-marmalade:bool
    ...
    (let*
      (
        ...
        (datum-uri (kip.token-manifest.uri "pact:schema" "free.mintit-policy-staging-v10.nft-manifest-datum"))
    ...
    (marmalade.ledger.create-token token-id token-precision nft-manifest mintit-policy-staging-v10)
...
;     (with-capability (ADMIN_ACCESS)
;       (bind (free.mintit-policy-staging-v5.get-nft-collection collection-name) 
;           { 'creator := creator
;           , 'description := description
;           , 'name := name
;           , 'type := type
...
;         (let (
;           (starting-index (at 'mint-index (at 0 (sort ['mint-time] (free.mintit-policy-staging-v5.search-nfts-by-collection collection-name)))))
;         )
...
;     (with-capability (ADMIN_ACCESS)
;       (let* (
;           (content-hash (free.mintit-policy-staging-v5.get-nft-collection-token-hash collection-name index))
;           (nft (free.mintit-policy-staging-v5.read-nft-table collection-name content-hash))
;         )
;         (write nft-table (get-nft-key collection-name content-hash) nft)
;       )
;     )
