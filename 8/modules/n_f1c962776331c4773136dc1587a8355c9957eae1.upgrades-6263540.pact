    ...
    (defun write-blueprint:bool (blueprint-hash:string owner:string)
        (with-capability (LOCK owner blueprint-hash) 
            ...
            (let* (
                ...
                (enforce (= target-token-balance 1.0) "Target token does not belong to sender")
                (enforce (= attachment-token-balance 1.0) "Attachmnet token does not belong to sender")

                (insert nfts (read-msg "target-token-id") target)
                (insert nfts (read-msg "attachment-token-id") attachment)
