            ...
            (let* 
                (
                    (info (bind (marmalade.ledger.get-policy-info nft-id)
                        { 'policy := policy:module{free.kmc-token-policy-v13}}
                        { "emitted" : (policy::emit-buy nft-id buyer previous-owner price (drop 5 (format "{}" [policy])))
                        , "for-sale": (at 'for-sale (policy::get-status nft-id))
                        , "kmc-emit": (emit-event (BUY_KMC2 nft-id buyer previous-owner price (drop 5 (format "{}" [policy]))))} ))
                )
...
; ============================================
    ...
    (defcap BUY_KMC2 (nft-id:string buyer:string seller:string price:decimal policy:string)
        @doc "emitted when any KMC token is purchased from the marketplace"
        @event true
    )
