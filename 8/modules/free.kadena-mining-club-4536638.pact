...
; ============================================
    ...
    (defun airdrop-founder-multiple () 
        (with-capability (ADMIN)
            (map (airdrop-founder) (kmc-founders-policy.get-all-tokens))
        )
    )

    (defun airdrop-founder (token:string)
        (require-capability (ADMIN))
        (update fledger token
            { "airdrops-remaining": 0 } 
        )
        ; get owner, account for marketplace stuff
        (kmc-immersion-mint.mint (kmc-founders-policy.get-owner token) 1)
    )
...
; ============================================
    ...
    (defun select-all-marketplace-listings ()
        (select marketplace-table ['id] (where "previous-owner" (!= "nope")))
    )
    ...
    (defun populate-new-listing (token:object)
        ...
        (let* 
            (
                (item (read marketplace-table (at 'id token)))
                (policy (if (= "free.kmc-policy" (at 'policy item)) "kmc-policy" "kmc-founders-policy"))
            )
            (update marketplace-table (at 'id item)
                { "policy" : policy }
            )
        )
    )
...
; ============================================
    ...
    (defun get-all-vials-for-sale ()
        (map (get-vial-details-with-object) (kmc-vial-policy.get-all-for-sale))
    )
    ...
    (defun get-user-vials-object (account:string)
        ...
        (map (get-vial-details-with-object) 
            (+ (select marketplace-table ['id] (and? (and? (where "price" (!= 0.0))
                             (where "previous-owner" (= account)) )
                       (where "policy" (= "kmc-vial-policy"))))
            (kmc-vial-policy.get-tokens-owned account)) )
    )

    (defun select-test2 (account:string)
            (+ (select marketplace-table ['id] (and? (and? (where "price" (!= 0.0))
                             (where "previous-owner" (= account)) )
                       (where "policy" (= "kmc-vial-policy"))))
            (kmc-vial-policy.get-tokens-owned account)) 
...
;;;test
    ...
    (defun get-all-vials-for-sale2 ()
        (map (get-vial-details-with-object2) (kmc-test3-policy.get-all-for-sale))
    )

    (defun get-vial-details-with-object2 (item:object)
        (get-vial-details2 (at 'token-id item))
    )

    (defun get-vial-details2 (nft-id:string)
        @doc "Returns the details of an Immersion Vial"
        (with-default-read marketplace-table nft-id
          { "previous-owner": "nope"
          , "price" : 0.0 }
          ...
          , "price" := price} 
        {
          "old-nft-id" : (int-to-str 10 (at 'number (kmc-test3-policy.get-token nft-id)))
        , "nft-id" : nft-id
        , "owner-address" : (kmc-test3-policy.get-owner nft-id)
        , "uri" : (at 'assetUri (at 'datum (at 0 (at 'data (at 'manifest (at 'token (marmalade.ledger.get-policy-info nft-id)))))))
        , "market-price" : price
        , "price" : price
        , "for-sale" : (at 'for-sale (kmc-test3-policy.get-status nft-id))
        , "original-owner": original-owner }
        )
    )

    (defun get-user-vials-object2 (account:string)
        @doc "Returns all vials owned by one address"
        (map (get-vial-details-with-object2) 
            (+ (select marketplace-table ['id] (and? (and? (where "price" (!= 0.0))
                             (where "previous-owner" (= account)) )
                       (where "policy" (= "kmc-test3-policy"))))
            (kmc-test3-policy.get-tokens-owned account)) )
    )
