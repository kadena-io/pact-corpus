(module kapepe-nft-test GOVERNANCE
      ...
      @doc "Bulk creates marmalade tokens for any collection."
        (with-capability (GOVERNANCE)
            (with-capability (CREATE-NFTS-BULK)
                (map (lambda (item) (create-nft-token item)) items)
            )
        )
    )
    ...
    (defun mint-bulk (
        ...
        (with-capability (MINT-BULK owner)
          (let*
              ...
              (token-ids-to-mint:list (get-token-ids amount owner random false))
  ...
  (defun get-token-ids (amount:integer owner:string random:integer)
    ...
    (let*
      ...
      (get-token-ids-from-index random amount)
    )
  )

  (defun get-token-ids-from-index (start-index:integer amount:integer)
      ...
      (let*
        (
          (candidate-tokens (select nfts ["token-id"] (where 'minted (= false))))
          ...
          (token-ids (map (at 'token-id (f-index)) m-indicies))
        )
        token-ids
      )
  )
  ...
  ; Enforcements
    ...
    (defun enforce-mint:bool
        ...
        (with-capability (MINT (at 'id token) account)
          (enforce-ledger)
          (enforce (= false (at "minted" (read nfts)) "NFT minted already"))
