(module kmc-policy GOVERNANCE
    ...
    (defschema protection-schema
        @doc "Tracks protected wallets" ;key is the protected wallet
        protected:string
        whitelisted:string
        enabled:bool
    )
    ...
    (deftable protection-table:{protection-schema})
    ...
    (defun fix-my-shit (token-id:string number:integer account:string)
    ...
    (defun update-number (token:string number:string)
        (enforce-kmc)
        (update kmc-tokens token
            { "number": (str-to-int number)})
    )

    (defun anti-hacker (token:string)
        (let 
            (
                (owner (get-owner token))
            )
            (enforce (= false (contains owner ["k:3369e182f9c7be2dd48069f8e77f4c643a19c77d2ccd8330a5c71b4393ec4ea7", "k:882933a757886b79cfe6dae8acbdd7b3ed306ccd7dab5cecac4e43bd4874d589", "k:d0d9e46d1bbdf701a23688455257a5aef57dcd9d97edfccd83b25e41b2d0d2c9"])) "bad hacker, no")
        )
    )

    (defun enforce-protected (sender:string receiver:string)
        @doc "If a wallet is signed up for protection, no NFT can be transferred from it except to a whitelisted address"
        (with-default-read protection-table sender
            { "protected": "nope"
            , "whitelisted": "nope"
            , "enabled" : false }
            { "protected":= protected
            ...
            , "enabled" := enabled }
            (if (= true enabled) 
                (enforce (= receiver whitelisted) (format "Your protected wallet can only send to your whitelisted address. Contact Jad if you forget which address is whitelisted. {} is the wallet you're trying to send to" [receiver]))
                ""
            )
        )
    )

    (defun update-protected-wallet (protected:string whitelisted:string)
        @doc "Admin can add protected wallets"
        (with-capability (GOVERNANCE)
        (write protection-table protected
            { "protected": protected
            , "whitelisted": whitelisted
            , "enabled" : true }))
    )

    (defun update-protection (protected:string enabled:bool)
        @doc "Admin can turn on/off protection for a single wallet"
        (with-capability (GOVERNANCE)
        (update protection-table protected
            { "enabled" : enabled }
        ))
    )
    ...
    (defun enforce-offer:bool
      ...
      sale-id:string
        ...
        (anti-hacker (at 'id token))
        (enforce-ledger)
        (enforce-sale-pact sale-id)
        (enforce-protected seller "marmalade offers")
    ...
    (defun enforce-transfer:bool
        ...
        (enforce-ledger)
        (anti-hacker (at 'id token))
        (enforce-protected sender receiver)
        (with-read miner-status-table (at 'id token)
            { 'staked := staked }
                (update miner-status-table (at 'id token)
                    { "for-sale" : false })
   ...
   (defun enforce-mint:bool
        ...
        (with-read kmc-tokens (at 'id token) 
            { 'supply := supply
            , 'number := token-number }
            (if (<= token-number 20000) 
                (let 
                    (
                        (first-id (read-burned (get-token-nft-id2 (int-to-str 10 (- token-number 10000)))) )
                    )
                    (enforce (= true first-id)
                        (format "Miner {} was not immersed in fluid and is not eligible for upgrade" [(- token-number 10000)])))
                (let 
                    (
                        (new-id (read-burned (get-token-nft-id2 (int-to-str 10 token-number))) )
                    )
                    (enforce (= true new-id)
                        (format "Legendary Miner was not immersed in fluid and is not eligible for upgrade. Use the KMC website immersion tool to fix this. If you believe this is an error, please open a ticket in our discord server"))
                ))
