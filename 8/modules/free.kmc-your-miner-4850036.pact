(module kmc-your-miner GOVERNANCE
  @doc "Kadena Mining Club NFT Your Miner policy. NFTs representing physical ASIC miners"
    ...
    (defschema supported-asic-schema
        @doc "A list of all currently supported ASICs"
        supported-asics:list 
    )
    ...
    (defschema token-schema
        ...
        asic-number:integer ; counts up from 1 for each type of miner
        electric-expiry:time ; 11:59:00 UTC at the end of their 6month/1year
        advertised-hashrate:decimal
    ...
    (deftable supported-asics:{supported-asic-schema})
    ...
    (defun initialize ()
        ...
        (insert counts-table "KA3" {"count": 0})
        (insert counts-table "L7" {"count": 0})
        (insert counts-table "S19" {"count": 0})
        (insert supported-asics ASIC_LIST {"supported-asics": ["KA3" "L7" "S19"]})
    )
        ...
        (with-capability (ADMIN_OR_DISCORD)
            ...
            (let*
                (
                    ...
                    (asic-number (+ (get-count asic-model) 1))
                    (electric-expiry (time (read-msg "expiry")))
                    (advertised-hashrate (read-msg "advertisedHasrate"))
                    (hashrate-units (read-msg "hashrate-units"))
                    (watcher-link (read-msg "watcher-link"))
                    (first-owner (read-msg "first-owner"))
                ...
                (insert kmc-tokens (at 'id token)
                    ...
                    , "asic-number": asic-number
                    , "electric-expiry": electric-expiry
                    , "advertised-hashrate": advertised-hashrate
                    , "hashrate-units": hashrate-units
