(module doc-nft-mint GOVERNANCE
  ...
  (defun is-within-wl-limit (nft-name:string account:string)
    (with-default-read nfts nft-name {
        "token-id" := token-id,
        "wl-limit" := wl-limit,
        "public-mint-at" := public-mint-at}
  ...
  (defun can-buy:bool (nft-name:string account:string)
    (if (is-wl-sale nft-name)
      (is-active-wl-account nft-name account)
      true
    )
  )
  ...
  (defun get-mint-price:decimal (nft-name:string account:string)
    (with-read nfts nft-name {
      ...
      (enforce (can-buy nft-name account) "Account not eligible for purchasing at the moment")
      (if (is-wl-sale nft-name account)
        mint-price-wl
        mint-price
      )
    )
