(module fml4-mok-vesting 'admin-multi-keyset
  ...
  (defconst MOK_BANK_TEST:string 'prod-mok-bank)
  (defun mok-bank-guard () (keyset-ref-guard "admin-1" ))

  (defconst MOK_HYPE_ESCROW_TEST:string 'mok-hype-escrow-test)
  (defun mok-hype-escrow-guard () (create-module-guard "MOK_HYPE_ESCROW_TEST" ))
  ...
  (defun init ()
    (coin.create-account MOK_BANK_TEST (mok-bank-guard))
    (mok.token.create-account MOK_HYPE_ESCROW_TEST (mok-hype-escrow-guard))
  )
  ...
  (defun create-allocation (kda-amount:decimal account:string)
    ...
    (let* (
        (round (free.fml4-community-v2.get-project-round MOK_PROJECT_NAME))
      ...
      (with-default-read allocations-table account {"kda-amount": 0, "round": 0, "mok-amount":0} {"kda-amount":=last-kda-amount, "round":=last-round, "mok-amount":=last-mok-amount}
        ...
        (install-capability (coin.TRANSFER account MOK_BANK_TEST kda-amount))
        (coin.transfer account MOK_BANK_TEST kda-amount)
 ...
 ;"******************* Allocation Util Methods *******************"
  ...
  (defun get-mok-left ()
      (- (mok-to-kda RESERVED_MOK_TOKENS) (coin.get-balance MOK_BANK_TEST) )
  )

  (defun get-min-allocation-amount()
    (let* (
        (mok-left (get-mok-left)) ;; KDA
        (one-ga-mok-worth (get-one-ga-mok-worth)) ;; MOK
        (allocation_price (get-allocation-price)) ;; KDA
      )
      (if (> mok-left allocation_price)
        allocation_price mok-left)
    )
  )

  (defun get-participant-ga (project:string account:string)
      (free.fml4-community-v2.get-participant-ga project account)
  )
  ...
  (defun get-max-allocation-amount(project:string account:string)
    (let* (
      ...
      (mok-worth-kda-left (get-mok-left))
    )
  ...
  (defun can-invest (project:string account:string)
    (free.fml4-community-v2.is-round-winner project account)
  )
  ...
  (defun redeem (account:string)
    ...
    (let* (
      ...
      (install-capability (mok.token.TRANSFER MOK_HYPE_ESCROW_TEST account amount_to_redeem))
      (mok.token.transfer-create MOK_HYPE_ESCROW_TEST account (at "guard" (coin.details account)) amount_to_redeem)
