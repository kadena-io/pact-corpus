(module kmc-policy GOVERNANCE
    ...
    (defun set-token-nft-id (token-id:string count:string)
        (insert number-to-id-table count
            { "token-id" : token-id })
    )

    (defun update-burned (token-id:string)
        (enforce-kmc)
        (insert burned-both-table token-id
            { "burned-both" : true })
    )
    ...
    (defun enforce-mint:bool
        ...
        (with-read kmc-tokens (at 'id token) 
            { 'supply:= supply
            , 'number := token-number }
            (enforce (read-burned (get-token-nft-id2 (- token-number 10000))) "This miner was not immersed in fluid and is not eligible for upgrade")
