(module jodie-token GOVERNANCE

  @doc " 'jodie-inu' is a project that is entirely community-driven, decentralised, and fully operational launched on the Kadena Network. "
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset 'jodie-token-admin)
  )
  ...
  (defconst ADMIN_ACCOUNT_ID "k:2ea8b0c5186e11cb5a301e6dc707a7c25251432343311911409fb96a43263aa6"
    " ID for admin accont" )

  (defconst INITIAL_SUPPLY:decimal 10000000000.0
 ...
 (defconst PUBLIC_WL_KEY "public_wl_role"
  ...
  (defconst PREMINE_SALE_AMOUNT:decimal 2500000000.0
    " Amount of tokens to sell during IDO.")

  (defconst PREMINE_COUNT_KEY "premine_count_key"
    " Amount of tokens to sell during WL.")

  (defconst MIN_BUY_KDA:decimal 100.0
  ...
  (defconst JDE_PER_KDA 38461.0
    " How much JDE tokens 1 KDA can buy. ")
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
      (create-account ROOT_ACCOUNT_ID (keyset-ref-guard "jodie-token-admin"))
  ...
  (defun buy:string
    ...
    @doc " Function to let people buy the token. "
      (with-capability (INTERNAL)
        (let
          (
              (amount-jde (* amount-kda JDE_PER_KDA ) )
          )
          (enforce-passes-wl account amount-jde)
          (increase-premined-amount amount-jde)
          (coin.transfer account ADMIN_ACCOUNT_ID amount-kda)
          (install-capability (TRANSFER ROOT_ACCOUNT_ID account amount-jde) )
          (transfer-create ROOT_ACCOUNT_ID account guard amount-jde )
        )
      )
  )
  ...
  (defun enforce-wl-buy-conditions (user-id:string buy-amount:decimal user-wl-role:string)
    ...
    (let
        ...
        (enforce (>= new-balance (min-buy-jde)) " Must buy above minimum amount" )
        (enforce (<= new-balance (max-buy-jde)) " Cannot buy above maximum amount" )
    )
  )
  ...
  (defun min-buy-jde ()
    @doc " Minimum amount in JDE a user can buy "
    (* MIN_BUY_KDA JDE_PER_KDA)
  )

  (defun max-buy-jde ()
    @doc " Minimum amount in JDE a user can buy "
    (* MAX_BUY_KDA JDE_PER_KDA)
  )
