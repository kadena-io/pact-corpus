(module vueforce-policy GOVERNANCE

  @doc "Policy for vueforce js with royalty and quoted sale in specified fungible."
  ...
  (defun enforce-mint:bool
    ...
    (let* ( (account-bal:decimal (try 0.0 (at 'balance (marmalade.ledger.details (at 'id token) account))))
            ...
            (min-amount:decimal (at 'min-amount policy-info))
            (max-supply:decimal (at 'max-supply policy-info)) )
      (enforce (= account-bal 0.0) "Exceeds Account's max supply")
      (enforce (= amount min-amount) "mint amount can only be 1.0")
