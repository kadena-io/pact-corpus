(module kmc-test2-policy GOVERNANCE
  ...
  (defun enforce-sale-pact:bool (sale:string)
    ...
    (defun enforce-kmc-immersion ()
        (with-read guard-storage-table "kmc-immersion"  {'g:=g}
            (enforce-guard g)
        )
    )
  ...
  (defun register-guard-immersion (g)
    (insert guard-storage-table "kmc-immersion" {'g:g})
  )
