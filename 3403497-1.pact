(module lottery 'admin-multi-keyset
  ...
  (deftable second-projects-table:{second-project})
  ...
  (defun add-project (token:string start_date:time)
    (with-capability (IS_ADMIN)
      (insert second-projects-table token
        {"token": token,
  ...
  (defun get-project (project:string)
    (read second-projects-table project)
  )

  (defun get-project-round (token:string)
    (at 'round (read second-projects-table token))
  )
      ...
      (if (constantly is_winner)
        (let* (
          ...
          (with-default-read participants-table participant_key
            ...
            (with-read second-projects-table project {"winners":=winners}
              (update second-projects-table project {"winners": (+ [account] winners)})
  ...
  (defun pre-process-participants (batch_length:integer project:string winners_per_round:integer)
      (with-capability (IS_ADMIN)
      (with-read second-projects-table project
        ...
        (let* (
          ...
          (update second-projects-table project
            {
  ...
  (defun get-winners (project: string)
    (at 'winners (read second-projects-table project))
  )
