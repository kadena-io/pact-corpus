    ...
    (defun create-marmalade-token:string 
        ...
        (let*
            (
                ...
                (count (int-to-str 10 (get-count NFT_COUNT)))
                (pre-10k-token-id (at 'token-id (kadena-mining-club.get-token-id count)))
            )
            (kmc-policy.set-token-nft-id pre-10k-token-id count)
