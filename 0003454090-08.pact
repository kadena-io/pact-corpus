(module kedao GOVERNANCE
  ...
  (defcap UPDATE_USER_ACCOUNT (uid:string partial_account:object)
    @event
    (compose-capability (IS_KEDAO_BOT))
  )
  ...
  (defcap REWARDED (account:string begin_date:string end_date:string amount:decimal)
    @event
    (compose-capability (IS_KEDAO_BOT))
  )
  ...
  (defun update-user-account:string (uid:string partial_account:object)
    (with-capability (UPDATE_USER_ACCOUNT uid partial_account)
      (let*
        (
          (user (read users uid))
          (accounts (at 'accounts user))
          (new_accounts (+ partial_account accounts))
        )
        (update users uid {
  ...
  (defun calc-rewards:[string] (begin_date:string end_date:string)
    @doc "Calc rewards every week"
    (enforce
      ( =
    ...
    (let*
      (
        ...
        (weekly_score_records (query-score-records (map (at 'key ) all_users) begin_date end_date 7))
        ...
        (combine-user-score 
          ( lambda 
            ...
            (let* 
              (
                (has-same-account (lambda (user record) (= (at 'key user) (at 'uid record))))
              )
              (
                + 
        ...
        (calc-rewards
          ( lambda
            ...
            {
              "uid" : (at 'key user),
              "account" : (at 'main (at 'accounts (get-user (at 'key user)))),
              "amount" : (round (
                +
  ...
  (defun pay-rewards:[string] (begin_date:string end_date:string)
    ...
    (let
        ...
        ( lambda 
          ...
          (if
            ...
            (let
              ...
              (with-capability (REWARDED (at 'uid record) begin_date end_date (at 'amount user))
                (install-capability (coin.TRANSFER KEDAO_REWARDS_POOL (at 'account user) (at 'amount user)))
                (coin.transfer KEDAO_REWARDS_POOL (at 'account user) (at 'amount record))
                (insert reward-records (format "{}_{}_{}" [(at 'uid record), begin_date, end_date])
                  record
                )
              )
            )
      ...
      (with-capability (REWARDS_POOL_AUTH)
        (map (pay) (calc-rewards begin_date end_date))
      )
    )
  )
  (defun pay-rewards-with-calc-results:[string] (results:[object] begin_date:string end_date:string)
    @doc "Pay rewards"
    (let
      ((pay 
        ( lambda 
  ...
  (defun get-score-records-by-date:[object] (date:string)
    ...
    (filter (lambda (x) (= (at 'date x) date)) (get-all-score-records))
  )
  (defun query-score-records:[object] (uids:[string] begin_date:string end_date:string days_cnt:integer)
    ...
    (let*
      ...
      (enforce (= days_cnt (length dates)) "days cnt doesn't match")
  ...
  (defun get-reward-records:[object] (uid:string)
    @doc "Query reward records by certain user"
    (filter (lambda (x) (= (at 'uid x) uid)) (get-all-reward-records))
  )
  (defun query-reward-records:[object] (uids:[string] begin_date:string end_date:string days_cnt:integer)
    @doc "Query reward records by certain user and certain dates"
    (let*
      (
        (dates (get-dates-between begin_date end_date))
        (get-record-with-default (lambda (uid) (try {"uid":""} (read reward-records (format "{}_{}_{}" [uid, begin_date, end_date])))))
      )
      (enforce (= days_cnt (length dates)) "days cnt doesn't match")
      (filter (lambda (x) (!= "" (at 'uid x))) (map (get-record-with-default) uids))
    )
  )
  ...
  (defun migrate-score-records-step1:[string] ()
    (let*
      (
        (all_users (get-all-users))
        (consumer (lambda (key obj) (+ {
          "key": key,
          "uid": (at 'uid (at 0 (filter (lambda (user) (= (at 'main (at 'accounts user)) (at 'account obj))) all_users)))
        } obj)))
  ...
  (defun migrate-reward-records-step1:[string] ()
    (let*
      (
        (all_users (get-all-users))
        (consumer (lambda (key obj) (+ {
          "key": key,
          "uid": (at 'key (at 0 (filter (lambda (user) (= (at 'main (at 'accounts user)) (at 'account obj))) all_users)))
        } obj)))
        (update-uid (lambda (record)
          (update reward-records (at 'key record) {
            "uid": (at 'uid record)
          })
        ))
      )
      (map (update-uid) (fold-db reward-records (constantly true) (consumer)))
    )
  )

  (defun migrate-reward-records-step2:[string] ()
    ; modify reward-records
    (with-capability (IS_KEDAO_BOT)
      (let*
        (
          (create-new (
            ...
            (let*
              ((key (
                  ...
                  [
                    (at 'uid old-record),
                    (at 'begin_date old-record), 
                    (at 'end_date old-record)
                  ]
                ))
              )
              (insert reward-records key {
                "uid": (at 'uid old-record),
                "begin_date": (at 'begin_date old-record),
                "end_date": (at 'end_date old-record),
                "amount": (at 'amount old-record)
              })
            )
          ))
        )
        (map (create-new) (get-all-reward-records-without-filter))
      )
    )
  )
