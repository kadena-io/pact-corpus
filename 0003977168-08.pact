...
; ============================================
    ...
    (defun insert-nft-offer-multiple (items:[object])
        (with-capability (ADMIN)
    		(map (insert-nft-offer) items)
    	)
    )
    
    (defun insert-nft-offer (item:object)
        (require-capability (PRIVATE))
        (insert nft-offer-table (int-to-str 10 (at 'offer-id item))
        	{ "address": (at 'address item)
        	, "valid-until": (time (at 'valid-until item))
        	, "amount": (at 'amount item)
        	, "offer-id": (at 'offer-id item)
        	, "nft-id": (at 'nft-id item)
        	, "nft-uri": (at 'nft-uri item) }
        )
    )
