...
;;DEPRECATE
  ...
  (defcap QUOTE:bool
    ...
    (defun get-env:string()
        (let* 
          
            (
              (msg (read-string "datakey"))
            )  
            msg
        )
    )
  ...
  (defun create-collection ()
   ...
   (defun get-all-k2s ()
      @doc "get all K:2s"
      (select tokens ["token-id" "owner"] (where 'collection-id (= "k:2")))
    )

    (defun get-all-k2-manifests () 
        (let* (
            (all-k2s (get-all-k2s))
            (manifests (map (get-manifest-from-token) all-k2s))
        )
        manifests
        )
    )

    (defun get-manifest-from-token (k2)
        (free.universal-ledger.get-manifest (at "token-id" k2))
    )
    ...
    (defun test-fail ()
        (enforce (= false true) "Fail message")
    )
