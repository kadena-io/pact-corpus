(module radio02 GOVERNANCE
  ...
  (deftable nodeExtras7:{nodeExtra})
  ...
  (defun get-nodeExtras ()
    "Get all nodeExtras"
    (select nodeExtras7 (constantly true))
  )

  (defun update-node-extra (address online hosted oracle factor)
    (with-default-read nodeExtras7 address 
      {"address": "", "online": false, "hosted": false, "oracle": false, "factor": 0.000001}
      {"address":= addressDb, "online":= onlineDb, "hosted":= hostedDb, "oracle":= oracleDb, "factor":= factorDb}
      (if (= addressDb "")
      (insert nodeExtras7 address {
          "address": address,
          "online": online,
          "hosted": hosted,
          "oracle": oracle,
...
; create-table
; (create-table radmin)
; (create-table nodeExtras7)
; (create-table gateways7)
; (create-table gwTransfers7)
; (create-table node-stats7)
