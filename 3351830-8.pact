(module doc-nft GOVERNANCE

  @doc "Module for minting the Docushield/Mintit tokens"

  (defcap GOVERNANCE ()
    (enforce-guard "doc-access-nft-admin"))
  ...
  (defun init-nft:bool (
    ...
    (enforce (>= wl-limit 0.0) "Whitelist limit should not be negative")
    (enforce (>= limit 0.0) "Regular limit should not be negative")
    (enforce-guard guard)
