...
; ============================================
    ...
    (defun init-kda-mined-all (index-object:[{kda-mined-schema}])
        (map (init-kda-mined) index-object)
    )

    (defun init-kda-mined (index-object:{kda-mined-schema})
        (insert kda-mined-table (get-count KDA_MINED_UPDATE_COUNT)
            { "kda-mined" : (at 'kda-mined index-object)
            , "datetime" : (time (at 'datetime index-object)) }
        )
    )
