(module mod-fixed-quote-policy GOVERNANCE
  ...
  (deftable collection-info3:{collection-schema2})
  ...
  (defun initialize-collection (
    ...
    (write collection-info3 (at 'name new-collection) {
    ...
    (defun initialize-tokens-list (new-tokens-list:[string] collection:string)
        ...
        (with-read collection-info3 collection
            ...
            (with-read mint-status MINT_STATUS
                ...
                (update mint-status MINT_STATUS 
                    { "tokens-list": (+ old-tokens-list (map (str-to-int 64) new-tokens-list))
                    , "current-length": (+ (length new-tokens-list) old-length)
                    }
                )
                (with-read collection-info3 collection
                    { "tokens-list" := collection-tokens-list }
                    (update collection-info3 collection
                        { "tokens-list": (+ collection-tokens-list new-tokens-list) }
                    )
                (format "Old length = {}, new additions = {} and curr-list = {}, new-list = {}" [old-length (length new-tokens-list) collection-tokens-list new-tokens-list]) 
                )
            )
        )
    )
