(module ktoshi GOVERNANCE
  ...
  (implements kaddex.supply-control-v1)
  ...
  (defcap GOVERNANCE ()
    (enforce-keyset (read-keyset 'ktoshi-admin-keyset)))

  (defcap OPS ()
    (enforce-keyset (read-keyset 'ktoshi-ops-keyset)))
  ...
  (defconst TOKEN_PURPOSES
    [{ 'purpose: 'dao,             'percentage-cap: 0.225 }
     { 'purpose: 'rewards,         'percentage-cap: 0.225 }
     { 'purpose: 'liquidity,       'percentage-cap: 0.15 }
     { 'purpose: 'sale,            'percentage-cap: 0.15 }
     { 'purpose: 'marketing,       'percentage-cap: 0.1 }
     { 'purpose: 'team,            'percentage-cap: 0.1 }
     { 'purpose: 'advisors,        'percentage-cap: 0.05 }
  ...
  (defun init ()
    ...
    (with-capability (GOVERNANCE)
      (insert token-table ISSUANCE_ACCOUNT
        { 'balance: 0.0
        , 'guard: (read-keyset 'ktoshi-admin-keyset) })
      ...
      (assign-privilege BURN_PRIVILEGE (read-keyset 'ktoshi-admin-keyset))
      (assign-privilege MINT_PRIVILEGE (read-keyset 'ktoshi-admin-keyset))
    )
  )
)
