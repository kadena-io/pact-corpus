(module marmalade-staging11-ledger GOVERNANCE
  ...
  (defcap GOVERNANCE ()
    (enforce-guard (keyset-ref-guard 'hype-admin-local-test-staging)))
  ...
  (defun get-account-tokens (account:string)
      (map (get-manifest)   (map (at "id") (select ledger ["id"] (and? (where "balance" (< 0.0)) (where 'account (= account)) )))   )
  )

  (defun get-collection-tokens (collection:string)
     (filter (lambda (manifest) (= (at "collection" (at "datum" (at 0 (at "data" manifest)))) collection)) (map (get-manifest) (keys tokens)))
  )
  ...
  (defun mint:bool
    ...
    (with-capability (MINT id account amount)
      (let (
        (distribution-contract:module{staging11-callable} (free.staging11-callable-contracts.get "hype-nft-distribution")))
