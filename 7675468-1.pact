(module brawler-bears GOV
    ...
    (bless "t2svb9lmjXgJkGEAOWiAS084sUZI-MlpH6j0WXL_5V4")
    ...
    (defun create-battle (nft-id:string amount:decimal)
        ...
        (with-capability (OWNER nft-id)
            (let*
                ...
                (coin.transfer owner ADMIN_ADDRESS 2.0)
    ...
    (defun perform-attack:string (nft-id:string battle-id:string attack-id:string default:bool m:module{test-i-2})
          ...
          (let
            ...
            (if (= new-defender-health-power 0) 
              (update battles-table battle-id {
                ...
                "playerOneLastAttack": (if (= owner playerOne) (if default name attack) (at 'playerOneLastAttack battle)),
                "playerTwoLastAttack": (if (= owner playerTwo) (if default name attack) (at 'playerTwoLastAttack battle)),
                "updated-at": (at "block-time" (chain-data))
              })
              (update battles-table battle-id {
                "turn": defender,
    ...
    (defun heal (nft-id:string potion:string attacks:list m:module{test-i-2})
        @doc "Heal an NFT"
      (with-capability (OWNER nft-id)
        (let*
          (
          ...
          (enforce (< (at "current-battle-id" nft) max-health-power) "Already at maximum HP")
          (if (and (= "free" potion) (!= last-free-healed-at "")) 
            (enforce (> (diff-time (at "block-time" (chain-data)) last-free-healed-at) (* 24 60 60)) "You can only heal for free once a day") 
          true)

          (if (= "free" potion) (m::replenish-bulk account attacks))
          (if (= "half" potion) (with-capability (PRIVATE)(arkade.token.transfer account ADMIN_ADDRESS 1200.0)))
          (if (= "full" potion) (m::replenish-bulk account attacks))
