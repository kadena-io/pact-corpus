...
; ============================================
    ...
    (defun init-init-multiple (token-ids:[string])
        (with-capability (CALL-POLICY-MODULES)
        (with-capability (PRIVATE)
            (map (init-init) token-ids)
        ))
    )
    
    (defun init-init (token-id:string)
        (require-capability (PRIVATE))
        (kmc-founders-policy.init-token-id-status-table token-id)
