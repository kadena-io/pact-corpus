(module prod-nft-sale-royality-policy GOVERNANCE
  ...
  (defun enforce-mint:bool
    ...
    (bind (get-policy token)
      ...
      , 'max-supply:=max-supply:decimal
        ...
        (let* ((can_mint (can-mint account (get-token-collection token) (at "id" token))))
          (enforce can_mint "Can user mint")
  ...
  (defun allow-mint (account:string collection:string tokens:[string])
    (let (
      (distribution-contract:module{callable} (hypercent.prod-callable-contracts.get "hype-nft-distribution")))
