(module kadena-place-paint GOV ; ---- CHANGE ON NEW
  ...
  (defcap OWNER (account:string)
    "capability"
    (compose-capability (ACCOUNT-GUARD account))
  )
  ...
  (defun enforce-owner (account:string)
  ...
  (defun get-pixels-data:list (pixel-id-strs:list)
    "get pixels data"
    (map
      (lambda
        (
  ...
  (defun get-place-price:string ()
    "get place price"
    (at "price" (read place-table "data"))
  )
)
