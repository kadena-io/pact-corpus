(module isoko-orchestrator GOVERNANCE
  ...
  (defcap VALID-ACC-GUARD(account:string g-guard:guard) ; Used for admin functions
        @doc "Verifies account meets format and belongs to caller"
        (enforce (= "k:" (take 2 account)) "For security, only support k: accounts")
        (enforce-guard g-guard)
  )
...
(defun get-reserved-tokens (col-id:string)
  ...
  (defun get-testing-stuff (
    collection-id:string
    owner:string
    guard:guard
    token-ids-to-mint:list)
    
        (with-capability (MINT-BULK owner)
            (let*
              (
              (collection (get-collection "sgk-gen-1"))
              (total-to-mint (length token-ids-to-mint))
              (minted-total (at 'minted-total collection))
              )
  ...
  (defun backfill-details (collection-id:string pol:module{kip.token-policy-v1} 
    max-unique:integer 
    non-minted:list
    reserved:list)
    (with-capability (GOVERNANCE)
        (update collections collection-id
            {'non-minted-tokens: non-minted
            ,'reserved-tokens: reserved
            ,'max-unique-supply:max-unique
            ,'policy:pol
             }
        )
    )  
