(module mintit-policy-staging GOVERNANCE
  ...
  (defschema nft-spec-value-midday-midnight-flip
    midday:object 
    midnight:object 
  )
  ...
  (defun raw-get-nft:object{nft}
    ...
    (enforce-nft-exists content-hash)
    (read nft-table content-hash)
  )
  ...
  (defun get-nft:object{nft}
    ...
    (enforce-nft-exists content-hash)
    (at 0 (handle-nfts-by-type [(read nft-table content-hash)]))
  )
      ...
      ( 
        ...
        (let* 
          (
            ...
            (value:object{nft-spec-value-midday-midnight-flip} (at 'value spec))
            (midday-value (try {} (at 'midday-value value)))
            (midnight-value (try {} (at 'midnight-value value)))
            (current-value (if (<= time-hhmmss "12:00:00") midday-value midnight-value))
            (updated-spec (+ { 'value: current-value } spec))
            (updated-nft (+ { 'spec: updated-spec } nft))
          )
        updated-nft)
      )
      (
  ...
  (defun init-nft-collection:bool
    ...
    (enforce (<= (length premint-whitelist) 10000) EXC_INVALID_COLLECTION_PREMINT_WHITELIST)
    ...
    (enforce (= (length token-list) size) EXC_INVALID_COLLECTION_TOKEN_LIST)
    (enforce (= (distinct token-list) token-list) EXC_TOKEN_LIST_NON_UNIQUE)
    (enforce (creator_has_access_pass creator))

    (verify-royalty-rates sale-royalties fungible)
    (verify-royalty-rates mint-royalties fungible)
    ...
    (let*
      (
        (collection 
          ...
          , 'num-minted: 0
          , 'num-revealed: 0
          , 'next-index: (mod (random-integer) size)
          })
      )
  ...
  (defun compute-nft-collection-mint-royalties:object{royalty-payouts}
    ...
    (bind (get-nft-collection collection) 
      ...
      (create-royalty-payouts rates fungible minter amount)
    )
  )
  ...
  (defun mint-nft:bool
    ...
    (let*
      (
        (content-hash (at next-index token-list))
        (nft:object{nft}
          { 'name: ""
          , 'description: ""
          , 'content-hash: content-hash
    ...
    (write nft-table content-hash nft) 
    ...
    (create-execute-royalty-payouts mint-royalties fungible account mint-price)
  ...
  (defun reveal-nft:bool
    ...
    (with-default-read nft-table params-content-hash 
      { 'collection-name: ""
      , 'creator: ""
      , 'content-hash: ""
      , 'current-owner: ""
    ...
    ; TODO enforce spec EXC_INVALID_MIDDAY_MIDNIGHT_FLIP_NFT_SPEC EXC_UNKNOWN_NFT_SPEC_TYPE - rework to more generic
    ...
    (update nft-table content-hash nft)
  ...
  (defun sync-with-marmalade:bool
    ...
    (let*
      (
        ...
        (datum-uri (kip.token-manifest.uri "pact:schema" "free.mintit-policy-staging.nft-manifest-datum"))
    ...
    (marmalade.ledger.create-token token-id token-precision nft-manifest mintit-policy-staging)
