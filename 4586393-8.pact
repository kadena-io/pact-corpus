...
; ============================================
    ...
    (defun immerse (account:string vial-id:string miner-id:string)
        ;TODO;enforce old-miner is not staked or for sale
        (let*
            (
                (miner-number (int-to-str 10 (at 'number (kmc-policy.get-token miner-id))))
                (new-miner-id (kmc-policy.get-token-nft-id miner-number)) ; TODO need to make new function in kmc-policy
                                                            ;alternatively we can just add 10,000 to the current miner number when initializing,
                                                            ;then subtract 10,000 and replace the value at the end of the transaction
            ...
            ; kmc-policy.set-token-nft-id miner-number new-miner-id
        )
    )
