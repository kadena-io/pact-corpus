  ...
  @doc " Grab yours algo-gen Inu Gangster cause something serious Ahusharmota is about to happen  \
    ...
    (defun inu-club-fight (owner:string kdaAmount:integer gangster:string)
        ...
        (let (
            ...
            (enforce (= owner gangster-owner) "You are not the owner of this NFT")
        )
    ...
    (defun calcGangsterHp (gangsterId:string)
      ...
      (require-capability (PRIVATE))
      (with-capability (PRIVATE)
        (let* ((gangster-stats (select nfts ["strength" "wisdom" "charisma" "endurance"] (where "id" (= gangsterId))))
               (sum (fold (+) gangster-stats)))
          sum
        )
      )
    )
