(module kda-skellies-testnet GOV
	...
	(defconst STAKING_END_TIME "2023-11-13T22:40:00Z")
    ...
    (defun calculate-reward (stakedTime:time)
        ;enforce(enforce-staking-live)
        (enforce (>= (at "block-time" (chain-data)) STAKING_END_TIME) (ceiling (* (/ (diff-time (at "block-time" (chain-data)) STAKING_END_TIME 86400) 0.02) 6)))
        (enforce (<= (at "block-time" (chain-data)) STAKING_END_TIME) (ceiling (* (/ (diff-time (at "block-time" (chain-data)) stakedTime 86400) 0.02) 6)))
    )
