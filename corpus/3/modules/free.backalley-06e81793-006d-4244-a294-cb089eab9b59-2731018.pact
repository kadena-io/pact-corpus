(module backalley-06e81793-006d-4244-a294-cb089eab9b59 GOVERNANCE
  ...
  (defconst POOL_ID:string "06e81793-006d-4244-a294-cb089eab9b59"
    "The Pool ID.")

  (defconst ESCROW_ACCOUNT:string "ba-b-06e81793-006d-4244-a294-cb089eab9b59"
  ...
  (defconst OWNER:string "k:2ad0abae3e25f1d73b68599cb003c6f3873f8c46bdd641be51bf32eafe4abffc"
    "Account that initiated this pool")

  (defconst TOKEN_AMOUNT:decimal 700000000.000000000000
    "Amount of tokens for sale.")

  (defconst MIN_RAISE:decimal 1.000000000000
    "Mininimum raise amount necessary for a pool to be considered successful.")

  (defconst MAX_RAISE:decimal 500000.000000000000
    "Maximum raise after which the pool will not accept more deposits.")

  (defconst MIN_ALLOCATION:decimal 1.000000000000
    "Minimum individual allocation.")

  (defconst MAX_ALLOCATION:decimal 750.000000000000
    "Maximum individual allocation.")

  (defconst START_DATE:time (time "2022-02-14T12:00:00Z")
    "The date that the sale starts.")

  (defconst END_DATE:time (time "2022-02-20T23:59:59Z")
    "The date that the sale ends.")

  (defconst DISTRIBUTION_DATE:time (time "2022-04-21T23:59:59Z")
    "The date after which tokens will start being available for redeem.")

  (defconst VESTING_SHARE:decimal 0.040000000000
    "Percentage of tokens available for redeem after each vesting period.")

  (defconst VESTING_PERIOD_IN_SECS:integer 604800
  ...
  (defun init ()
    ...
    (free.backalley-token.create-account ESCROW_ACCOUNT (escrow-guard))
  ...
  (defun fund (account:string amount:decimal)
    ...
    (let ( (funds (get-funding-amount)) )
      ...
      (with-capability (FUND account amount)
        (free.backalley-token.transfer account ESCROW_ACCOUNT amount)
  ...
  (defun redeem-tokens:string (from-acct:string to-acct:string amount:decimal)
    ...
    (with-capability (REDEEM-TOKENS from-acct to-acct amount)
      (let
        ...
        (install-capability (free.backalley-token.TRANSFER ESCROW_ACCOUNT to-acct amount))
        (free.backalley-token.transfer-create ESCROW_ACCOUNT to-acct to-guard amount)
  ...
  (defun withdraw-tokens:string ()
    ...
    (with-capability (WITHDRAW-TOKENS OWNER)
      (let ( (unsold-tokens (get-unsold-tokens)) )
        (install-capability (free.backalley-token.TRANSFER ESCROW_ACCOUNT OWNER unsold-tokens))
        (free.backalley-token.transfer ESCROW_ACCOUNT OWNER unsold-tokens)
