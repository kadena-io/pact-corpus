(module babena-ledger GOVERNANCE
  ...
  (implements free.babena-poly-fungible-v2)
  (use free.babena-poly-fungible-v2 [account-details sender-balance-change receiver-balance-change])
  ...
  (defschema token-schema
    ...
    precision:integer
    supply:decimal
    policy:module{free.babena-token-policy-v1}
  )
  ...
  (defschema collection-schema
    ...
    policy:module{free.babena-token-policy-v1}
    guard:guard
    token:object{free.babena-token-policy-v1.token-info}
  )
  ...
  (defun enforce-transfer-policy
    ...
    (bind (get-policy-info id)
      { 'policy := policy:module{free.babena-token-policy-v1}
      , 'token := token }
  ...
  (defun enforce-collection:bool (name:string)
    (with-read collections name
      ...
      (enforce-one "Only Admin or Collection creator can create tokens" [(enforce-guard collection-guard) (enforce-guard (keyset-ref-guard 'babena-admin))])
    )
  )
  ...
  (defun mint-babena:bool
    ...
    (require-capability (PRIVATE))
      (let* ( (collection-info:object{collection-schema} (get-collection collection))
              (policy:module{free.babena-token-policy-v1} (at 'policy collection-info))
              (token:object{free.babena-token-policy-v1.token-info} (at 'token collection-info))
              (index:string (policy::enforce-mint token account guard amount))
  ...
  (defun mint-bulk-babena
    ...
    (let ( (policy:module{free.babena-token-policy-v1} (at 'policy (get-collection collection)))
  ...
  (defun claim-reserved (account:string)
    (with-capability (CLAIM-RESERVED account)
      (let* ( (reservation-details (read-reservation account))
              (babena-reserved:integer (at 'amount-babena reservation-details))
