(module kdoge GOVERNANCE
  ...
  @doc " 'jeena-token' is a project that is entirely community-driven, decentralised, and fully operational launched on the Kadena Network. "
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset "free.kdoge2-admin")
  )
  ...
  (defconst ADMIN_ACCOUNT_ID "k:aad2aa29823b3e28338b025236aa2e8db566074f7daa8531570896aeb909137f"
    " ID for admin accont" )
 ...
 (defconst PUBLIC_WL_KEY "public_wl_role"
  ...
  (defconst JEE_PER_KDA 10000000.0
    " How much JEE tokens 1 KDA can buy. ")
  ...
  (defun buy:string
    ...
    @doc " Function to let people buy the token. "
      (with-capability (INTERNAL)
        (let
          (
              (amount-jee (* amount-kda JEE_PER_KDA ) )
          )
          (enforce-passes-wl account amount-jee)
          (increase-premined-amount amount-jee)
          (coin.transfer account ADMIN_ACCOUNT_ID amount-kda)
          (install-capability (TRANSFER ROOT_ACCOUNT_ID account amount-jee) )
          (transfer-create ROOT_ACCOUNT_ID account guard amount-jee )
        )
      )
  )
  ...
  (defun enforce-wl-buy-conditions (user-id:string buy-amount:decimal user-wl-role:string)
    ...
    (let
        ...
        (enforce (>= new-balance (min-buy-jee)) " Must buy above minimum amount" )
        (enforce (<= new-balance (max-buy-jee)) " Cannot buy above maximum amount" )
    )
  )
  ...
  (defun min-buy-jee ()
    @doc " Minimum amount in JEE a user can buy "
    (* MIN_BUY_KDA JEE_PER_KDA)
  )

  (defun max-buy-jee ()
    @doc " Minimum amount in JEE a user can buy "
    (* MAX_BUY_KDA JEE_PER_KDA)
  )
