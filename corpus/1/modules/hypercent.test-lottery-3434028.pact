(module test-lottery 'hyper-admin-multi-keyset
  (defcap IS_ADMIN ()
  ...
  (defun has-guaranteed-allocation(project:string account:string)
      (hypercent.test-community.has-ga account project)
  )
  ...
  (defun add-participant (account:string tickets:integer project:string)
    (enforce-guard (at 'guard (test-hype-coin.details account)))
    (let* (
      ...
      (if (constantly is_winner)
        ...
        (let* (
            ...
            (debit-tickets (test-community.debit-tickets account tickets))
