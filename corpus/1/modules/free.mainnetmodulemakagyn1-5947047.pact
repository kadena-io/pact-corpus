(module mainnetmodulemakagyn1 GOV ; ---- CHANGE ON NEW
  ...
  (defconst POOL:string "kadena-place mainnet pool mainnetmodulemakagyn1") ; ---- CHANGE ON NEW
  ...
  (defun create-account (account:string)
    ...
    (insert account-table account {"joined":(at "block-time" (chain-data)),"min-tip":0.0,"tip-count":0})
  )

  (defun buy-one (pixel-id:integer account:string color:string request-price-hundredths:integer is-new-pixel:bool)
    "buy a pixel"
    (validate-color color)
    (validate-pixel-id pixel-id)
    (let
      (
      ...
      (with-capability (INTERNAL "buy")
        (if is-new-pixel
          (buy-new pixel-id-str account color place-price (/ request-price-hundredths 100.0) true)
          (buy-used pixel-id-str account color place-price (/ request-price-hundredths 100.0) true)
        )
      )
    )
  )
            ...
            (lambda
              ...
              (let
                (
                  ...
                  (request-price-hundredths:integer (at "request-price-hundredths" pixel))
                  (is-new-pixel (at "is-new-pixel" pixel))
                  (place-price (at "price" (read place-table "data")))
                )
                (validate-color color)
                (validate-pixel-id pixel-id)
                (let
                  ...
                  (with-capability (INTERNAL "buy")
                    (if is-new-pixel
                      (buy-new pixel-id-str account color place-price (/ request-price-hundredths 100.0) false)
                      (buy-used pixel-id-str account color place-price (/ request-price-hundredths 100.0) false)
                    )
                  )
                )
              )
  ...
  (defun buy-new:object (pixel-id-str:string account:string color:string place-price:decimal request-price:decimal pay:bool)
    ...
    (let
      ...
      (let
        ...
        (insert pixel-table pixel-id-str {"price":pixel-price-new,"owner":account,"last-claim-place-price":place-price-new})
  ...
  (defun buy-used:object (pixel-id-str:string account:string color:string place-price:decimal request-price:decimal pay:bool)
    ...
    (let*
      ...
      (let
        ...
        (update place-table "data" {"price":place-price-new})
        (update pixel-table pixel-id-str {"owner":account,"last-claim-place-price":place-price-new,"price":pixel-price-new})
  ...
  (defun change-pixel-color (pixel-id:integer color:string)
  ...
  (defun tip (from-account:string to-account:string amount:decimal message:string)
    "tip an account"
  ...
  (defun get-tip:object (tip-hash:string)
  ...
  (defun get-pixel-data:object (pixel-id:integer)
    "get pixel data"
    (let*
      (
        (pixel-id-str (int-to-str 10 pixel-id))
      ...
      (+ (+ {"price-hundredths":(floor (* (at "price" pixel) 100))} (drop ["price"] pixel)) (read color-table pixel-id-str))
    )
  )
