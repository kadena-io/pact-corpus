(module kda-skellies GOV
	...
	(defconst FUNDED_STAKING_BANK_BALANCE 2500.00)
	(defconst IMAGES_URI "https://arkade-prod.s3.us-east-1.amazonaws.com/kadena-skellies")
	(defconst STAKING_START_TIME "2023-11-21T00:00:00Z")
	(defconst STAKING_END_TIME "2024-02-29T00:00:00Z")
	(defconst BUTTER_END_TIME "2024-04-25T02:00:00Z")
    ...
    (defun initialize ()
        ...
        (coin.create-account STAKING_BANK (create-BANK-guard))
	...
	(defun transfer-bulk:string
		...
		enforce(enforce-butter-is-live)
		(with-capability (ADMIN)
			(map 
				(transfer receiver)
			  ids
    ...
    (defun stake-bulk (owner:string ids:list )
		enforce(enforce-butter-is-live)
        enforce(enforce-staking-live)
        (map (stake-item owner) ids )
    )

    (defun stake-item (owner:string id:string)
		enforce(enforce-butter-is-live)
        enforce(enforce-staking-live)
        (stake id owner)
    )
    ...
    (defun claim-unstake-bulk (owner:string ids:list)
		enforce(enforce-butter-is-live)
        (map (claim-unstake-item owner) ids )
    )

    (defun claim-unstake-item (owner:string id:string)
		enforce(enforce-butter-is-live)
        (unstake id owner)
    )
    ...
    (defun claim-without-unstake-bulk (owner:string ids:list)
		enforce(enforce-butter-is-live)
        enforce(enforce-staking-live)
        (map (claim-without-unstake-item owner) ids )
    )

    (defun claim-without-unstake-item (owner:string id:string)
		enforce(enforce-butter-is-live)
        enforce(enforce-staking-live)
        (claim-without-unstake id owner)
    )
    ...
    (defun reward-from-stake (account:string stakedTime:time)
        (require-capability (PRIVATE))
			(let (
				...
				(coin.transfer-create STAKING_BANK account (at 'guard (coin.details account)) reward)
    ...
    (defun calculate-reward (stakedTime:time)
        (if (> (at "block-time" (chain-data)) (time STAKING_END_TIME)) (ceiling (* (/ (diff-time (time STAKING_END_TIME) stakedTime) 86400) 0.02) 6)
        (ceiling (* (/ (diff-time (at "block-time" (chain-data)) stakedTime) 86400) 0.02) 6))
    )
    ...
    (defun get-nft-staked (nft-id:string)
        ...
        ;; if id not in table return object with staked set to false so other functions can work
        (try 
            {"staked":false}
            (read staked-table nft-id)
        ) 
    ...
    (defun get-current-total-staked ()
		(length (select staked-table ["staked"] (where "staked" (= true))))
    )
    ...
    (defun claimed-rewards () 
        (try 0.0 (with-read counts TOTAL_REWARDS_CLAIMED_KEY {"count":=count} count))
    )

    (defun withdraw-staking-bank (amount:decimal)
        (with-capability (ADMIN)
            (coin.transfer-create STAKING_BANK ADMIN_ADDRESS (at 'guard (coin.details ADMIN_ADDRESS)) amount)
        )
    )																													 
    ...
    (defun enforce-staking-live ()
        @doc "Enforces staking is live or has ended"
		(enforce (>= (at "block-time" (chain-data)) (time STAKING_START_TIME)) "Staking is not live.")
        (enforce (<= (at "block-time" (chain-data)) (time STAKING_END_TIME)) "Staking has ended.")
    )			 

    (defun enforce-butter-is-live ()
        @doc "Enforces butter NFT's are live"
        (enforce (<= (at "block-time" (chain-data)) (time BUTTER_END_TIME)) "Butter NFT's offline.")
    )
