(module kor-og-badge GOV
  ...
  (defun get-all-percent-payout-infos:[object{percent-payout-info}] ()
    (let 
      ...
      (map (transform (get-percent-payout)) (get-all-owned-og-badges))
    )
  )
  ...
  (defun get-all-owned-og-badges:[object{og-badge}] ()
    (select og-badges (where "account" (!= "")))
  )
  ...
  (defun update-account (nft-id:string account:string)
    (with-capability (OPS)
