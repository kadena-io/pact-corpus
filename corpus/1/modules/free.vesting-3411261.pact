(module vesting 'admin-multi-keyset
  ...
  (defconst VESTING_TYPES_IDO [{"months": 0, "percent": 33}, {"months": 2, "percent": 33}, {"months": 5, "percent": 34}])
  ...
  (defun add-account-vesting-entries  (account:string amount:decimal vesting_type:string curr_time:time)
    (require-capability (CAN_VEST))
    (let* (
        (inserts (map (build-vesting-entry-json account curr_time vesting_type amount) VESTING_TYPES_IDO))
      )
