(module unitt-presale GOVERNANCE
  ...
  (use coin)
  (use utt-token [ transfer get-balance create-account transfer-create ])
  (use util.guards)

  ;; Constants -----------------------------------------------------------------
  (defconst ADMIN_KEYSET:string "n_e0c95a382916e9c475dfc756cf247c903f9876e9.presale-admin-keyset")
  (defconst PRESALE_PROCEEDS_ACCOUNT:string "k:4d3baf29041329a813fa50ef33d4cac08ed58fff56521b08b6d82832e48c479b") ; all KDA from presale goes here
  (defconst PRESALE_REWARDS_ACCOUNT:string "k:4d3baf29041329a813fa50ef33d4cac08ed58fff56521b08b6d82832e48c479b") ; all UTT rewards to users come from here
  ...
  ;(defconst PRESALE_START:time (time "2024-05-21T15:00:00Z")) ; presale starts on May 21th, 2024 at 15:00 UTC
  (defconst PRESALE_START:time (time "2024-05-18T12:00:00Z")) ; just for testing, TODO: replace with the above
  (defconst PRESALE_END:time (add-time PRESALE_START (days 31))) ; presale lasts 4 weeks

  ;(defconst REWARD_START_DAYS:integer 180) ; wait n days after the first swap, then start giving out rewards
  (defconst REWARD_START_DAYS:integer 1) ; just for testing, TODO: replace with the above
  ...
  (defcap SWAP (amount:decimal)
    ...
    (with-default-read swapping-stats-table ""
      ...
      (enforce (<= (+ amount totalSwappedKDA) TOTAL_UTT_AVAILABLE) "Not enough presale tokens left.")
    )
  )
  ...
  (defun initialize ()
    ...
    (with-capability (GOVERNANCE)
      ; set initial values
      (insert swapping-halted-table "" { "halted": false } )
      ...
      (insert price-table KDA_USD_ROW_KEY { "price": 0.77 })

			; create the account for presale rewards protected by a user guard
      (utt-token.create-account PRESALE_REWARDS_ACCOUNT (create-BANK_DEBIT-guard))
    )
  )
      ...
      (with-capability (SWAP amountKDA)
        ...
        (with-default-read swaps-table account
          ...
          (let ((earnedUTT (calculate-utt-amount amountKDA)))
            ...
            (with-default-read swapping-stats-table ""
              { "totalSwappedKDA": 0.0 } { "totalSwappedKDA":= totalSwappedKDA }
              (update swapping-stats-table "" { "totalSwappedKDA": (+ totalSwappedKDA amountKDA) })
            )
          )
        )
      )
  ...
  (defun claim-utt (account:string amountUTT:decimal)
    ...
    (with-capability (WITHDRAW account amountUTT)
      (with-read swaps-table account
        ...
        (install-capability (utt-token.TRANSFER PRESALE_REWARDS_ACCOUNT account amountUTT))
        (utt-token.transfer-create PRESALE_REWARDS_ACCOUNT account guard amountUTT)
    ...
    (with-default-read swaps-table account
      ...
      (let ((daysSince (days-since swapTime)))
				(if (< daysSince REWARD_START_DAYS) 0.0
					(let ((totalRewards (* totalEarnedUTT REWARD_RATIO)))
