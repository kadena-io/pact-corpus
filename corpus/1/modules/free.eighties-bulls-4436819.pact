(module eighties-bulls 'bulls-keyset
  "80s Bulls"
    ...
    (defschema no-minted-nfts-schema
    ...
    (deftable no-minted-nfts:{no-minted-nfts-schema})
    ...
    (defun create-80s-bull-for-sale (item:object)
        ...
        (let 
            ((id (int-to-str 10 (at "edition" item))))
            (insert no-minted-nfts id
                item
            )
        )
    ...
    (defun buy-80s-bull (owner:string number:integer)
        ...
        (let (
            ...
            (mint-bull (int-to-str 10 (at "id" data)) {
                "id": (int-to-str 10 (at "id" data)),
                "item": data,
    ...
    (defun get-latest-80s-bull-to-mint-data ()
        (let 
            ...
            (let 
                (
                    (data (at minted-count (keys no-minted-nfts)))
                )
                data
            )
        )
