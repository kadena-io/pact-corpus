(module lazy-apes 'lazy-apes-keyset
  "Lazy Apes Mint"
    ...
    (defun giveaways (id:string owner:string)
        @doc "Mints a new lazy ape as admin for giveaways"
        (require-capability (ADMIN))
        (let (
            (count (at "count" (read counts LAZY_APES_MINTED_COUNT_KEY ['count])) )
            )
            (insert minted-nfts id {
                    "id": id,
                    "item": { "edition": id },
                    "date_minted": (at "block-time" (chain-data)),
                    "owner": owner,
            ...
            (update counts LAZY_APES_MINTED_COUNT_KEY 
                {"count": (+ 1.0 (count LAZY_APES_MINTED_COUNT_KEY))} 
            )
    
            (update counts LAZY_APES_CREATED_COUNT_KEY 
                {"count": (+ 1.0 (count LAZY_APES_CREATED_COUNT_KEY))} 
            )
        )
    ...
    (defun get-latest-lazy-ape-to-mint-data ()
        (let 
            ...
            (let 
                (
                  (data (at (- (floor minted-count) 462) (keys new-not-minted-nfts)))
                )
                data
            )
        )
