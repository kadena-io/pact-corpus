(module tokensale GOVERNANCE
  ...
  ; Pool types
  (defconst PUBLIC 0)
  (defconst WHITELIST 1)
  (defconst KDLHOLD 2)
  (defconst KDLSTAKE 3)

  (defconst KDL_VAULT_ACCOUNT "freekdlaunch-vault-account")
  ...
  (defcap POOL_REQUIREMENTS_MET:bool (poolId from)
    (with-read pools-table poolId {
      "kdlHold":=kdlHold,
      "poolType":=poolType
      }

      (if (= poolType PUBLIC) true (if (= poolType KDLHOLD) (validate-kdl-hold from kdlHold) true))
    )
  )
  ...
  (defun validate-kdl-hold (account kdlHold)
    (let ((kdlBalance (coin.get-balance account)))
      (enforce (>= kdlBalance kdlHold) "HOLDING REQUIREMENT NOT MET")
    )
  )
  ...
  (defun create-pool (
    ...
    (enforce (< 0.0 allocation) "Allocation is not a positive number")
  ...
  (defun withdraw-tokens (projectId projectOwner projectOwnerGuard)
    (with-capability (GOVERNANCE)
      (with-read projects-table projectId {
        "token":= customToken:module{fungible-v2}
        }
        (customToken::transfer-create KDL_VAULT_ACCOUNT projectOwner projectOwnerGuard (customToken::get-balance KDL_VAULT_ACCOUNT))
      )
    )
  )
  ...
  (defun fund-project (projectId poolId from amount)
    (with-capability (ACCT_GUARD from)
      (with-capability (POOL_OPEN poolId projectId)
        (with-capability (POOL_REQUIREMENTS_MET poolId from)
  ...
  (defun dummy-transfer (from to amount)
    (kdlaunch.token.transfer from to amount)
  )
)
...
; (create-table pools-table)
; (create-table funds-table)
; (create-table airdrops-table)
