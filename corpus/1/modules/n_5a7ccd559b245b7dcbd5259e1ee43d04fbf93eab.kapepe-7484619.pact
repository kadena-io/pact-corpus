(module kapepe GOVERNANCE

  @doc " Make Kadena Great Again "
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset "n_5a7ccd559b245b7dcbd5259e1ee43d04fbf93eab.kapepe-admin-keyset")
  )
  ...
  (defconst ROOT_ACCOUNT_ID:string "kapepe-vault"
    " ID for the account which initially owns all the tokens. ")

  (defconst INITIAL_SUPPLY:decimal 77000000000.0
    " Initial supply of 7.7 billion tokens. ")
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
        (coin.create-account ROOT_ACCOUNT_ID (keyset-ref-guard "n_5a7ccd559b245b7dcbd5259e1ee43d04fbf93eab.kapepe-admin-keyset"))
        (create-account ROOT_ACCOUNT_ID (keyset-ref-guard "n_5a7ccd559b245b7dcbd5259e1ee43d04fbf93eab.kapepe-admin-keyset"))
        (update token-table ROOT_ACCOUNT_ID { "balance" : INITIAL_SUPPLY })
    )
  )

  (defun get-all-holders ()
    @doc " Return all token holders. "
    (keys token-table))

  (defun get-all-holders-details (accounts:list) 
    @doc " Return all token holders balances . "
    (map (details)
        accounts))

  (defun swap-old-for-new
    (sender:string)

    (enforce (= (at 'chain-id (chain-data)) "1")
     " Only accepting swaps from Chain 1. ")
     
    (validate-account-id sender)

    (let ((amount (free.kapepe-coin.get-balance sender))
    (sender-guard (at 'guard (coin.details sender))))
    ;   (enforce-guard sender-guard)
    ;   (create-account sender sender-guard)
    (if (contains sender (keys token-table))
      "Account exists" (create-account sender sender-guard))

      (enforce (> amount 0.0) "Swap amount must be positive.")
      (enforce-unit amount)
      
      (with-capability (DEBIT sender)
        (free.kapepe-coin.debit-swap sender amount))
        
      (write token-table sender
          { "balance" :     (if (contains sender (keys token-table))
                (+ amount (get-balance sender)) amount)
          , "guard"   : sender-guard
      })
        
      (let ((new-sender-balance (get-balance sender)))
        (enforce (= amount new-sender-balance) "Balance update error."))
    ))
