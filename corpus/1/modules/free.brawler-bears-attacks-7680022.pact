(module brawler-bears-attacks GOV
    ...
    (defconst PUBLIC_MINT_START_TIME "2023-05-28T19:00:00Z")
    ...
    (defun replenish-bulk:string (account:string attacks:list)
      ...
      (map (lambda (attack)
             (let* 
                (
                  (nft (read nfts attack))
                  (max-power (at 'max-power nft))
                )
                (with-capability (OWNER attack)
                    (update nfts attack {"power": max-power})
                )
             )
           ) 
           attacks
    ...
    (defun replenish-item:string (account:string attack-id:string)
      @doc "Replenish the power of NFTs to their max-power"
      (with-capability (PRIVATE)(arkade.token.transfer account ADMIN_ADDRESS 500.0))
        (let* 
            (
              (nft (read nfts attack-id))
              (max-power (at 'max-power nft))
            )
            (with-capability (OWNER attack-id)
                (update nfts attack-id {"power": max-power})
            )
        )
    )
    ...
    (defun get-owner-unequiped-attacks (owner:string)
      ...
      (defun get-item-details:string (id:string)
