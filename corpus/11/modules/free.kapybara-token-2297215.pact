(module kapybara-token GOVERNANCE

  @doc " 'Kapybara' is one of the meme tokens launched on the Kadena Network. \
  ...
  (bless "lpyddBq96IbddPF5eLWn-24Nn7i_jV5LeDst9BtnM_0")
  (bless "NwknPbAhpAoQ0kqWWspXJ92iMDNRsyNpeUp6iRL1mHs")
  (bless "6ZbgoP_j3W1ARLmf6W9lg4xVD7BtEjTxV8SbBVOkc50")
  ...
  (defconst ADMIN_ACCOUNT_ID "k:921b0de453da7e95897ffe980bee268c9acc1f71e1accd3e348432db7fc5c07a"
    " ID for admin accont" )
 ...
 (defconst PUBLIC_WL_KEY "public_wl_role"
  ...
  (defconst PREMINE_SALE_AMOUNT:decimal 500000000000.0
    " Amount of tokens to sell during WL.")

  (defconst PREMINE_COUNT_KEY "premine_count_key"
    " Amount of tokens to sell during WL.")

  (defconst MIN_BUY_KDA:decimal 25.0
     " Minimum amount of KDA worth of tokens a user can buy. ")

  (defconst MAX_BUY_KDA:decimal 1000.0
    " Maxinimum amount of KDA worth a user can buy. ")

  (defconst KAPYS_PER_KDA 6250000.0
    " How much Kapy tokens 1 KDA can buy. ")
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
      (create-account ROOT_ACCOUNT_ID (keyset-ref-guard 'kapybara-admin))
  ...
  (defun buy:string
    ...
    @doc " Function to let people buy the token. "
      (with-capability (INTERNAL)
        (let
          (
              (amount-kapys (* amount-kda KAPYS_PER_KDA ) )
          )
          (enforce-passes-wl account amount-kapys)
          (increase-premined-amount amount-kapys)
          (coin.transfer account ADMIN_ACCOUNT_ID amount-kda)
          (install-capability (TRANSFER ROOT_ACCOUNT_ID account amount-kapys) )
          (transfer-create ROOT_ACCOUNT_ID account guard amount-kapys )
        )
      )
  )
  ...
  (defun enforce-wl-buy-conditions (user-id:string buy-amount:decimal user-wl-role:string)
    ...
    (let
        ...
        (enforce (>= new-balance (min-buy-kapys)) " Must buy above minimum amount" )
        (enforce (<= new-balance (max-buy-kapys)) " Cannot buy above maximum amount" )
    )
  )
  ...
  (defun min-buy-kapys ()
    @doc " Minimum amount in kapys a user can buy "
    (* MIN_BUY_KDA KAPYS_PER_KDA)
  )

  (defun max-buy-kapys ()
    @doc " Minimum amount in kapys a user can buy "
    (* MAX_BUY_KDA KAPYS_PER_KDA)
  )
