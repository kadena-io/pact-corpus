...
; ============================================
    ...
    (defun create-multiple-marmalade-tokens (t-data:[object] burned:bool number:string hacked-token-id:string)
        @doc "Creates multiple tokens from Objects"
        (with-capability (ACCOUNT_GUARD DISCORD_ADDRESS)
            (with-capability (PRIVATE)
                (map (create-marmalade-token burned number hacked-token-id) t-data)
            )
        )
    )

    (defun create-marmalade-token:string 
          (burned:bool number:string hacked-token-id:string t-data:object
          )
