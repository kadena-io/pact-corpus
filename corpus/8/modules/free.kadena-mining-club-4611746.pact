        ...
        (let*
            (
                ...
                (test (with-capability (PRIVATE)
                                    ...
                                    (at 'token-id (kmc-policy.get-token-nft-id2 (int-to-str 10 (+ 1 legendary-count))))
                                ))
                ; (new-miner-id (if (contains vial-id legendary-vials)
                ;                 (with-capability (PRIVATE)
                ;                     (increase-count LEGENDARIES_MINTED_COUNT)
                ;                     (at 'token-id (kmc-policy.get-token-nft-id (int-to-str 10 (+ 20001 (get-count legendary-count)))))
                ;                 )
