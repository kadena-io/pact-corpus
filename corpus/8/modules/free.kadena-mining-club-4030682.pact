...
; ============================================
    ...
    (defun init-init-multiple (index1:integer index2:integer)
        (with-capability (CALL-POLICY-MODULES)
            (map (init-init) (enumerate index1 index2))
        )
    )
    
    (defun init-init (old-nft-id:integer)
        (with-read nft-token-pairs (int-to-str 10 old-nft-id)
            { "token-id":= token-string }
            (kmc-founders-policy.init-token-id-status-table token-string)
        )
    )
...
; ============================================
    ...
    (defun get-founders-details:string (item:object)
    ...
    (defun get-user-founders-object (account:string)
        @doc "Returns all Founder's Passes owned by one address"
        (map (get-user-founders) (kmc-founders-policy.get-tokens-owned account)) ;todo make fancier
...
; ============================================
    ...
    (defun transfer-founder:string (id:string receiver:string guard:guard)
    ...
    ; (defun get-all-miners-for-sale ()
    ;     @doc "Gets all Miner NFTs for sale, indexes are inclusive"
    ;     (take (- (- index-end (- index-start 1))) (take index-end (sort (select mledger ["nft-id", "for-sale", "market-price", "uri"] (where "for-sale" (= true))))))
    ; )
