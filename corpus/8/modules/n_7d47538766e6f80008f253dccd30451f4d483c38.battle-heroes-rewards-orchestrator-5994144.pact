(module battle-heroes-rewards-orchestrator GOVERNANCE
    ...
    (use n_f1c962776331c4773136dc1587a8355c9957eae1.isoko-orchestrator [bulk-claim-weapons-staking bulk-claim-weapons-staking-v2])
    ...
    (defun claim-rewards-v2 (account:string tokens:[string])
        @doc "Allows a user to claim multiple rewards"
        (enforce false "Staking is Down for upgrades")
        (with-capability (REWARDS account tokens)
        (with-capability (ACCOUNT_GUARD account)
            (map (claim-reward account) tokens)
            (bulk-claim-weapons-staking-v2 account (length tokens) (at "guard" (coin.details account)))
        ))
    )
