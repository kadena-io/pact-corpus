...
; ============================================
    ...
    (defun initialize-listings (ids:list)
        (with-capability (ADMIN)
    ...
    (defun set-false (id:string)
        (with-capability (ADMIN)
            ...
            (kmc-policy.update-staked id false))
        )
    )
    ...
    (defun accept-collection-offer (account:string nft-id:string offer-id:string)
        ...
        (with-read collection-offer-table offer-id
            ...
            (with-default-read listings nft-id
                ...
                (if (<= 0.0 price); if the price is less than or equal to 0.0, the nft is not listed so it must be transferred from the owner not the kmc bank
                    (marmalade.ledger.transfer-create nft-id orig-owner new-address (at 'guard (coin.details new-address)) 1.0)
                    (transfer-from-marm-bank nft-id new-address)
                )
                ...
                (if (!= -1.0 price) 
                    ...
                    "nft never listed, no need to update price"
                )
            )
    ...
    (defun transfer-from-marm-bank (nft-id:string new-address:string)
        (install-capability (marmalade.ledger.TRANSFER nft-id MARMALADE_BANK new-address 1.0))
        (marmalade.ledger.transfer-create nft-id MARMALADE_BANK new-address (at 'guard (coin.details new-address)) 1.0)
    )
...
; ============================================
    ...
    (defun get-airdrop-details-select (account:string)
        (select claim-table ['claim] (where "account" (= account)))
    )

    (defun get-airdrop-details-select-format (account:string)
        (map (combine-claims) (get-airdrop-details-select account))
    )
    ...
    (defun combine-lists (account:string)
         (let 
            (
                (airdrop (at 'airdrop (read airdrop-table account)))
