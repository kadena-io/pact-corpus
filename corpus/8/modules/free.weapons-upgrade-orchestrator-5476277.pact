(module weapons-upgrade-orchestrator GOVERNANCE
    ...
    (defun create-count (collection:string)
        [
            (insert counter-table (concat ["sgk-weapons-" collection "2"]) {"count": 0})
            (insert counter-table (concat ["sgk-weapons-" collection "3"]) {"count": 0})
            (insert counter-table (concat ["sgk-weapons-" collection "4"]) {"count": 0})
        ]
    )

    (defun get-count:integer (key:string)
        @doc "Gets the count for a key" 
        (at "count" (read counter-table key ['count]))
    )
    
    (defun increase-count (key:string)
        @doc "Increase the count of a key in a table by 1"
        (require-capability (PRIVATE))
        (update counter-table key {"count": (+ 1 (get-count key))})
    )
