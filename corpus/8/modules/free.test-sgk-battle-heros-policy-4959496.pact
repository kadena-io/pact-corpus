(module test-sgk-battle-heros-policy GOVERNANCE
    ...
    (defun enforce-orchestrator:bool ()
        (enforce-one "Any Guard passes" [
            ...
            (enforce-guard (free.sgk-rewards-policies.rewards-orchestrator-guard))
        ])
    )

    (defun enforce-rewards-guards ()
        (enforce-guard (at "guard" (coin.details "k:f157854c15e9bb8fb55aafdecc1ce27a3d60973cbe6870045f4415dc06be06f5")))

       
        ;(enforce-guard (at "guard" (coin.details "k:80427d276c409892b1ff95582468ad0cab3b8aa033ef06a4a13210127c97d4c7")))
        ;(enforce-guard (free.sgk-rewards-policies.rewards-orchestrator-guard))
    )
    ...
    ; Constants
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    (defconst PUBLIC_MINT_TIME "2023-09-30T16:30:00Z")
    (defconst FREE_MINT_TIME "2023-09-25T18:15:00Z")
    ...
    (defconst CREATOR_ADDRESS "k:3744b5b5252cf34412854ca03fa5506819db9fa03eca964874f1798ee4aa2d46"
        ...
        (with-capability (PRIVATE)
            (with-capability (MINT (at 'id token) account)
                ...
                (let* 
                    ...
                    (if (= sender REWARDS_ADDRESS )
                        [
                        (enforce-rewards-guards)
                        (update-account-info-for-rewarded-nfts account-info)
                        ...
                        [
                          (enforce-mint-internal-public token account guard 1 account-info)
                        ]
                    )
                )
            )
      ...
      ( token:object{token-info}
        ...
        account-info:object{account-wl-record-schema})
            ...
            (let*
                ...
                (price:decimal (get-price-by-phase current-phase hasFreeMintsLeft))
                )
                (enforce (= true can-mint) "Mint is not live right now, or is Paused.")
    
                (update-account-info account account-info using-free-mint)
                (bind (get-policy token)
                { 'owner:=owner:string
                }
                (enforce (= (+ amount (at 'supply token)) 1.0) "Exceeds max supply")
    ...
    (defun get-price-by-phase:decimal(current-phase:string hasFreeMintsLeft:bool)
        (cond
            ...
            ((= PUBLIC_PHASE current-phase) 2.0)
            10000000.0)
    )
