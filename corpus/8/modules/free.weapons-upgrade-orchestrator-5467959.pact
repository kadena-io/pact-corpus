(module weapons-upgrade-orchestrator GOVERNANCE
    ...
    (defconst LEVEL_TWO_COUNT "level-two")
    (defconst LEVEL_THREE_COUNT "level-three")
    (defconst LEVEL_FOUR_COUNT "level-four")
    ...
    (defun upgrade (account:string tk-id-one:string tk-id-two:string)
        ...
        (with-capability (PRIVATE)
            ...
            (let*
                ...
                (install-capability (marmalade.ledger.MINT non-minted-token account 1.0))
                (marmalade.ledger.mint non-minted-token account (at "guard" (coin.details account)) 1.0)
    ...
    (defun get-next-col (token-id:string)
        (let*
            ...
            {"next-col-id":next-col-id, "id":id, "col-name":col-name, "next-id":next-id, "next-col-id":next-col-id}
        )
    )
