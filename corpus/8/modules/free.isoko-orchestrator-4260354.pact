(module isoko-orchestrator GOVERNANCE
  ...
  (defcap ADMIN-OR-COL-OWNER (collection-id:string)
    (let*
      ...
      (enforce-one "Any Guard passes" [(enforce-guard (keyset-ref-guard "n_f1c962776331c4773136dc1587a8355c9957eae1.isoko-admin")) (enforce-guard (keyset-ref-guard "free.boxing-badger-wallet-guard"))])
    )
  )
  (defcap MINT-RESERVED-NFTS (collection-id:string)
    (let*
      ...
      (enforce (= "collection-id" "boxing-badger") "Only enabled for boxing badger now")
      (compose-capability (ADMIN-OR-COL-OWNER collection-id))
    )
  )
