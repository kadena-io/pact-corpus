...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   (defun TEST_bulk-claim-weapon-staking:list (
    owner:string
    claim-amount:integer
    guard:guard)
        (let* 
            (
                (claim-list (make-list claim-amount true))
                (token-ids-to-mint:list (map (TEST_claim-weapon-staking owner guard) claim-list))
            )
            (map (mint owner guard) token-ids-to-mint)
        )
   )
  ...
  (defun TEST_claim-weapon-staking:list (
    ...
    (defun TEST-get-and-decrement-random-staking-reserve-collection:string (rand:integer)
        ...
        (let* 
            (
                (collection-suffix-list:list (enumerate 1 2)) 
                ...
                (reserved-cols-list:list  (map (lambda (x) (get-reserve-token-amounts-staking-for-col x)) collection-full-names-list))
    ...
    (defun TEST_TEST_claim-weapon-staking (
    owner:string
    guard:guard)
