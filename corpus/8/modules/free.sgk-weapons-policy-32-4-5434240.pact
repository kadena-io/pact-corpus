(module sgk-weapons-policy-32-4 GOVERNANCE @doc "Policy for fixed issuance with royalty and quoted sale in specified fungible."
...
; Constants
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defconst PUBLIC_MINT_TIME "2024-01-25T00:00:00Z")
(defconst FREE_MINT_TIME "2024-01-25T18:15:00Z")
...
(defconst MAX-UNIQUE-SUPPLY 5000
    @doc "Max unique supply")
...
(defun enforce-mint:bool
    ...
    (with-capability (PRIVATE)
        (with-capability (MINT (at "id" token) account)
            (enforce-orchestrator) ;;enforce guard of burn policy
            (enforce-ledger)
...
(defun enforce-mint-internal-public 
  ( token:object{token-info}
    ...
    account-info:object{account-wl-record-schema})
        ...
        (require-capability (free.weapons-upgrade-orchestrator.PRIVATE))
        (let*
            (
            (nft-id (at "id" token))
            ...
            (enforce (<= split-id-number 4995) "id is not slated for public mint")
