(module mintit-policy-staging-v2 GOVERNANCE
  ...
  (defcap GOVERNANCE
    ()
    (enforce-guard (read-keyset "mintit-admin-staging"))
  )
  ...
  (defun write-nft-collection-token-hash:bool (
    ...
    (with-capability (ADMIN_ACCESS)
      (with-read nft-collection-table
        collection-name
        { 'num-token-initalized := num-token-initalized }

        (let ((index (+ num-token-initalized 1 )))
          ...
          (update nft-collection-table collection-name { 'num-token-initalized : index})
        )
      )
  ...
  (defun write-nft-collection-token-hashes:bool (
    ...
    (map (write-nft-collection-token-hash collection-name) token-list)
  )
  ...
  (defun sync-with-marmalade:bool
    ...
    (let*
      (
        ...
        (datum-uri (kip.token-manifest.uri "pact:schema" "free.mintit-policy-staging-v2.nft-manifest-datum"))
    ...
    (marmalade.ledger.create-token token-id token-precision nft-manifest mintit-policy-staging-v2)
