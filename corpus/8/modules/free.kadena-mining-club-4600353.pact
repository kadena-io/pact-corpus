...
; ============================================
    ...
    ; (deftable legendary-vial-ids:{legendary-vial-id-schema})
...
; ============================================
    ...
    (defun immerse (account:string vial-id:string miner-id:string)
        (let*
            (
                ...
                (test (with-capability (PRIVATE)
                                    (increase-count LEGENDARIES_MINTED_COUNT)               ;20001
                                    (at 'token-id (kmc-policy.get-token-nft-id2 (int-to-str 10 (+ 1 (get-count LEGENDARIES_MINTED_COUNT)))))
                                ))
                ; (new-miner-id (if (contains vial-id legendary-vials)
                ;                 (with-capability (PRIVATE)
                ;                     (increase-count LEGENDARIES_MINTED_COUNT)
                ;                     (at 'token-id (kmc-policy.get-token-nft-id (int-to-str 10 (+ 20001 (get-count LEGENDARIES_MINTED_COUNT)))))
                ;                 )
                ;                 (at 'token-id (kmc-policy.get-token-nft-id (+ 10000 (str-to-int miner-number)))) )
                ; )
            ...
            ; (with-capability (CALL-POLICY-MODULES) (update-burned miner-id))
            ...
            ; (insert mledger new-miner-id
            ;     { "nft-id" : new-miner-id
            ;     , "uri" : (at 'assetUri (at 'datum (at 0 (at 'data (at 'manifest (at 'token (marmalade.ledger.get-policy-info new-miner-id)))))))
            ;     , "hashrate" : 5.1
            ;     , "tied-asic" : []
            ...
            ; )
            ; (format "Used vial of immersion fluid on miner #{}" [miner-number])
        )
    )
    ...
    (defun populate-upgrade (start:integer)
        (with-capability (PRIVATE)
        (with-capability (ADMIN)
            (map (populate-new-upgrade) (enumerate start (+ 1000 start)))
        ))
    )
    
    (defun populate-new-upgrade (item:integer)
        (require-capability (PRIVATE))
        (update mledger (at 'token-id (get-token-id (int-to-str 10 item)))
            { "upgraded" : false }
        )
    )
