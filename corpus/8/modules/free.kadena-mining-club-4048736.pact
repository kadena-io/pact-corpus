...
; [{"id": "t:1Gouz2VbVQsicFEiGaj_up4Gu5BbT4xx51YauDE4HTw"}]
...
; ============================================
    ...
    (defun list-marm-bank (nft-id:string owner:string price:string)
    ...
    (defun delist-marm-bank (nft-id:string owner:string)
        ...
        (with-read listings nft-id 
            ...
            (enforce (and (= orig-owner-guard (at 'guard (coin.details owner)) ) (= owner orig-owner)) "You are not the original owner of this NFT")
    ...
    (defun update-for-sale (nft-id:string for-sale:bool)
        ...
        (let 
            ( 
              (exists (try false (let ((ok true)) (kmc-founders-policy.get-status nft-id) ok)))
            )
            (if (= exists true)
