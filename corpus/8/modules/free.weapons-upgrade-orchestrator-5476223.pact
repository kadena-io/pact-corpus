(module weapons-upgrade-orchestrator GOVERNANCE
    ...
    (defun create-count (collection:string)
        [
            (insert counts-table (concat ["sgk-" collection "2"]) {"count": 0})
            (insert counts-table (concat ["sgk-" collection "3"]) {"count": 0})
            (insert counts-table (concat ["sgk-" collection "4"]) {"count": 0})
        ]
    )
    ...
    (defun get-next-mint-token (token-id:string)
        (let*
            ...
            {"id":id,"col-name":col-name,"next-id":next-id,"next-col-id":next-col-id,"next-mint-id":next-mint-id,"next-mint-token":next-mint-token}
        )
    )
