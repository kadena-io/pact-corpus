(module doc-nft-mint GOVERNANCE
  ...
  (defun update-mint-price-wl:bool (nft-name:string new-mint-price-wl:decimal)
    (enforce-guard "free.doc-access-nft-admin")

    (update nfts nft-name { "mint-price-wl": new-mint-price-wl })
  )

  (defun update-mint-price:bool (nft-name:string new-mint-price:decimal)
    (enforce-guard "free.doc-access-nft-admin")

    (update nfts nft-name { "mint-price": new-mint-price })
  )
  ...
  (defun get-mint-price:decimal (nft-name:string account:string)
    (if (is-wl-sale nft-name)
      ...
      (get-public-mint-price nft-name)
    )
  )
)
