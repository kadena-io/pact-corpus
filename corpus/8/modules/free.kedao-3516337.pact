(module kedao GOVERNANCE
  ...
  (defun create-user:string
    ...
    (let*
      ...
      ; execute creation
      (update-user uid (at 'join_time profile) update_contents)
    )
  )
  ...
  (defun query-point-records-with-uid-range:[object] (uid:string begin_date:string end_date:string)
    (let
      (
        (flt (lambda (key obj) 
          ( let ( (uid (take 6 key)) (date (take -10 key)) )
            (and 
              (contains uid key) 
  ...
  (defun migrate-users:[string] ()
    (map (lambda (user)
      (update users (at 'uid (at 'profile user)) (take ['wallet 'profile 'points 'additions ] (+ {
        "additions": {"auto_redeem": true},
        "points": (get-default-points)
        } user)))
      )
      (get-users)
  ...
  (defun clear-droplets-records:string (key:string)
    (update point-records key {
      "records": (remove 'leaves (remove 'droplets (at 'records (read point-records key))))
    })
  )
