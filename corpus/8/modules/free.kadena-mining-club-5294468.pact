...
; ============================================
    ...
    (defun get-activity (page:integer)
        (let*
            (
                (sale-count (get-count SALE_COUNT)) 
                (start-index (+ (- sale-count (* page 10))1)) 
                (end-index (+ start-index 10))
            )
            (select activity-table 
                (and? (where 'sale-id (>= start-index))
                  (where 'sale-id (<= end-index))))
        )
    )
            ...
            (with-read marketplace-table nft-id 
                ...
                (with-capability (CALL-POLICY-MODULES)
                    (let* 
                        ...
                        (insert activity-table (int-to-str (+ 1 sale-count))
                            { "buyer": new-address
                            , "seller" : orig-owner
                            , "price" : amount
