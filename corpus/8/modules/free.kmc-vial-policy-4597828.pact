(module kmc-vial-policy GOVERNANCE
  @doc "Kadena Mining Club Immersion Vial policy"
    ...
    (defun add-legendary-admin (token-id:string)
        (with-capability (GOVERNANCE)
            (insert legendary-vials token-id { "token-id": token-id } )
        )
    )

    (defun get-all-minted-legendary-vials ()
        (read legendary-vials ['token-id])
    )
    ...
    (defun enforce-mint:bool
        ...
        (if (=  "Legendary" 
            ...
            (insert legendary-vials (at 'id token){ "token-id": (at 'id token) } )
            "")
        (update kmc-tokens (at 'id token) 
            {"supply": 1.0
            ,"owner": account }
        ...
        (let 
            ...
            (enforce (<= number 10008) "Error: There are not enough NFTs remaining in the mint for your transaction")
