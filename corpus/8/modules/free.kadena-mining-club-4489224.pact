...
; ============================================
    ...
    (defschema listings-schema 
        ...
        price:decimal
        updated-at:time
        policy:module{free.kmc-token-policy-v12}
    )
...
; ============================================
    ...
    (defun get-vial-details (nft-id:string)
          ...
          , "price" := price} 
        {
          "old-nft-id" : (int-to-str 10 (at 'number (kmc-founders-policy.get-token nft-id)))
        , "nft-id" : nft-id
        , "owner-address" : (kmc-test-policy.get-owner nft-id)
        , "market-price" : price
        , "price" : price
        , "for-sale" : (at 'for-sale (kmc-founders-policy.get-status nft-id))
        , "original-owner": original-owner }
    ...
    (defun get-user-vials-object (account:string)
        (kmc-test-policy.get-tokens-owned account)
    )
...
;;;;testing updates
    ...
    (defun list-marm-bank2 (nft-id:string owner:string price:decimal)
        ...
        (let 
            (
                (exists (try false (let ((ok true)) (with-read listings-table nft-id {'previous-owner := temp1}"") ok)))
                (policy (bind (marmalade.ledger.get-policy-info nft-id)
                    { 'policy := policy:module{free.kmc-token-policy-v12}}
                    policy))
            )
            (if (= false exists)
                (insert listings-table nft-id { "previous-owner-guard": (at 'guard (coin.details owner)) ,
                    "id": nft-id , "previous-owner": owner , "price": price, "updated-at": (at "block-time" (chain-data)),
                    "policy": policy})
                (update listings-table nft-id { "previous-owner-guard": (at 'guard (coin.details owner)) ,
    ...
    (defun delist-marm-bank2 (nft-id:string owner:string)
        (require-capability (PRIVATE))
        (with-read listings-table nft-id 
        ...
        (update listings-table nft-id 
            { 'price : 0.0} )
        (format "NFT with id {} has been taken off the market" [nft-id])
    )
; Failure: Type error: expected module{free.kmc-token-policy-v12}, found module{kip.token-policy-v1}
    ...
    (defun edit-market-price2 (account:string nft-id:string new-price:decimal)
        ...
        (with-read listings-table nft-id 
            ...
            (with-default-read listings-table nft-id
              { "price": 0.0 }
              { "price":= price }
...
; ============================================
    ...
    (defun reg-test-policy ()
        (with-capability (ADMIN)
            (kmc-test-policy.register-guard (create-capability-guard (CALL-POLICY-MODULES)))
        )
    )
