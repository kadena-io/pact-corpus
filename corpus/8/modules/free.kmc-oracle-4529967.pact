(module kmc-oracle GOVERNANCE
  @doc "Kadena Mining Club oracle contract."
  (use coin)
; ============================================
; ==               CONSTANTS                ==
; ============================================
...
; ============================================
    ...
    (defun set-price (price:decimal price-key:string account:string)
        (with-capability (ADMIN_OR_DISCORD account)
            (update price-table price-key {"price": price})
        )
    )

    (defun admin-set-price (price:decimal price-key:string)
