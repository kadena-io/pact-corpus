(module fsm GOVERNANCE
    ...
    (defconst BANK_KDA_ACCT "fsm-bank") 
    ...
    (defconst FSM_BANK:string "k:ffa795e58cf729e065aa618fcc9451a9895a428d7c9894520860c5a63c0fc0f5")
    (defconst RILEY_ACCOUNT "k:2ebb8a9631802e85be36cdd84a239e8e20fcd952a25943619c7dc7d949cdf3f2") ;TODO change for rileys address
    ...
    (defun initialize ()
        ...
        (with-capability (GOVERNANCE)
            (insert price-table PRICE_KEY { "price": 282.07})
    ...
    (defun insert-nft-collection (collection:object)
        (require-capability (PRIVATE))
        (with-capability (GOVERNANCE)
    ...
    (defun stake-ids (account:string ids:[string] percent-reinvested:decimal stake-type:integer)
		...
		(with-capability (PRIVATE)
			...
			(map (stake-id stake-type percent-reinvested) ids)
		)
    ...
    (defun stake-id (stake-type:integer percent-reinvested:decimal id:string)
        ...
        (enforce (not (is-staked id)) "Already staked")
    ...
    (defun unstake-id (account:string id:string)
        (require-capability (PRIVATE))
        (enforce (is-staked id) "Not staked or Minted")
        (with-capability (OWNER id)
            (if (= (nft-stake-type id) 1)
                [
    ...
    (defcap STAKE_FSM:bool (stake-type:integer percent-reinvested:decimal id:string)
