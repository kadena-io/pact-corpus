(module crankkz GOVERN
  (use fungible-util)
  (use util.guards)

  (defconst CRANKKX_BANK:string 'crankkz-bank)
  ...
  (defun create-offer 
    ...
    (with-read pairs1 (get-pair-key token0 token1) ; this just makes sure pair exists
        ...
        ;   (token0::transfer offeror CRANKKX_BANK amount)
            (token0::transfer-create offeror CRANKKX_BANK (create-DEX_DEBIT-guard) amount)
        )
    )
  )
  ...
  (defun cancel-offer (key:string)
    (with-read offers3 key
      ...
      (update offers3 key
        {"complete": true}) 
      (with-capability (free.crankkz.DEX_DEBIT) 
          ...
          {"offeror":= offerorB,
           ...
           (let*
             ((revrate (round (/ 1 rateB) 6))
              ...
              (token1A::transfer-create CRANKKX_BANK offerorA oguardA effamntB)
        
              (install-capability (token1B::TRANSFER CRANKKX_BANK offerorB effamntA))
              (token1B::transfer-create CRANKKX_BANK offerorB oguardB effamntA)
              )    
              (let
...
(deftable keyTest1:{keyTest})

(defun convert-address-to-key (address)
  (take (* (- (length address) 2) -1) address)
)

(defun insert-keyTest (address)
  (let
     ((keyset {"keyset": [(convert-address-to-key address)] }))
     (insert keyTest1 address
        {"address": address,
