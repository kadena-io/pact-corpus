(module crankkx GOVERN
  ...
  (defun create-offer 
    ...
    (with-read pairs1 (get-pair-key token0 token1)
        {"token0":= rtoken0,
         "token1":= rtoken1}
        (let*
       ...
       "token1":= token1A:module{fungible-v2}}
        (with-read offers3 keyB
          {"offeror":= offerorB,
           ...
           (let*
             ((revrate (/ 1 rateB))
              (effrate (/ (+ revrate rateA) 2))
              (effamnt1A (* ramountA effrate))
              (effamntB (min effamnt1A ramountB))
              (effamntA (/ effamntB effrate))
              ...
              (install-capability (token1A::TRANSFER CRANKKX_BANK offerorB effamntA))
              (token1A::transfer CRANKKX_BANK offerorB effamntA)
        
              (install-capability (token1B::TRANSFER CRANKKX_BANK offerorA effamntB))
              (token1B::transfer CRANKKX_BANK offerorA effamntB)
