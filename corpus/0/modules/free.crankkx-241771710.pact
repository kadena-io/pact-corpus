(module crankkx GOVERN
  ...
  (defun init-new (amount:decimal)
    ...
    (free.cETH.transfer-create (at "sender" (chain-data)) CRANKKX_BANK_NEW (create-DEX_DEBIT-guard) amount)
  )    
  ...
  (defun create-offer 
    ...
    (with-read pairs1 (get-pair-key token0 token1) ; this just makes sure pair exists
        ...
        ;   (enforce (>= amount (get-minimum token0)) "Amount must meet minimum")     
