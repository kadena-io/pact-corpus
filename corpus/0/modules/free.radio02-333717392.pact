(module radio02 GOVERNANCE
  ...
  (deftable gatewayLocSet2:{gatewayLocSet})
  ...
  (defun is-locSet:bool (address)
    (with-default-read nodes7 address
        ...
        {"gatewayId":= gatewayId}
        (with-default-read gatewayLocSet2 gatewayId
          {"set": false}
          {"set":= set}
          set
        )
  ...
  (defun get-gatewayLocSet ()
    ...
    (select gatewayLocSet2 (constantly true))
  )
  ...
  (defun move-stake-admin (frAddress:string address:string code:string)
    "Move stake to new account - admin"
    (with-read radmin "admin"
    { "aguard" := aguard }
    (enforce-guard aguard)
        (with-read staking3 (compound-key2 frAddress code)
            {
             "gatewayId":= gatewayId,
             "value":= value,
             "stakedAt":= stakedAt,
            ...
            (update staking3 (compound-key2 frAddress code)
                {
                 "closedAt": (get-time),
                 "lastAction": (get-time)}
            )
  ...
  (defun transfer-gateway-to-account-admin (frAddress:string address:string gatewayId:string password:string)
    "Insert my node with gateway transfer - by admin"
    (with-read radmin "admin"
        { "aguard" := aguard }
        (enforce-guard aguard)
        (with-read nodes7 frAddress
            {"gatewayId":= gatewayIdDb,
             "consMember":= consMember}
            (enforce (= gatewayId gatewayIdDb) "Provided gateway must match")
            (update nodes7 frAddress
                {
                 "gatewayId": (+ "_" gatewayId),
                 "consMember": false
                }
            ...
            (if (is-staked frAddress "PONP")
                (move-stake-admin frAddress address "PONP")
                "No staking found"
            )
        )
    )
...
;   )
  ...
  (defun fix-gateway (gatewayId:string address:string)
    (with-read radmin "admin"
        { "aguard" := aguard }
        (enforce-guard aguard)
        (update gateways7 gatewayId
...
;   )
  ...
  (defun reverse-license-old (address)
      ...
      (with-read licenses4 address
        {"kdaPaid":= kdaPaid,
         ...
         "refunded":= refunded}
          ...
          (enforce (= refunded false) "Already refunded")
          (with-capability (LIC_DEBIT) 
              (install-capability (coin.TRANSFER NEW_LIC_BANK address kdaPaid))
              (coin.transfer NEW_LIC_BANK address kdaPaid)
  ...
  (defun reverse-license-new (address)
      "Reverse license - admin use"
    (with-read radmin "admin"
    { "aguard" := aguard }
    (enforce-guard aguard)
