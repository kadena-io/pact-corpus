(module simplesales5 GOVERNANCE
  ...
  (deftable reservations5:{reservation})

  (defschema entry
    balance:decimal
    guard:guard)

  (deftable ledger5:{entry})

  (defcap GOVERNANCE ()
    (enforce-guard
      (keyset-ref-guard 'admin-keyset-peters)))
      (defcap DEBIT (sender:string)
        (enforce-guard (at 'guard (read ledger5 sender))))
  ...
  (defcap REDEEM (account:string)
    (enforce-guard (at 'guard (read ledger5 account)))
  )
  ...
  (defconst ZTRUST_BANK:string 'ztrust-bank5)
  ...
  (defun get-balance:decimal (account:string)
    (at 'balance (read ledger5 account))
  )

  (defun details:object{fungible-v2.account-details}
    ( account:string )
    (with-read ledger5 account
      { "balance" := bal
      , "guard" := g }
      { "account" : account
      , "balance" : bal
  ...
  (defun reserve:string (account:string amount-kda:decimal)
    ...
    (let
      ...
      (insert reservations5 (format "{}-{}" [account, tx-id])
        { "account"        : account
  ...
  (defun credit:string (account:string guard:guard amount:decimal)
    ...
    (with-default-read ledger5 account
      { "balance" : 0.0, "guard" : guard }
      { "balance" := balance, "guard" := retg }
      ; we don't want to overwrite an existing guard with the user-supplied one
      (enforce (= retg guard)
        "account guards do not match")
      (write ledger5 account
        { "balance" : (+ balance amount)
        , "guard"   : retg
        })
      ))
  ...
  (defun read-reservations (account:string)
    (select reservations5 (where 'account (= account)))
  )
