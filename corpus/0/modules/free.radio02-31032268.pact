(module radio02 GOVERNANCE
  ...
  (defun award (address:string amount:decimal)
    ...
    (require-capability (INTERNAL))
    (let ((amnt 0.00002))
        (with-read staking3 (compound-key2 address "PONP") 
          {"code":= code}
          amnt = amount
        )
        (with-read nodes7 address
            {"guard":= nodeguard}
            (install-capability (free.crankk01.TRANSFER RADIO01_BANK address amnt))
            (free.crankk01.transfer-create RADIO01_BANK address nodeguard amnt)
        )
    )
  )    
