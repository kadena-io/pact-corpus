(module crankks GOVERN

  (use fungible-util)
  (use util.guards)

  (defconst ROW_KEY_SEPARATOR:string "/")

  (defcap GOVERN ()
  (with-read cAdmin1 "admin"
    { "aguard" := aguard }
    (enforce-guard aguard)
  ))

  (defcap INTERNAL ()
    "mark some functions as internal only...."
    true
  )

  (defun init (aguard:guard)
    "Set admin...."
    (insert cAdmin1 "admin" {
      "admin": (at "sender" (chain-data)),
      "aguard": aguard
      } )
  )

  (defschema admin
    admin:string
    aguard:guard
  )

  (deftable cAdmin1:{admin})

  (defschema nodeExtra
     address: string
     oracle: bool
  )    

  (deftable nodeExtras9:{nodeExtra})

  (defschema tokens
    token:module{fungible-v2}
    symbol:string
    toUSDRate:decimal
  )

  (deftable tokens1:{tokens})

  (defun add-token (token:module{fungible-v2} symbol:string toUSDRate:decimal)
    (adminOnly)
    (insert tokens1 symbol {
        "token": token,
        "symbol": symbol,
        "toUSDRate": toUSDRate
        }
    )
  )

  (defschema subscriptionTypes
    name:string
    description:string
    firstPrice:decimal
    firstPriceCurr:string
    firstPriceTokens:[string]
    firstPeriod:integer
    firstPeriodUnit:string
    price:decimal
    priceCurr:string
    priceTokens:[string]
    period:integer
    periodUnit:string
  )

  (deftable subscriptionTypes2:{subscriptionTypes})

  (defun add-subscriptionType (name:string
    description:string
    firstPrice:decimal
    firstPriceCurr:string
    firstPriceTokens:[string]
    firstPeriod:integer
    firstPeriodUnit:string
    price:decimal
    priceCurr:string
    priceTokens:[string]
    period:integer 
    periodUnit:string )

    (adminOnly)
    (insert subscriptionTypes2 name {
        "name": name,
        "description":description,
        "firstPrice":firstPrice,
        "firstPriceCurr":firstPriceCurr,
        "firstPriceTokens":firstPriceTokens,
        "firstPeriod":firstPeriod,
        "firstPeriodUnit":firstPeriodUnit,
        "price":price,
        "priceCurr":priceCurr,
        "priceTokens":priceTokens,
        "period":period,
        "periodUnit":periodUnit
        }
    )
  )

  (defun adminOnly ()
    (with-read cAdmin1 "admin"
      { "aguard" := aguard }
      (enforce-guard aguard)
    )
  )

  (defun k-account ()
    (enforce (= (take 2 (at "sender" (chain-data))) "k:") "Has to be a k: address")
    (enforce (= (length (at "sender" (chain-data))) 66) "Has to be a k: address")
  )

  (defun account-exists (address token:module{fungible-v2})
    (try -1.0 (token::get-balance address))
  )

  (defun compound-key3:string
    ( part1:string
      part2:string
      part3:string )
    (format "{}{}{}{}{}" [part1 ROW_KEY_SEPARATOR part2 ROW_KEY_SEPARATOR part3])
  )

  (defun update-node-extra (address oracle)
    (adminOnly)
    (with-default-read nodeExtras9 address 
      {"address": "", "oracle": false}
      {"address":= addressDb, "oracle":= oracleDb}
      (if (= addressDb "")
          (insert nodeExtras9 address {
              "address": address,
              "oracle": oracle
              }
          )
          (update nodeExtras9 address {
              "oracle": oracle
              }
          )
      )    
    )
  )

  (defun get-time ()
    (at "block-time" (chain-data))
  )

  (defun get-formatted-time:string ()
    "Get formatted time"
    (format-time "%Y-%m-%dT%H:%M:%S" (get-time))
  )

  (defun get-epoch:time ()
    "Get epoch"
    (time "1970-01-01T00:00:00Z")
  )

  (defun get-token-key
    ( token:module{fungible-v2}
    )
    "Token key"
    (format "{}" [token])
  )

  (defun read-admin ()
    "Read admin guard"
    (with-read cAdmin1 "admin"
      { "admin":= admin,
      "aguard":= aguard }
    (format "Admin guard is {}, admin {}" [aguard admin]))
  )

  (defun get-nodeExtras () 
    (select nodeExtras9 (constantly true)) 
  )

  (defun get-subscriptionTypes () 
    (select subscriptionTypes2 (constantly true)) 
  )

;; Capability user guard: capability definition
(defcap SAL_DEBIT () true)
;; Capability user guard: capability predicate function
(defun require-SAL_DEBIT () 
  (require-capability (SAL_DEBIT)))

;; Capability user guard: guard constructor
(defun create-SAL_DEBIT-guard ()
  (create-user-guard (require-SAL_DEBIT)))
)
; create-table
; (create-table cAdmin1)
; (create-table nodeExtras9)
; (create-table tokens1)
; (create-table subscriptionTypes2)



