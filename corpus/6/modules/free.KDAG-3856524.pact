(module KDAG GOVERNANCE
    ...
    (defcap GOVERNANCE
      ...
      (enforce-keyset "free.admin-kdag")
    )
    ...
    (defconst KDAG_BANK:string "kdag-bank" 
        " Account holding KDAG and KDA. ")
    ...
    (defun debit
      ...
      (enforce-unit amount)
      (if (= accountId KDAG_BANK) (require-capability (INTERNAL)) true)
    ...
    (defun rotate:string
      ...
      (with-read token-table account
        { "guard" := oldGuard }
        (if (= account KDAG_BANK) (require-capability (INTERNAL)) true)
    ...
    (defun initialize ()
      ...
      (coin.create-account KDAG_BANK (create-module-guard "kdag-holdings"))  
      (create-account KDAG_BANK (create-module-guard "kdag-holdings")) 
    ...
    (defun lock-kda
      ...
      (with-default-read supply-table ""
          { "supply" : 0}
          { "supply" := supply }
          (enforce (<= supply 100000000.00) "100,000,000 KDAG Supply has been reached" ) 
      )
      (let* (
        (bal (get-balance KDAG_BANK))
      )
          (enforce (<= bal 100000000.00)  "100,000,000 KDAG Supply has been reached" ) 
      )
      ;Check Last lock-id
      (with-default-read last-id-table ""
        ...
        (with-default-read supply-table ""
          ...
          (enforce (>= duration 240)  "Lock is too short" )
          ...
          (with-default-read token-table KDAG_BANK
            { "balance" : 0.0}
            { "balance" := oldbalance }
            (update token-table KDAG_BANK { "balance" : (+ oldbalance (measurerewardskda amount duration)) }))
          ;Deposit the KDA into the contract
          (coin.transfer account KDAG_BANK amount)
          (with-default-read token-table account
            ...
            { "balance" : oldbalance
            , "guard"   : guard }))
          (format "{} KDA locked for {} KDAG." [amount,(measurerewardskda amount duration)]))) )
    ...
    (defun unlock-kda (id:integer)
      ...
      (with-read lock-kda-table (format "{}" [id])
        ...
        (write lock-kda-table (format "{}" [id])
          ...
          , "timestamp"  : timestamp
          , "withdrawn"  : true })
        ;Send both KDA and KDAG
        (with-capability (INTERNAL)
        (install-capability (coin.TRANSFER KDAG_BANK account amount))
        (coin.transfer KDAG_BANK account amount)
        (transfer KDAG_BANK account rewards)
        (format "Unlocked id {} for {} KDA and {} KDAG." [id,amount,rewards])))))
    ...
    ; --------------------------------------------------------------------------
    ; Kadena KDAG front-end functions
    ; --------------------------------------------------------------------------
  
    (defun getcirculatingsupply ()
      @doc " Get circulating supply. (Total - locked and preminted) "
      (- (at "supply" (read supply-table "" [ 'supply ])) (get-balance KDAG_BANK )))
