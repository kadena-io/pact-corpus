(module coal COALGOV
  ...
  (defcap NFT_STRENGHENED (id:string old-strength:integer new-strength:integer)
    @event true
    )

  (defcap OWNERSHIP_TRANSFER (id:string old-owner:string new-owner:string)
    @event true
  )

  (defcap MINTERSHIP_TRANSFER (id:string old-owner:string new-owner:string)
    @event true
  )
  ...
  (defun no-front-running-me (mhash:string)
    ...
    (defun transfer-ownership (id:string reci:string)
      (enforce-contract-running)
      (with-read coal-tbl id {'owner:=owner,'coal-guard:=coal-guard}
        ...
        (emit-event (OWNERSHIP_TRANSFER id owner reci))
      )
    )

    (defun transfer-mintership (id:string reci:string)
      (enforce-contract-running)
      (with-read coal-tbl id {'minter:=minter,'minter-guard:=minter-guard}
        (enforce-guard minter-guard)
        (update coal-tbl id {'minter:reci,'minter-guard:(at 'guard (coin.details reci))})
        (emit-event (MINTERSHIP_TRANSFER id minter reci))

      )
    )

    (defun transfer-ownership-mintership (id:string reci:string)
      (enforce-keyset "free.coal-maker-keyset")
      (with-read coal-tbl id {'owner:=owner,'coal-guard:=coal-guard,'minter:=minter,'minter-guard:=minter-guard}
        (enforce-guard minter-guard)
        (enforce-guard coal-guard)
        (enforce (= owner ADMIN_ADDRESS) "not admin")
        (enforce (= minter ADMIN_ADDRESS) "not admin")

        (update coal-tbl id {
          'minter:reci,
          'minter-guard:(at 'guard (coin.details reci)),
          'owner:reci,
          'coal-guard:(at 'guard (coin.details reci))})
        (emit-event (OWNERSHIP_TRANSFER id ADMIN_ADDRESS reci))
        (emit-event (MINTERSHIP_TRANSFER id ADMIN_ADDRESS reci))

      )
    )

    (defun set-contract-running (state:integer)
      (enforce-keyset "free.coal-maker-keyset")
      (update coal-int-settings-kv-tbl COALS_RUNNING_STATE_KEY {'teh-val:state})
    )

    (defun coal-maker-mint ()
      (enforce-keyset "free.coal-maker-keyset")
      (enforce (= (at 'chain-id (chain-data)) MINT_CHAIN_ID) (+ "minting only available on chain " MINT_CHAIN_ID))
      (enforce (> (get-mint-price) 0.0) "mint price should not be zero")
      (let ((minter ADMIN_ADDRESS))
        (with-read coal-int-settings-kv-tbl COALS_MINTED_COUNT_KEY {'teh-val := coals-minted}
          (enforce (<= coals-minted TOTAL_COALS) "all coals are minted")
          (let (
            (teh-id (int-to-str 10 coals-minted))
