(module heron GOV
      ...
      (select exempt (constantly true))
        ...
        (defun debit:string (account:string amount:decimal)
          ...
          (defconst TX_BURN:decimal 0.04
            "Percentage of every transaction burned")
      
          (defconst TX_FEE:decimal 0.01
        ...
        (defun credit:string (sender:string account:string guard:guard amount:decimal)
          ...
          (if (or (is-excluded-account account sender) (= account sender))
          (emit-event (BURN_EVENT 0.0))(emit-event (BURN_EVENT (* amount TX_BURN)))
          )
