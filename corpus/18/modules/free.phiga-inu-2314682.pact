(module phiga-inu GOVERNANCE

  @doc "phiga-inu is a new token implemented on the Kadena mainnet"
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset 'phiga-inu-admin)
  )
  ...
  (defconst MINT_CHAIN:string "0"
    "ID of the chain to premint the token")

  (defconst INITIAL_SUPPLY:decimal 1000000000000000.0
    " Initial supply of 1 quadrillion tokens. (Minted on chain MINT_CHAIN)")
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
      (create-account ROOT_ACCOUNT_ID (keyset-ref-guard 'phiga-inu-admin))
