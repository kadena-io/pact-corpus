...
;   )
  ...
  (deftable licensePackTypes2:{licensePackType})
  ...
  (defun update-packUSDtoCRKK (code:string USDtoCRKK:decimal)
    ...
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"oracle":= oracle}
        (enforce (= oracle true) "Must be an oracle node")
        (with-read licensePackTypes2 code
            {"priceUSD":= priceUSD}
            (update licensePackTypes2 code
...
;     hashedPw: string
 ...
 (defun buy-subscription (   
  ...
  (defun get-licensePackTypes () 
    (select licensePackTypes2 (constantly true)) 
  )
