...
;   )
  ...
  (defun add-received-with-check (address:string gatewayId:string mic:string chain:string)
    "Add received"
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"oracle":= oracle}
        (enforce (= oracle true) "Must be an oracle node")
        (let ((received [{"address": address, "mic": mic, "chain": chain}]))
        ; (with-read nodes7 address
        ;     ;This ensures that receiver owns a Gateweay
        ;     {"gatewayId":= receiverGw}
            (with-read gateways7 gatewayId
    ...
    (with-read nodes7 (at "sender" (chain-data))
        ...
        (with-read nodes7 sender
            ...
            (if (> (length receivers) 0)
                (let*
                  ((base (round (/ 91.28 100) 5))
                   (amount (round (/ base (length receivers)) 5)))
                   (if (is-staked sender "PONP")        
                    (award sender base)
                    ""     
                   )    
  ...
  (defun award (address:string amount:decimal)
    ...
    ;   {"code": "123"}
    ;   {"code":= code}
      (let ((guard (format "{}" [(at 'guard (free.crankk01.details-new address))])))
      (if (!= guard "")
    ;   (if (= code "123")
    ;     ;   (with-read nodes7 address
    ;     ;       {"guard":= nodeguard}
    ...
    ;               (free.crankk01.transfer RADIO01_AWARD_BANK address 0.00002)
    ;           )    
        ;   )
        ;   (with-read nodes7 address
        ;       {"guard":= nodeguard}
...
(defun init-award-bank ()
  ...
  (defun init-license-bank ()
      (coin.transfer-create (at "sender" (chain-data)) RADIO01_LICENSE_BANK 
        (create-AWARD_DEBIT-guard) 1.0)
  )
...
; (create-table gwTransfers8)
; (create-table gateways7)
; (create-table nodes7)
; (create-table licenses6)
; (create-table stakeMenu2)
; (create-table staking3)
