(module mesh03 GOVERNANCE

;   (use fungible-util)
  (use util.guards)

  (defconst ROW_KEY_SEPARATOR:string "/")
  (defconst MESH03_BANK:string 'mesh03-bank)
  (defconst MESH03_STAKE_BANK:string 'mesh03-stake01-bank)
  (defconst MESH03_LICENSE_BANK:string 'mesh03-license01-bank)
  (defconst NEW_LIC_BANK:string 'mesh03-license02-bank)
  (defconst CRANKK_SELFAFF_BANK:string 'crankk-selfaff-bank)
  (defconst MESH03_AWARD_BANK:string 'mesh03-award-bank) 
  (defconst CRANKK_FOUND_BANK:string 'crankk-found-bank) 

  (bless "u9wx8ZmqAaFzEaZetp6LSW1zJjuPaCz-AEtdQSSZbYE")
  ...
  (defun update-USDtoCRKK (code:string USDtoCRKK)
    (enforce-guard (at 'guard (read nodes8 (at "sender" (chain-data)) ['guard])))
  ...
  (defun stake (code gatewayId)
    ...
    (if (= (take 4 code) "CONS")
        (with-default-read nodes8 (at "sender" (chain-data))
    ...
    (with-read stakeMenu2 code 
      {"valueInCRKK":= value,
       ...
       "lockup":= lockup}
        ...
        ; (free.crankk01.transfer-create (at "sender" (chain-data)) MESH03_STAKE_BANK 
        ;     (create-STAKE_DEBIT-guard) value)
        (free.crankk01.transfer (at "sender" (chain-data)) MESH03_STAKE_BANK value)
    )
  )
              ...
              (let* 
                ((remTerm (diff-time (add-time stakedAt term) (get-time)))
                 ...
                 (with-capability (STAKE_DEBIT) 
                     (install-capability (free.crankk01.TRANSFER MESH03_STAKE_BANK address retAmnt))
                     (free.crankk01.transfer MESH03_STAKE_BANK address retAmnt)
                     (install-capability (free.crankk01.TRANSFER MESH03_STAKE_BANK CRANKK_FOUND_BANK penalty))
                     (free.crankk01.transfer-create MESH03_STAKE_BANK CRANKK_FOUND_BANK 
          ...
          {"valueInCRKK":= value,
           ...
           "lockup":= lockup}
            ...
            (with-read staking3 (compound-key2 address code)
              ...
              (enforce (= closedAt (get-epoch)) "Already closed")
                 (with-capability (STAKE_DEBIT) 
                     (install-capability (free.crankk01.TRANSFER MESH03_STAKE_BANK address value))
                     (free.crankk01.transfer MESH03_STAKE_BANK address value)
                 )
                 (update staking3 (compound-key2 address code)
                  {"closedAt": (get-time)}
                 )
  ...
  (defun set-my-pubkey (pubkey)
    (enforce-guard (at 'guard (read nodes8 (at "sender" (chain-data)) ['guard])))
       (update nodes8 (at "sender" (chain-data)) 
           {"pubkey": pubkey} 
       )   
  )
  ...
  (defun get-nodes ()
    "Get all nodes8"
    (select nodes8 (constantly true))
  )

  (defun get-my-node ()
    "Get my node"
    (read nodes8 (at "sender" (chain-data)))
  )

  (defun get-my-gatewayExtras ()
    "Get my node"
    (with-read nodes8 (at "sender" (chain-data))
        {"gatewayId":= gatewayId}
  ...
  (defun insert-my-node (gatewayId:string)
    "Insert my node"
        ...
        (let ((miss (- 8 (length gatewayId))))
        (insert nodes8 (at "sender" (chain-data))
            {"address": (at "sender" (chain-data)),
             "guard": (at 'guard (coin.details (at "sender" (chain-data)))),
             "gatewayId": (+ (take miss "00000000") gatewayId),
             "pubkey": "",
             "send": false,
             "director": "",
             "pubkeyd": "",
        ...
        (insert gateways7 (+ (take miss "00000000") gatewayId)
            {"address": (at "sender" (chain-data)),
             "receives": []}
        )
  ...
  (defun update-node-with-gateway (address:string gatewayId:string)
    ...
    (with-read radmin "admin"
        { "aguard" := aguard }
        (enforce-guard aguard)
        (with-read nodes8 address
            {"gatewayId":= gatewayIdDb}
            (update nodes8 address
                {"gatewayId": gatewayId}
            )
...
;   )
  ...
  (defun add-received-with-chain (gatewayId:string mic:string chain:string)
    ...
    ; (enforce-guard (at 'guard (read nodes8 (at "sender" (chain-data)) ['guard])))
    (let* (
        (miss (- 8 (length gatewayId)))
        (received [{"address": (at "sender" (chain-data)), "mic": mic, "chain": chain}]))
        (with-read gateways7 (+ (take miss "00000000") gatewayId)
          {"receives":= receives}
          (if (< (length receives) 5)
            (update gateways7 (+ (take miss "00000000") gatewayId)
                {"receives": (append receives received)}
            )
            "Maximum witnesses reached"
          )     
  ...
  (defun close-send-receive-internal (sender receivers:[object:{receive}] gateways:[object:{witness}])
    ...
    (with-read nodes8 (at "sender" (chain-data))
        ...
        (with-read nodes8 sender
            ...
            (update nodes8 sender
                {"sent": "",
                 "net": (add-time (get-time) 3600), ;hourly for now
    ...
    "Award"
      ...
      (if (account-exists-crkk address)
          (with-capability (AWARD_DEBIT) 
              (install-capability (free.crankk01.TRANSFER MESH03_AWARD_BANK address amount))
              (free.crankk01.transfer MESH03_AWARD_BANK address amount)
          ) 
          ""        
      )
  )
  ...
  (defun has-gps (address) 
    (with-default-read nodes8 address
        {"gatewayId": ""}
        {"gatewayId":= gatewayId}
        (with-default-read gatewayGPSs1 gatewayId
  ...
  (defun mitigate-award-bank (amount:decimal)
    (with-read radmin "admin"
        ...
        (enforce-guard aguard)
          (let* 
            ((admAddress (at 'admin (read radmin "admin" ['admin]))))
            (install-capability (free.crankk01.TRANSFER MESH03_BANK admAddress amount))
            (free.crankk01.transfer MESH03_BANK admAddress amount)
          )
    )
  )     
        ...
        {"used":= used,
         ...
         "kdaPaid":= kdaPaid}
          ...
          (enforce (= refunded false) "Already refunded")
            ...
            ;     {"address" := affAddress,
            ;      "percentage":= affPerc}
            ;   (install-capability (coin.TRANSFER MESH03_LICENSE_BANK affAddress (round (* kdaPaid affPerc) 3)))
            ;   (coin.transfer MESH03_LICENSE_BANK affAddress (round (* kdaPaid affPerc) 3))
...
;           (enforce (= used false) "Already used")
;           (enforce (= refunded false) "Already refunded")
;           (install-capability (coin.TRANSFER MESH03_LICENSE_BANK (at "sender" (chain-data)) kdaPaid))
;           (coin.transfer MESH03_LICENSE_BANK (at "sender" (chain-data)) kdaPaid)
;           (update licenses4 (at "sender" (chain-data)) 
;             {"refunded": true,
;             "refundedAt": (get-time)}
;           )
...
(defun init-award-bank ()
      (free.crankk01.transfer-create (at "sender" (chain-data)) MESH03_AWARD_BANK 
