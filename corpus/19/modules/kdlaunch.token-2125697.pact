(module token GOVERNANCE
  ...
  @doc " 'KDL' is the ecosystem token belonging to the first launchpad \
       \ on the Kadena mainnet, KDLaunch. "
  ...
  (bless "Xn8W0aNo18cfza3_X82lv19cmBQvn5RQ9GZNfnREaVs")
  ...
  (defcap MANAGER_GUARD
    ...
    (enforce-guard (at 'guard (read token-table KDL_MANAGER_ACCOUNT ['guard])))
  )
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
      (create-account rootAccountId rootGuard)
      (update token-table rootAccountId { "balance" : INITIAL_SUPPLY })
    )
  )

  (defun take-snapshot ()
    (with-capability (MANAGER_GUARD)
      (let ((allRows (keys token-table)))
        (map (details) allRows)
      )
    )
  )
