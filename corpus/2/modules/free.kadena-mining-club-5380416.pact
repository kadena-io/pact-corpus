...
; ============================================
    ...
    ; i need to make sure the account claiming owns all of the nft ids being passed in
    (defun claim (recipient:string amount:decimal nft-id:string)
    ...
    (defun get-kda-since-last-claim (nft-id:string)
        @doc "calculates the amount of kda that has been mined since the last claim for one nft"
        (with-read mledger nft-id 
          { 'kda-mined-index := kda-mined-index 
          , 'staked := staked}
            (enforce (= true staked) (format "nft with id {} is not turned on" [nft-id]))
            (with-read kda-mined-table (int-to-str 10 kda-mined-index) 
    ...
    (defun cap-get-kda-since-last-claim (nft-id:string)
        @doc "calculates the amount of kda that has been mined since the last claim for one nft"
        (with-read mledger nft-id 
          { 'kda-mined-index := kda-mined-index 
          , 'staked := staked
    ...
    (defun compiled-no-cap (nft-ids:list)
        (let 
            (
                (earned-kda-list (map (get-kda-since-last-claim) nft-ids))
            )
            (sum-list earned-kda-list)
        )
    )    

    (defun compiled-cap (nft-ids:list)
        (let 
            (
                (earned-kda-list (map (cap-get-kda-since-last-claim) nft-ids))
            )
            (sum-list earned-kda-list)
        )
    )
    
    (defun get-total-earned-kda (nft-ids:list)
        @doc "returns the sum of kda earned for all nfts passed in"
        (let 
            (
                (earned-kda-list (get-list-of-earned-kda nft-ids))
            )
            (sum-list earned-kda-list)
        )
    )

    (defun sum-list (values:[decimal])
        @doc "sums all decimal values in a list"
        (fold (+) 0.0 values)
    )

    (defun get-list-of-earned-kda:list (nft-ids:list)
        @doc "returns a list of earned kda for each nft passed in"
        (map 
            (get-kda-since-last-claim)
            nft-ids
        ) 
    )
    ...
    (defun update-kda-mined-admin (kda-mined:decimal)
        @doc "updates the total kda mined"
        (with-capability (ADMIN)
            (update kda-mined-table (int-to-str 10 (get-count KDA_MINED_UPDATE_COUNT))
                { "kda-mined": kda-mined
                , "datetime": (at "block-time" (chain-data))}
            )
        )
...
; ============================================
    ...
    (defun update-kda-mined (kda-mined:decimal account:string)
        @doc "updates the total kda mined"
        (with-capability (ACCOUNT_GUARD account)
            (with-capability (DISCORD account)
                (insert kda-mined-table (int-to-str 10 (get-count KDA_MINED_UPDATE_COUNT))
                    { "kda-mined": kda-mined
                    , "datetime": (at "block-time" (chain-data))}
                )
