...
; ============================================
    ...
    (defun create-nft-to-mint (account:string)
        ...
        (insert mledger (int-to-str 10 (get-count MINERS_CREATED_COUNT))
            ...
            , "special-attributes": "1 year extra warranty"
            , "staked": false
            , "staked-unstaked": []
            , "for-sale": false 
            , "market-price": 0.0})
    ...
    (defun accept-kwUSDC (account:string amount:decimal)
        (lago.kwUSDC.transfer account MINT_WALLET amount)
    )
...
; ============================================
    ...
    (defun add-to-wl (role:string calling-account:string account:string )
        ...
        (let 
            ...
            (if (= role-exists true) (update wl account {"role": role}) (insert wl account {"role": role}))
        )
    ) 

    (defun manual-add-to-wl-for-role (role:string accounts:list)
        @doc "Adds wl users with a role"
        (enforce
            (contains role [WL_MINERLIST_ROLE, WL_KD2_ROLE, WL_KD5_ROLE, WL_KD6_ROLE, WL_KD7_ROLE, WL_KD8_ROLE])
              "Must specify a valid role for adding WL members"
        ...
        (with-capability (ADMIN)
            (map (manual-add-to-wl role) accounts)
        )
    )   

    (defun manual-add-to-wl (role:string account:string)
