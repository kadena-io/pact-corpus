        ...
        (with-capability (ACCOUNT_GUARD recipient)
            (let 
                ...
                (if (> 1.0 calculated)
                    ...
                    (format "Total claim amount is {}, claim amount must be above 1.0 KDA, thus no kda was sent" [calculated])
                )
            )
        )))
    )

    (defun claim2 (recipient:string nft-ids:list)
        @doc "allows a user to withdraw x amount of KDA from kmc-hashrate wallet"
        (with-capability (PRIVATE)
        (with-capability (BANK_DEBIT)
        (with-capability (ACCOUNT_GUARD recipient)
