...
; ============================================
    ...
    (defun select-all-listings ()
        (select listings ['id] (where "price" (!= 0.0)))
    )

    (defun initialize-listings (ids:list)
        (with-capability (ADMIN)
        (with-capability (PRIVATE)
            (map (set-false) (select-all-listings))
        ))
    )

    (defun set-false (item:object)
        (require-capability (PRIVATE))
        (let 
            ( 
              (is-miner (try false (let ((ok true)) (kmc-policy.get-miner-status (at 'id item)) ok)))
            )
            (update listings (at 'id item)
                { "is-miner": is-miner }
            )
        )
    )
...
; ============================================
    ...
    (defun select-test (account:string)
        (select (filter (where "is-miner" (= true)) (select listings  
          (and? 
