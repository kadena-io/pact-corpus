(module USD2 GOVERNANCE
    ...
    (defconst SINGLETON-KEY "single-row")
    ; --------------------------------------------------------------------------
    ; Schemas and Tables
    ...
    (defschema guard g:guard)
    (deftable token-table:{token-schema})
    (deftable singleton-guard:{guard})
    ...
    (defun mint-create:string 
      ...
      @doc " Mint specified amount of tokens. Create account for receiver if it does not exist."
      ...
      @model [ (property (conserves-mass amount))
        ...
        (let ((obj (read singleton-guard SINGLETON-KEY ['g ])))
        (enforce-guard (at 'g obj))
    ...
    (defun init (g:guard)
    ;; You can only init if you're admin
    (enforce-guard (keyset-ref-guard 'lago-ns-user ))
    (write singleton-guard SINGLETON-KEY { 'g : g })
    )
