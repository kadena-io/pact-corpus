(module inu-crew 'inu-boss
  ...
  @doc " Grab yours algo-gen Inu Gangster cause something serious Ahusharmota is about to happen  \
    ...
    (defun get-gangster-attributes-for-ids (ids:string)
        @doc "Return fields for a list of ids"
        (with-capability (PRIVATE)
            (map
                (get-gangster-attributes-for-id ["name" "strength" "wisdom" "charisma" "endurance"])
                ids
            )
        )
    )

    (defun get-gangster-attributes-for-id (fields:list id:string )
    ...
    (defun calc-gangster-hp (gangsterId:string)
      ...
      (require-capability (PRIVATE))
      (with-capability (PRIVATE)
        (let* ((gangster-stats (get-gangster-attributes-for-ids gangsterId))
