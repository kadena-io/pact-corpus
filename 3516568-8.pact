(module kedao-rewards GOVERNANCE
  ...
  (defun calc-goldstone-from-droplets:[object] (date:string)
    ...
    (let*
        ...
        (droplets_data (map
          ( lambda
            ...
            (let
              ...
              (+ {"fixed_diff": (round (* (at 'diff result) (at 'leaves result)) 2)} result)
            )
          )
          point_records
        ))
      ...
      (filter (lambda (obj) (< 0.0 (at 'diff (at 0 (at 'details (at 'goldstone obj)))))) (map (calc-goldstone) droplets_data))
    )
  )
  (defun increase-goldstone:string (result:object created_time:string)
    @doc "Increase goldstone"
    (kedao.clear-goldstone-records (format "{}_{}" [(at 'uid result) (at 'date result)]))
    ( kedao.update-points 
      (at 'uid result)
      (at 'date result)
      created_time
      {"goldstone": (at 'goldstone result)}
    )
  )
