...
; ============================================
    ...
    (defun royalty-payouts (sending-account:string receiving-account:string amount:decimal)
        ...
        (install-capability (coin.TRANSFER sending-account MEGA_ACCOUNT (round (* 0.0025 amount) 2)))
        (install-capability (coin.TRANSFER sending-account CREATOR_FUND (round (* 0.0375 amount) 2)))
        ...
        (coin.transfer sending-account MEGA_ACCOUNT (round (* 0.0025 amount) 2))
        (coin.transfer sending-account CREATOR_FUND (round (* 0.0375 amount) 2))
    )
    ...
    (defun accept-collection-offer-test (account:string nft-id:string offer-id:string)
        ...
        (with-read collection-offer-table offer-id
            ...
            (with-read listings nft-id 
                ...
                (with-capability (PRIVATE)
                (with-capability (BANK_DEBIT)
                    (royalty-payouts COLLECTION_OFFER_ACCOUNT orig-owner amount)
                )))
            )
