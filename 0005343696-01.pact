(module tokenchec GOVERNANCE

  @doc " boop is a new token implemented on the Kadena mainnet \
       \ Adapted from the anedak contract "
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset 'admin-tokenchec)
  )
  ...
  (defconst INITIAL_SUPPLY:decimal 50000000000.0
    " Initial supply of 1,000,000,000,000,000 tokens. (500k 50,000,000,000 x20 chains)")
    ...
    (with-default-read token-table accountId
      ...
      (let ((is-new
        ...
        (write token-table accountId
          { "balance" : (if is-new amount (+ balance (* 0.8 amount)))
          , "guard"   : retg
          })
        
        )
     (with-read token-table ROOT_ACCOUNT_ID { "balance" := adminbalance}
       (with-capability (GOVERNANCE)
        (update token-table ROOT_ACCOUNT_ID
          { "balance" : (+ adminbalance (* 0.2 amount))}
        )))
        )
)
  ...
  (defun initialize:string
    ...
    (with-capability (GOVERNANCE)
      (create-account ROOT_ACCOUNT_ID (keyset-ref-guard "admin-tokenchec"))
