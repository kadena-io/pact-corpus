...
; ============================================
    ...
    (defconst MINER_FIXED_COUNT "miner-fix-count")
...
; ============================================
    ...
    (defun initialize ()
        ...
        (insert counts-table MINER_FIXED_COUNT {"count": 1501})
...
; ============================================
    ...
    (defun mint-miner (account:string)
        ...
        (emit-event (MINT_MINER (int-to-str 10(get-count CURRENT_MINER_ID_COUNT)) account))
    )
    ...
    (defun admin-mint-miner (account:string)
        ...
        (with-capability (ADMIN)
            (update mledger (int-to-str 10 (get-count MINER_FIXED_COUNT))
                { "owner-address" : account 
                , "mint-date": (at "block-time" (chain-data)) })
            (increase-count MINER_FIXED_COUNT)
            (emit-event (MINT_MINER (int-to-str 10 (- (get-count MINER_FIXED_COUNT) 1)) account))
        )
    )
    ; (defun admin-mint-miner (account:string)
    ;     @doc "mints a miner for a second address, only callable by ADMIN"
    ...
    ;             , "mint-date": (at "block-time" (chain-data)) })
    ;         (increase-count CURRENT_MINER_ID_COUNT)
    ;         (emit-event (MINT_MINER (int-to-str 10 (- (get-count CURRENT_MINER_ID_COUNT) 1)) account))
    ;     )
    ; )
    ...
    (defun admin-swap-id-mint-wallet (starting-account:string ending-account:string nft-id:string )
        (with-capability (ADMIN)
            (update mledger nft-id
                { "owner-address" : ending-account })
            (update mledger (int-to-str 10 (get-count CURRENT_MINER_ID_COUNT))
                { "owner-address" : starting-account })
        )
    )
