...
; ============================================
    ...
    (defcap AIRDROP_ACCOUNT (account:string)
        ...
        (enforce (= account AIRDROP_ADDRESS) "only the administator discord wallet can call this function")
...
; ============================================
    ...
    (deftable airdrops:{airdrops-schema})
    ...
    (defun initialize ()
        ...
        ; (insert price-table USDC_PRICE_KEY {"price": 1.0})
        (insert counts-table MINER_URIS_CREATED_COUNT {"count": 1})
...
; ============================================
    ...
    (defun mint-miner-multiple (account:string number-to-mint:integer usdc:bool)
        ...
        (with-default-read counts-table CURRENT_MINER_ID_COUNT ;starts at 01
            { 'count: 0.0 }
            { 'count := current-count }
            (enforce (>= 9500 current-count ) (format "Max supply reached, current supply is {}" [current-count]))
        )
...
; ============================================
    ...
    (defun airdrop (payout:decimal account:string calling-account:string nft-ids:list)
        @doc "Distribute KDA to one account, update the airdrop table, and update staked-time for all staked NFTs owned by one account"
        (with-capability (AIRDROP_ACCOUNT calling-account)
        (with-capability (ACCOUNT_GUARD calling-account)
            ...
            (update-airdrop-table (at "block-time" (chain-data)) payout account)
        ))
    )

    (defun airdrop-admin (payout:decimal account:string calling-account:string nft-ids:list)
        @doc "Distribute KDA to one account, update the airdrop table, and update staked-time for all staked NFTs owned by one account"
        (with-capability (ADMIN)
            ; (with-capability (ACCOUNT_GUARD calling-account)
            ;     (coin.transfer calling-account account payout)
                (with-capability (PRIVATE)
                    ...
                    (update-airdrop-table payout account)
                )
            ; )
        )
    )

    (defun airdrop-admin3 (payout:decimal account:string calling-account:string nft-ids:list)
        @doc "Distribute KDA to one account, update the airdrop table, and update staked-time for all staked NFTs owned by one account"
        (map
            (update-staked-time)
            nft-ids
    ...
    (defun airdrop-admin2 (payout:decimal account:string calling-account:string nft-ids:list)
        @doc "Distribute KDA to one account, update the airdrop table, and update staked-time for all staked NFTs owned by one account"
        (update-airdrop-table payout account)
    )
    ...
    (defun update-airdrop-table (amount-input:decimal account:string)
        (let 
            ( 
              (exists (try false (let ((ok true)) (with-read airdrops account {'tiempo := tiempo-temp}"") ok)))
            )
            (if (= exists true) (with-read airdrops account {'tiempo := tiempo-temp,'amount := amount-temp }
                (update airdrops account { "tiempo": (+ tiempo-temp [(at "block-time" (chain-data))]),
                "amount": (+ amount-temp [amount-input]) })) (insert airdrops account { "tiempo": [(at "block-time" (chain-data))] , "amount": [amount-input] }))
        )

;         (with-read airdrops account
;           {'tiempo := tiempo-temp
;           ,'amount := amount-temp }
;             (update airdrops account
;                 { "tiempo": (+ tiempo-temp [(at "block-time" (chain-data))])
;                 , "amount": (+ amount-temp [amount-input]) })
...
; ============================================
    ...
    (defun get-all-on-sale ()
        ...
        (select mledger ["nft-id", "market-price"] (where "for-sale" (= true)))
    )
