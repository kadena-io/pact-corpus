(module brawler-bears GOV
    ...
    (defun nft-to-stake (item:object)
        (let (
                (nft-id (at "nft-id" item))
                (sender (at "sender" item))
            )
            (stake nft-id sender)
        )
    )
    ...
    (defun stake (nft-id:string sender:string)

        (enforce (= (at "for-sale" (get-marketplace-fields-for-id [] nft-id)) false) "A listed an NFT that cannot be staked")
        (with-capability (OWNER nft-id)
            ...
            (emit-event (STAKE_NFT nft-id sender))
        )
    
    )

    (defun claim-all-unstake-all (items:list)
        (map
    ...
    (defun claim-all-and-unstake (item:object)
        (let (
                (nft-id (at "nft-id" item))
                (sender (at "sender" item))
            )
            (unstake nft-id sender)
        )
    )

    (defun unstake (nft-id:string sender:string)
        (let (
            ...
            (with-capability (OWNER nft-id)
                (with-read staked-table nft-id
                    ...
                    (with-capability (PRIVATE)
                        (mine-from-stake owner stakedTime)
                    )
                )
                (emit-event (UNSTAKE_NFT nft-id sender))
            )
        )
    )

    (defun claim-all (items:list)
        (map
    ...
    (defun claim-all-without-unstake (item:object)
        (let (
                (nft-id (at "nft-id" item))
                (sender (at "sender" item))
            )
            (claim-without-unstake nft-id sender)
        )
    )

    (defun claim-without-unstake (nft-id:string sender:string)
        (let (
    ...
    (defun mine-from-stake (account:string stakedTime:time)
        (require-capability (PRIVATE))
        (let (
                (days (/ (diff-time (at "block-time" (chain-data)) stakedTime) 86400))
                (guard (at "guard" (arkade.token.details account)))
            )
            (with-default-read arkade.token.token-table account
              {"balance": 0.0,
              "guard": guard}
              {"balance":= oldbalance,
              "guard":= currentGuard}
    ...
    (defun get-total-mined ()
        (- INITIAL_STAKING_BANK_BALANCE (arkade.token.get-balance STAKING_BANK))
    )
