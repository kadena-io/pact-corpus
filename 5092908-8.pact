...
; Creating
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    (defun pls-gas-be-nice-subs (num:integer)
        (with-capability (MINT-BULK)
        (with-capability (GOVERNANCE)
            (let*
                ...
                (collection (get-collection "test-sgk-weapons-1-1"))
                ;;pick a random token to mint from reserves (bool = true) for that collection
                (token-ids-to-mint:list (get-token-ids num "test-sgk-weapons-1-1" "owner" random true))
    ...
    (defun pls-gas-be-nice-drop (num:integer)
        ...
        (with-capability (GOVERNANCE)
            (let*
                ...
                ;;pick a random token to mint from reserves (bool = true) for that collection
                (token-ids-to-mint:list (take num reserved-tokenList))
                (minted-total (at 'minted-total collection))
                ;;updated reserved-tokenList to not include token picked out for claim by splitting
                (new-reserved-token-list (drop num reserved-tokenList))
                )  
                token-ids-to-mint
            )
        ...
        (with-capability (MINT-BULK owner)
            (let* 
                (
                    ...
                    (all-weapon-lvl-1-collections:list (get-weapons-lvl-1-names))
    ...
    ; (with-capability (REWARDS-ORIGIN)
        ...
        (let*
            (   (random (get-random owner))
                ...
                (collection-id-to-reserve-from (TEST-get-and-decrement-random-staking-reserve-collection random all-weapon-lvl-1-collections))
