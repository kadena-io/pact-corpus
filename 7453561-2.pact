...
; ============================================
    ...
    (defun force-accept-nft-offer (nft-id:string new-address:string offer-id:string amount:decimal account:string)
    ...
    (defun select-offers-on-nfts-owned-by (account:string)
        ...
        (let 
            ...
            ; (format "{}" [owned-miners])
            (map (select-valid-offers-for-id-object) owned-miners)
        )
    )

    ; filter nft-offer-table (where 


    (defun read-keys-coll-offers ()
        (keys collection-offer-table)
    )
    (defun select-all-nft-offers ()
        (select nft-offer-table (where 'valid-until (!= (time "2022-01-01T12:00:00Z"))))
    )

    (defun select-all-collection-offers ()
        (select collection-offer-table (where 'offer-id (!= -1)))
    )

    (defun select-valid-offers-for-id-object (nft:object)
        (select nft-offer-table 
            (and? (where 'nft-id (= (at 'nft-id nft)))
    ...
    ; (defun get-kda-mined-all ()
    ;     (sort ['kda-mined] (select kda-mined-table (where 'kda-mined (!= -1.0))))
    ; )
...
;     (defun select-all-offers-for-account (account:string)
;         @doc "Selects all collection offers from one account"
...
;     (defun get-founders-with-remaining-mints ()
;         (select fledger (where "free-mints-remaining" (= 0.5)))
;     )
; ; ============================================
...
;     (defun get-all-airdrop-table-per-key (keysss:[string])
;         (map (get-one-airdrop-table-per-key) keysss)
;     )

;     (defun get-one-airdrop-table-per-key (key:string)
;         (let 
;             (
;                 (air (at "airdrop" (read airdrop-table key)))
;             )
;             {"key": key, "air": air}
;         )
;     )

;     (defun get-airdrop-table-keys ()
;         (keys airdrop-table)
;     )

;     (defun select-all-airdrop-table ()
;         (select airdrop-table (where 'airdrop (!= [0])))
;     )
