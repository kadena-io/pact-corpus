...
; --------------------------------------------------------------------------
  ...
  (defcap GALLINASIO_GENE (id:string account:string gene:integer parent:integer)
...
; --------------------------------------------------------------------------
  ...
  (defun set-special-promo (gid:string special:integer)
    @doc " Sets a Gallinas Special Gene For Promotions "
    (with-capability (GOVERNANCE)
    (update gallinas-special-table gid {"special":special})
    (update gallinas-generation-table gid {"generation":1})
  ...
  (defun get-gift-store-weeks ()
   @doc " Gets number of weeks since gift voucher store start "
    (let ((giftgenisis (time "2022-03-13T01:01:02Z") ))
      (round (/ (diff-time (at "block-time" (chain-data)) giftgenisis ) 604800))
    )
  )
  ...
  (defun get-current-gift:integer ()
    @doc " Gets the current Gallina store gift from voucher table "
    (let ((currenttime (at "block-time" (chain-data))))
      (let ((x (select gift-release-table (and? (where 'start (>= currenttime))
            (where 'end (< currenttime))))))
             (at "gift" (at 0 x)) )
    ...
    (with-read gledger (key id account)
      ...
      , 'guard := l_guard }
        (with-capability (ACCOUNT_GUARD id l_account)
          ...
          (with-default-read set-breed-table (setkey (get-current-gift))
           { "count" : 0 }
           { "count" := countset}
          (write set-breed-table (setkey (get-current-gift)) {"count": (+ countset 1)})
          )
          (update gallinas-special-table id {"special": (get-current-gift)} )
          (with-default-read set-breed-table (setkey 0)
           { "count" : 0 }
           { "count" := countset0}
          (write set-breed-table (setkey 0) {"count": (- countset0 1)})
          )
          (emit-event (GALLINASIO_GIFT id account (get-current-gift)))
          (format "Gifted the Gallina with ID {} with Gift ID {}" [id (get-current-gift)])
        )
    )
  )
  ...
  (defun remint
    ( id:string
      ...
      (with-capability (ACCOUNT_GUARD id account)
        ...
        (insert gallinas-nextbreed-table id {"nextbreed" : (at "block-time" (chain-data)) })
              ...
              (let ((newid (create-random-id (+ gid1 gid2) account )))
                ...
                (with-read total-gallinas-table ""
                            ...
                            (with-default-read set-breed-table (setkey newspecial)
                             ...
                             { "count" := countset}
                              (with-default-read set-breed-table (setkey 0)
                                 { "count" : 0 }
                                 { "count" := countset0}
                                 (if (< countset (* newspecial MAX_SUPPLY) ) (insert gallinas-special-table newid {"special" : newspecial }) (insert gallinas-special-table newid {"special" : 0 }))
                                 (if (< countset (* newspecial MAX_SUPPLY) ) (write set-breed-table (setkey newspecial) {"count": (+ countset 1)}) (write set-breed-table (setkey 0) {"count": (+ countset0 1)}) )
                              )
                            )
                  ...
                  (if (= gene 0) true (with-read gift-voucher-table account
                    ...
                    (write gift-voucher-table account
                    { "balance" : (- gvbalance 1.0) })
                    (emit-event (GALLINASIO_GENE newid account gene parent))
                  ))
...
;(free.collect-gallinas.set-new-release-schedule (time "2022-04-13T00:00:02Z") (time "2022-04-20T00:00:01Z") 8 "1")
