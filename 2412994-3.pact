(module babena GOVERNANCE
  ...
  (defcap ACCOUNT_GUARD ( accountId:string )
  ...
  (defconst LP_PROVIDERS:string "babena-placeholder"
      " Placeholder until updated to liquidity pool account. ")

  (defconst PREMINE_ACCOUNT:string "babena-premine"
      " Temorary holder of the Initial Liquidity premine. ")
  ...
  (defun initialize ()
    ...
    (coin.create-account LP_PROVIDERS (create-module-guard "babena-placeholder"))
    (create-account LP_PROVIDERS (create-module-guard "babena-placeholder"))
    ...
    (write supply-table "" {"supply": 25000.0 })
    ...
    (create-account PREMINE_ACCOUNT (create-module-guard "premine-babena"))
    (update token-table PREMINE_ACCOUNT { "balance" : 25000.0 }))
  ...
  (defun lock-babe
    ...
    (with-default-read last-id-table ""
      ...
      (with-default-read supply-table ""
        ...
        (enforce (>= amount 10) "Minimum lock is 10 BABE" )
  ...
  (defun unlock-babe
    ...
    (with-read lock-babe-table (format "{}" [id])
      ...
      (write lock-babe-table (format "{}" [id])
        ...
        , "withdrawn"  : true })
      ;Send BABE
      (transfer BABE_BANK account amount)
      (transfer BABE_BANK account rewards)
      (format "Unlocked id {} for {} BABE." [id,(+ amount rewards)])))

  (defun emergencyunlock-babe
    ( id:integer
      account:string
      guard:guard )
