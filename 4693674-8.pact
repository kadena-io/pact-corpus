    ...
    (defun immerse (account:string token:object)
        ...
        (let*
            (
                ...
                (vial-number (at 'number (kmc-vial-policy.get-token vial-id)))
                ...
                ;                 )
                ;                 (at 'token-id (kmc-policy.get-token-nft-id2 (int-to-str 10 (+ 10000 (str-to-int miner-number))))) )
                ; )
                ...
                ; (new-uri (at 'assetUri (at 'datum (at 0 (at 'data (at 'manifest (at 'token (marmalade.ledger.get-policy-info miner-to-upgrade))))))))
            )
            "hello"
        ...
        ;     (install-capability (marmalade.ledger.BURN vial-id account 1.0))
        ;     (marmalade.ledger.burn vial-id account 1.0)
        ;     (with-capability (CALL-POLICY-MODULES) (update-burned miner-id) (update-burned miner-to-upgrade))
        ;     (if (= is-legendary-miner false)
        ;         (with-capability (PRIVATE)
        ;             (install-capability (marmalade.ledger.BURN miner-id account 1.0))
        ...
        ;         , "updated-at" : (at 'updated-at old-miner-info)
