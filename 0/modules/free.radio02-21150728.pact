(module radio02 GOVERNANCE
  ...
  (defun get-stake-menu ()
    "Get all stakeMenu"
    (select stakeMenu1 (constantly true))
  )
  ...
  (deftable staking2:{staking})

  (defun stake (code gatewayId)
    "Stake"
    (with-read stakeMenu1 code 
      {"valueInCRKK":= value,
       "stakeWait":= stakeWait}
        (insert staking2 (compound-key2 (at "sender" (chain-data)) code)
  ...
  (defun get-stakings ()
    "Get all stakings"
    (select staking2 (constantly true))
  )
