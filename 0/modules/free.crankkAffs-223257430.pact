(module crankkAffs GOVERNANCE
  ...
  (defun update-affiliate-address (affCode oldAddress newAddress)
  (with-read radmin "admin"
    { "aguard" := aguard }
    (enforce-guard aguard)
    (with-read codes1 affCode
      {"address":= address}
      (enforce (= address oldAddress) "Old address mismatch")
      (update codes1 affCode {"address": newAddress})    
      (with-read affiliates4 (compound-key2 oldAddress (take 1 affCode))
        {"address":= address}
        (enforce (= address oldAddress) "Old address mismatch")
        (update affiliates4 (compound-key2 oldAddress (take 1 affCode)) 
            {"address": newAddress}
        )    
      )
    )
