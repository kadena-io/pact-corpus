(module radio02 GOVERNANCE
  ...
  (defun get-my-node-with-check ()
    "Get my node with check"
  ...
  (defun update-sent-with-check (mic:string)
    "Update sent"
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (update nodes7 (at "sender" (chain-data))
        {"sent": mic,
...
;     hashedPw: string
  ...
  (deftable licenses3:{license})

  (defun buy-license (affCode aguard:guard)
      "Buy license"
      ...
      (with-read affiliates1 affCode
        ...
        ;   (coin.transfer (at "sender" (chain-data)) affAddress 0.01)
          (insert licenses3 (at "sender" (chain-data)) 
            {"address": (at "sender" (chain-data)),
            "aguard": aguard,
            "usedForGw": "",
            "used": false,
  ...
  (defun activate-license (gatewayId:string)
      "Activate license"
      (with-read licenses3 (at "sender" (chain-data))
        {"used":= used,
         "refunded":= refunded}
          ...
          (enforce (= refunded false) "Already refunded")
            ...
            (update licenses3 (at "sender" (chain-data)) 
              {"used": true,
               "usedAt": (get-time)}
            )
      )
  ...
  (defun reverse-license ()
      ...
      (with-read licenses3 (at "sender" (chain-data))
        {"aguard" := aguard,
         ...
         "used":= used,
         "refunded":= refunded}
;         (enforce-guard aguard) ;Doesn't want to work
          (enforce (= used false) "Already used")
          (enforce (= refunded false) "Already refunded")
  ...
  (defun get-licenses () 
    (select licenses3 (constantly true)) 
  )
