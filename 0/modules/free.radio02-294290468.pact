(module radio02 GOVERNANCE
  ...
  (deftable gatewayLocSet1:{gatewayLocSet})
  ...
  (defun get-gatewayLocSet ()
    ...
    (select gatewayLocSet1 (constantly true))
  )
  ...
  (defun update-gateway-locSet (gatewayId:string set:bool)
    (enforce-guard (at 'guard (read nodes7 (at "sender" (chain-data)) ['guard])))
    (with-read nodeExtras9 (at "sender" (chain-data))
        {"address":= sender, "oracle":= oracle}
    (enforce (= oracle true) "Must be an oracle node")
    (with-default-read gatewayLocSet gatewayId 
      ...
      {"gatewayId":= gatewayIdDb}
      (if (= gatewayIdDb "")
      (insert gatewayLocSet1 gatewayId 
         {"gatewayId": gatewayId, "set": set, "lastUpdated": (get-time)}
      )
      (update gatewayGPSs1 gatewayId
         {"gatewayId": gatewayId, "set": set, "lastUpdated": (get-time)}
      )
