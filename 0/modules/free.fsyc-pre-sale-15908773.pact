(module fsyc-pre-sale GOVERNANCE
  ...
  (defconst SALE_START_TIME:time (time "2022-11-01T00:00:00Z"))
  (defconst WHITELIST_TIME:time (time "2022-09-01T00:00:00Z"))
  (defconst END_TIME:time (time "2023-12-02T00:00:00Z"))
  ...
  (defun enforce-whitelist (account:string)
    (let ( (accounts:[string] (at 'accounts (read whitelists ""))))
      (enforce (contains account accounts) (format "You are not whitelisted: {}" [whitelists]))
      (enforce-guard (at-after-date WHITELIST_TIME))
