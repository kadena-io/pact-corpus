...
; ============================================
    ...
    (defun mint (account:string amount:integer)
        ...
        (let*
            (
                ...
                (tiempo (get-time))
                (isWhitelistTime (if (and (> tiempo (time "2023-06-19T12:00:00Z")) 
                                (< tiempo (add-time (time "2023-06-19T12:00:00Z") 86400)))
                                true false))
                (price (get-current-nft-price isWhitelistTime))
            )
            (enforce (> tiempo (time "2023-06-14T12:00:00Z")) "Mint has not started yet, please do not submit your transaction")
            (coin.transfer account ADMIN_ADDRESS (* price amount))
            (if (= true isWhitelistTime)
                (with-capability (PRIVATE)
                    (enforce (<= amount mints-remaining) (format "Cannot mint more than your allocated whitelist mints. \
                        \ You have {} remaining. Come back during the public sale in < 24 hours if you want to mint more than that" [mints-remaining]))
    ...
    (defun marmalade-mint (account-guard:guard account:string)
        ...
        (let 
            (
                (token-id (kmc-test2-policy.get-token-nft-id (int-to-str 10 (+ (kmc-test2-policy.get-count "mint-count") 1))))
            )
...
; ============================================
    ...
    (defun reg-vial-policy ()
        (with-capability (ADMIN)
            (kmc-test2-policy.register-guard-immersion (create-capability-guard (CALL-POLICY-MODULES)))
        )
    )
...
; ============================================
    ...
    (defcap OWNER (account:string id:string)
        ...
        (let
            (
                (nft-owner (kmc-test2-policy.get-owner id))
            )
