...
; --------------------------------------------------------------------------
  ...
  (defun get-gift:integer ()
    @doc " Gets the current Gallina store gift from gift table "
    ...
    (with-read gledger (key id account)
      ...
      , 'guard := l_guard }
        (with-capability (ACCOUNT_GUARD id l_account)
          ...
          (with-default-read set-breed-table (setkey (get-gift))
           { "count" : 0 }
           { "count" := countset}
          (write set-breed-table (setkey (get-gift)) {"count": (+ countset 1)})
          )
          (update gallinas-special-table id {"special": (get-gift)} )
          (emit-event (GALLINASIO_GIFT id account (get-gift)))
