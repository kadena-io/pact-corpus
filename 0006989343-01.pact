(module brawler-bears GOV
    ...
    (defun reward-from-stake (account:string stakedTime:time)
        (require-capability (PRIVATE))
          ...
          (let
              (
                (reward (ceiling (* (/ (diff-time (at "block-time" (chain-data)) stakedTime) 86400) 200) 2))
            )
    ...
    (defun get-total-unclaimed-amount ()
        (- FUNDED_STAKING_BANK_BALANCE (arkade.token.get-balance STAKING_BANK))
    )
