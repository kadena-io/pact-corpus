(module test-lottery-three-thousand 'admin-multi-keyset
  (defcap IS_ADMIN ()
  ...
  (defun add-participant (account:string tickets:integer project:string)
    ;(enforce-guard (at 'guard (hypercent.prod-hype-coin.details account)))
    (let* (
   ...
   (with-capability (IS_ADMIN)
        (with-read projects-table project {"winners":=winners}
          (let ((participant_key (get-participant-key account project)))
              (hypercent.prod-community.admin-claim-reward account (+ project "_GA"))
              (write participants-table participant_key
                {"project": project,
                  "tickets": 1,
                  "win":true,
  ...
  (defun pre-process-participants-local (batch_length:integer project:string winners_per_round:integer)
        ...
        (with-read projects-table project
          ...
          (let* (
                 ...
                 (winners_with_kyc (filter (hypercent.prod-community.has-kyc) total_winners))
            ...
            (update projects-table project
              {
                ...
                "participants_processed": (+ participants_processed (length pre_winners_batch))
              }
            )
          )
        )
