(module kadcars-nft-policy GOVERNANCE
  ...
  (defconst CHAMPION_MINT_TIME "2023-01-10T03:00:00Z")
  (defconst PUBLIC_MINT_TIME "2023-01-10T03:00:00Z")
  ...
  (defun enforce-mint:bool
    ...
    (enforce-ledger)
      (let* (
              ...
              (whitelist-info (get-whitelist-info account))
              (mint-price-multiplier (get-mint-multiplier whitelist-info))
              (base-mint-price (get-mint-price-by-threshold collection-id))
              (mint-price (* base-mint-price mint-price-multiplier))
              (token-id (at 'id token))
              (wl-live:bool (enforce-mint-live whitelist-info))
...
;;    (update-owner (at 'token-id token) buyer)
  )
  ...
  (defun create-collection ()
    ...
    ;;;;;;;;;;;;;; WL Functionality ;;;;;;;;;;;;;;
    (defun get-account-wl-info:object{account-wl-info-schema} (account:string)
      (read account-wl-info account)
    )
    ...
    (defun add-whitelist-wrapper(wl:object{account-wl-info-schema})
      ...
      (let*
          (
            ...
            (add-whitelist account free-mints-remaining whitelists-remaining is-champion minted-total)
        )

    )

    (defun add-whitelist:bool (account:string free-mints-remaining:integer whitelists-remaining:integer is-champ:bool minted-total:integer)
       (with-capability (GOVERNANCE)
    ...
    (defun get-whitelist-info:string (account:string)
       (let ( (accounts:[string] (keys account-wl-info))
            )
;;         (if (= false PUBLIC_SALE) (enforce (contains account accounts) "You are not whitelisted") (enforce (= true true)))
         (require-capability (UPDATE_WL))
           (with-default-read account-wl-info account
             ...
             (cond
               ...
               ((and (= minted-total 0) is-champion) (update-champion-minted-total-table account (+ 1 minted-total)))
    ...
    (defun get-mint-price-by-threshold:decimal (collection-id:string)
      (let*
        (
          (minted-tokens (get-minted-tokens-for-collection collection-id))
          (supply (length minted-tokens))
          (base-price (get-base-price-by-supply supply))
          )
          base-price
      )
    )
    ...
    (defun get-mint-multiplier:decimal (whitelist-info:string)
      ...
      (cond
        ...
        ((= whitelist-info CHAMPION_MINT_ROLE) 0.75)
    ...
    (defun get-whitelist:object{account-wl-info-schema} (account-id:string)
      (read account-wl-info account-id)
    )
    ;;;;;;;;;;;;;; ROYALTY ;;;;;;;;;;;;;;
    ...
    (defun enforce-mint-live(whitelist-info:string)
      ...
      (let* (
        ...
        (cond
          ...
          ((= CHAMPION_MINT_ROLE whitelist-info) (enforce-champ-mint-live))
    ...
    ;; WEIRD ASS PACT error for DB exception so using a wrapper
    (defun get-kill:bool()
      (let* (
    ...
    (defun enforce-public-mint-live:bool ()
      @doc "Enforces mint is live"
      (enforce (>= (at "block-time" (chain-data)) (time PUBLIC_MINT_TIME)) "Public Mint is NOT Live. chil...")
      true
    )
