...
(defun heal (nft-id:string potion:string attacks:list m:module{brawler-bears-interface-v03})
  ...
  (with-capability (OWNER nft-id)
    ...
    (defun get-top-bears ()
      @doc "Retrieves the bears with the highest level and most damage given."
      (let* 
          (
            (all-bears (map (read nfts) (keys nfts)))
            (sorted-by-level (sort all-bears "level" >))
            (sorted-by-damage (sort all-bears "total-damage-given" >))
          )
          {
            "highest-level-bears": (take 10 sorted-by-level),
            "most-damage-bears": (take 10 sorted-by-damage)
          }
      )
    )
