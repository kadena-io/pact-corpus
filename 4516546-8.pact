...
; ============================================
    ...
    (defun get-all-vials-for-sale ()
        (map (get-vial-details-with-object) (kmc-test-policy.get-all-for-sale))
    )
    ...
    (defun get-vial-details (nft-id:string)
        ...
        (with-default-read marketplace-table nft-id
          ...
          , "price" := price} 
        {
          "old-nft-id" : (int-to-str 10 (at 'number (kmc-test-policy.get-token nft-id)))
        , "nft-id" : nft-id
        , "owner-address" : (kmc-test-policy.get-owner nft-id)
        , "market-price" : price
        , "price" : price
        , "for-sale" : (at 'for-sale (kmc-test-policy.get-status nft-id))
        , "original-owner": original-owner }
        )
    )
