(module kor-payout GOV
    ...
    (with-capability (OPS)
      (let 
        (
          ...
          (management-fee:decimal (get-decimal-value MANAGEMENT_FEE_KEY) total-percent)
        )

        ; Calculate all necessary values to send appropriate payouts
        (let*
          (
            (management-account-guard:guard (at "guard" (coin.details management-account)))
  ...
  (defun handle-th-payout:object
    (
      ...
      management-account-guard:guard
      kda-per-th-per-day:decimal
        ...
        (pay 
          (lambda (info:object{payout-info})
            (let*
              (
                ...
                (management-info:object
                  (pay-account 
                    ...
                    management-account
                    management-account-guard
                    (* total-amount (- management-fee total-percent))
                  )  
                )
              )
              ...
              , "management-amount": (at "amount" management-info)
              }
            )
          )
        )
  ...
  (defun withdraw-remaining:object ()
    @doc "Withdraws remaining money to the remaining account"
    (with-capability (OPS)
      (let
        (
          (contract-account:string (get-managed-account))
