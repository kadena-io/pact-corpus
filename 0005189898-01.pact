(module eighties-bulls 'bulls-keyset
  "80s Bulls"
    ...
    (defun transfer:string (sender:string receiver:string id:string)
    ...
    (defun put-id-for-sale(id:string account:string price:decimal)
        @doc "Puts an NFT up for sale"
        (with-capability (OWNER account id)
            (enforce (> price 0.0) "Price must be positive")
            (let* 
                (
                    (owner (at "owner" (get-bull-fields-for-id ["owner"] id )))
                )
    ...
    (defun remove-id-from-sale (id:string account:string)
    ...
    (defun buy-id-on-sale (id:string curr-user:string)
        ...
        (with-capability (ACCOUNT_GUARD curr-user)
            ...
            (let* 
                (
                    (nft-data (get-bull-fields-for-id [] id ))
    ...
    (defun list-my-bull-on-market(id:string owner:string price:decimal for_sale:string)
        ...
        (with-read minted-nfts id
          ...
          (with-capability (ACCOUNT_GUARD l_owner)
            ...
            (emit-event (BULL_LIST id owner price for_sale))
    ...
    (defun buy-bull-off-market
        ...
        (with-read minted-nfts id
          ...
          'price := m_price }
            ...
            (enforce (!= buyer seller) "You cannot buy your own Bull.")
            ...
            (emit-event (BULL_BUY id buyer seller price))
            (format "Purchased a Bull with the ID {} for {} KDA " [id price])
        )
    )
    ...
    (defun enforce-id-on-sale (id:string)
        ...
        (let* 
            (
                ...
                (current-owner (at "owner" (get-bull-fields-for-id ["owner"] id)))
    ...
    (defun ids-owned-by (owner:string)
        ...
        (select minted-nfts ["id", "price"] (where "owner" (= owner)))
    )
