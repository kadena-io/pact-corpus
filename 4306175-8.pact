(module secret-garden-of-kadena-policy GOVERNANCE
  ...
  ; Constants
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (defconst OG_MINT_START_TIME "2023-04-22T16:00:00Z")
  (defconst OG_MINT_END_TIME "2023-04-23T15:59:59Z")
    ...
    (with-capability (MINT (at 'id token) account)
      ...
      (let*
          (
           ...
           (if (= current-phase PRIVATE_WL_PHASE) (deduct-wl-and-return-true account-wl-info) true)
  ...
  (defun get-current-phase()
    (let*
        (
            ...
            (isPrivateStarted (check-is-mint-live-for-phase PRIVATE_WL_MINT_START_TIME))
            (isOgStarted (check-is-mint-live-for-phase OG_MINT_START_TIME))
        )
        (cond
            ((= true isPublicStarted) PUBLIC_PHASE)
            ((= true isPrivateStarted) PRIVATE_WL_PHASE)
            ((= true isPrivateStarted) OG_PHASE)
            "Mint is not Live")
    )
  )
