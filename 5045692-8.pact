    ...
    (defun get-asic-details (nft-id:string)
        ...
        (with-default-read marketplace-table nft-id
          ...
          (let*  (
                    ...
                    (electric-rate (at 'electric-rate asic)) ; 0.078
                    (wattage (str-to-int (at 'Wattage (at 'asic-details asic)))) ;3050
                    (kda-price (kmc-oracle.get-price "kda-price-key")) ; 0.51
                    ;electric expiry ; (time "2024-03-21T11:59:59Z")
                    ;block time        (time "2023-10-30T8:18:18Z")
                    (contract-days-remaining (round (/ (diff-time (at 'electric-expiry asic) (at "block-time" (chain-data))) 86400.0) 1))
                    ;142.7 * 0.078 * 24 * 3050
                    (contract-cost-in-kda (/ (* (* contract-days-remaining (* electric-rate 24)) (/ wattage 1000.0)) kda-price))
                )
            {
