(module kdswap-token-locker GOVERNANCE
  ...
  (defun claim-tokens (lockId:string)
      ...
      (with-read locks-table lockId
        ...
        (with-capability (ACCT_GUARD benificiaryAccount)
          (let*
            ...
            ; Transfer amount
            (install-capability (token::TRANSFER (get-locking-account-principal (format "{}" [token])) benificiaryAccount claimableAmount))
            (with-capability (LOCKING_RESERVE (format "{}" [token]))
              (token::transfer-create (get-locking-account-principal (format "{}" [token])) benificiaryAccount (at 'guard (coin.details benificiaryAccount)) claimableAmount)
            )

            (update locks-table lockId { "claimedAmount" : (+ claimedAmount claimableAmount) })

            (reduce-lock claimableAmount (format "{}" [token]))

            ; Update lock status if all is claimed
            (if (= (+ claimedAmount claimableAmount) lockAmount)
