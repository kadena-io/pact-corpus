...
; {"claim": [{"amount": "0.1","tiempo": "2023-04-03T08:08:32Z"}]}]
    (defun get-airdrop-details-select-format (account:string)
        { "airdrop": (map (combine-claims) (get-airdrop-details-select account)) }
    )
    ...
    (defun combine-lists1 (item:object account:string)
         (let 
            (
                (airdrop (at 'airdrop (read airdrop-table account)))
                (combined-claims (get-airdrop-details-select-format account))
    ...
    (defun combine-lists2 (item:object account:string)
         (let 
            (
                (airdrop (at 'airdrop (read airdrop-table account)))
                (combined-claims (get-airdrop-details-select-format account))
            )
            (format "{}" [combined-claims])
        )
    )
