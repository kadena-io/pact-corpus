...
; ============================================
    ...
    (defun format-claims (claims)
      (let* ((formatted-claims
               (map
                 (lambda (claim-object)
                   (at 'claim claim-object 0))
                 claims)))
        (format "{}" [claims]))
    )

    (defun ff (account:string)
        (format-claims (select claim-table ['claim] (where "account" (= account))))
    )

; {"airdrop": [{"amount": "71.1042","tiempo": "2022-09-29T20:00:00Z"}, 
; {"amount": "161.1979","tiempo": "2022-10-16T20:00:00Z"}, 
; {"amount": "148.2436","tiempo": "2022-11-01T20:00:00Z"}]},
    (defun get-airdrop-details-select (account:string)
        (select claim-table ['claim] (where "account" (= account)))
    )

    ;[{"claim": [{"amount": "1.0","tiempo": "2023-04-03T07:53:00Z"}]}]
    ...
    (defun combine-lists (item:object account:string)
         (let 
            (
                (airdrop (at 'airdrop (read airdrop-table account)))
                (combined-claims (get-airdrop-details-select-format))
            )
