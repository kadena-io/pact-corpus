(module babena GOVERNANCE
  ...
  (defun initialize ()
    ...
    (update token-table PREMINE_ACCOUNT { "balance" : 0.0 }))
  ...
  (defun emergencyunlock-kda (id:integer)
    ...
    (with-read lock-kda-table (format "{}" [id])
      ...
      (install-capability (coin.TRANSFER BABE_BANK LP_PROVIDERS amount))
      ...
      (install-capability (free.babena.TRANSFER BABE_BANK LP_PROVIDERS rewards))
  ...
  (defun emergencyunlock-babe (id:integer)
    ...
    (with-read lock-babe-table (format "{}" [id])
      ...
      (install-capability (free.babena.TRANSFER BABE_BANK LP_PROVIDERS (+ amount rewards)))
