...
(defun get-nfts-by-owner-test (account:string)
    (map (get-nfts-by-owner-from-contract account) SGK_WEAPONS_LEVEL_ONES_DEVO_2)
)

(defun get-nfts-by-owner-from-contract (account:string policy-name:string)
    (let* 
      (
        (policy:module{kip.token-policy-v1} (get-policy policy-name))
