(module eighties-bulls GOV
  "80s Bulls"
    ...
    (defconst OFFERS_BANK:string "eighties-bulls-offers-bank")
    (defconst URI_KEY "uri-key")
    (defconst IMAGES_URI "https://arkade-prod.s3.us-east-1.amazonaws.com/arkade-80s-bulls")
    ...
    (defun buy-id-on-sale (id:string curr-user:string)
        ...
        (with-capability (ACCOUNT_GUARD curr-user)
            ...
            (let* 
                (
                    (nft-data (get-nft-fields-for-id ["owner", "id", "item"] id ))
    ...
    (defun make-offer (id:string buyer:string duration:integer amount:decimal)
        ...
        (let (
            (original-owner (at "owner" (read minted-nfts id ["owner"])))
        ...
        (with-read offers id
        {
            ...
            (with-capability (OWNER nft-id)
                ...
                (let* 
                    ...
                    (update minted-nfts nft-id (+ {"owner": buyer} nft-data ))
                    (with-capability (PRIVATE)
