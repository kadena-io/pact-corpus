(module KAO GOVERNANCE
    ...
    (defun createCandidate (name:string userId:string)
      ...
      (let ((id (int-to-str 10 (+ (get-count) 1))))
        (insert candidates id { "id": id, "name": name, "votes": 0, "userId": userId })  )
      
    )
  
    (defun getCandidate (id:string)
      "Get candidate by id"
      (read candidates id ['id 'name 'votes 'userId])
    )

    (defun getCandidatesByUserId (userId:string)
      @doc "Get candidates by userId"
      (select candidates ["id", "name", "votes", "userId"] (where "userId" (= userId)))
    )
  
    (defun getAllCandidates ()
      "Get all candidates"
      (select candidates ["id", "name", "votes", "userId"] (where "votes" (>= 0)))
    )
