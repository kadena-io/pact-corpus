(module vesting 'admin-multi-keyset
  ...
  (defconst HYPE_BANK:string 'hype-bank-1)
  ...
  (defconst MIN_HYPE_ALLOCATION 0.00000001)
  (defconst HYPE_PROJECT_NAME "hype")
  (defconst HYPE_START_DATE (time "1969-01-01T00:00:00Z"))
  (defconst HYPE_END_DATE (time "2069-01-01T00:00:00Z"))
  ...
  (defun get-max-allocation-amount()
    (let* (
        (allocated (fold (+) 0.0 (map (at "hype-amount") (select allocations-table ["hype-amount"] (where "hype-amount" (> 0.0))))))
      )
  ...
  (defun create-allocation (kda-amount:decimal account:string)
    (let* (
        (hype-amount (* kda-amount (get-swap-rate)))
      ...
      (with-capability (CAN_VEST) (add-account-vesting-entries account hype-amount "IDO"))
    )
  )
