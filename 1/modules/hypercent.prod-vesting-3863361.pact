(module prod-vesting 'hyper-admin-multi-keyset-prod
  ...
  (defconst HYPE_BANK:string 'hype-bank-prod)
  (defun hype-bank-guard () (keyset-ref-guard "hyper-admin-multi-keyset-prod" ))
  ...
  (defconst VESTING_START_DATE:time (time "2022-28-03T00:00:00Z"))
  ...
  (defconst DATE_FIXES {
    "2022-12-03T00:00:00Z": "2022-12-04T00:00:00Z",
    "2023-02-03T00:00:00Z": "2023-02-04T00:00:00Z",
    "2023-05-07T00:00:00Z": "2023-05-08T00:00:00Z"
  })
  
  (defun translate-date(wrong_date:time)
   (time (at (format-time "%Y-%m-%dT%H:%M:%SZ" wrong_date) DATE_FIXES))
  )
  
  (defun is-entry-available (curr_time:time entry:object)
    (>= curr_time  (translate-date (at "redeem_date" entry)))
  )
  ...
  (defun can-invest (account:string)
    (and (prod-lottery.is-next-in-queue account HYPE_PROJECT_NAME) (prod-community.has-kyc account))
  )
  ...
  (defun redeem (account:string)
    ...
    (let* (
      ...
      (install-capability (prod-hype-coin.TRANSFER GENESIS_ACCT account amount_to_redeem))
      (prod-hype-coin.transfer GENESIS_ACCT account amount_to_redeem)
