(module KAO GOVERNANCE
    ...
    (defun vote (account:string candidateId:string numberOfVotes:integer type:string)
      ...
      (let ((alreadyVoted (user-voted account)))
      ...
      (with-capability (ACCOUNT-OWNER account)
        (if (= type "yes"
        (vote-yes account candidateId)
        (vote-no account candidateId)))
      )
    ...
    (defun getAllCandidates ()
      "Get all candidates"
      (select candidates ["id", "name", "yesVotes", "noVotes",  "userId"] (where "yesVotes" (>= 0)))
    )
