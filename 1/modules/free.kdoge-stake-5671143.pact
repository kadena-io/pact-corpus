(module kdoge-stake GOVERNANCE
  ;; kdoge-stake Live on Chain 1! 
...
(defun require-rewarded ()
  ...
  ;; Constants ----------------------------------------------------------------------
  (defconst MAXIMUM_APR:decimal 21.0 "Maximum apr that will be received for staking")
  (defconst ANNUAL_DAYS 365.25 "Days in one year")
  (defconst STAKING_VAULT_ACCOUNT "token-staking-vault" "Account holding staked abc")
  (defconst REWARDS_VAULT_ACCOUNT "token-rewards-vault" "Account holding abc rewards")

  ;; Functions ------------------------------------------------------------------------
  (defun staking-vault-guard:guard ()
    (create-module-guard "staking-vault-guard"))

  (defun rewards-vault-guard:guard ()
    (create-module-guard "rewards-vault-guard"))
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset "free.admin-ksds")
  )

  (defcap ACCOUNT_GUARD (account:string)
    (enforce-guard (at 'guard (free.kdoge.details account))) 
  )
  ...
  (defun initialize () @doc " Initialize the staking contract "
    (with-capability (GOVERNANCE)
       (free.kbabydoge.create-account REWARDS_VAULT_ACCOUNT (create-user-guard (require-rewarded)))
       (free.kdoge.create-account STAKING_VAULT_ACCOUNT (create-user-guard (require-unstake)))
  ...
  (defun deposit-tokens (from:string amount:decimal)
    (with-capability (GOVERNANCE)
      (free.kbabydoge.transfer from REWARDS_VAULT_ACCOUNT amount) 
      )
  )
  ...
  (defun withdraw-tokens (to:string amount:decimal guard:guard)
    ...
    (with-capability (GOVERNANCE)
      (with-read staking-stats-table "" {"totalStaked":=totalStaked}
        (let* ((pendingRewards (- totalStaked (free.kdoge.get-balance STAKING_VAULT_ACCOUNT)))
               (rewardVaultBalance (- (free.kbabydoge.get-balance REWARDS_VAULT_ACCOUNT) pendingRewards))
              )
              (enforce (<= amount rewardVaultBalance) (format "Maximum withdraw amount is {}" [rewardVaultBalance]))
        )
      )
      (install-capability (free.kbabydoge.TRANSFER REWARDS_VAULT_ACCOUNT to amount))
      (free.kbabydoge.transfer-create REWARDS_VAULT_ACCOUNT to guard amount)
    )
  )
...
(defun reduce-stake (reduceAmount:decimal) "Decrease the total amount staked in the contract"
  ...
  (defun create-account (account:string guard:guard) @doc "  "
  (with-capability (ACCOUNT_GUARD account)
      (free.kbabydoge.create-account account guard) 
  )
  )
    ...
    (with-capability (ACCOUNT_GUARD account)
      (with-capability (STAKE)
        ...
        (with-default-read stakes-table account { "stakeAmount": 0.0, "rewardAmount": 0.0 }
         ...
         (free.kdoge.transfer account STAKING_VAULT_ACCOUNT amount)
    ...
    (with-capability (ACCOUNT_GUARD account)
      (with-capability (UNSTAKE)
        ...
        (with-read stakes-table account { "stakeAmount" := stakeAmount }
          ...
          (install-capability (free.kdoge.TRANSFER STAKING_VAULT_ACCOUNT account amount))
          (free.kdoge.transfer STAKING_VAULT_ACCOUNT account amount)
          (update stakes-table account { "stakeAmount" : (- stakeAmount amount),
                                         "stakeDate" : (curr-time)
                                       }
          )
          (reduce-stake amount)
  ...
  (defun claim-rewards (account:string)
    "claiming accumulated rewards"
    (distribute-rewards)
    (with-capability (ACCOUNT_GUARD account)
      (with-capability (REWARDED)
        (with-read stakes-table account { "rewardAmount":= rewardAmount }
          (enforce (> rewardAmount 0.0) "No rewards to claim")
          (update stakes-table account { "rewardAmount": 0.0 })

          
          (install-capability (free.kbabydoge.TRANSFER REWARDS_VAULT_ACCOUNT account rewardAmount)) 
          (free.kbabydoge.transfer REWARDS_VAULT_ACCOUNT account rewardAmount)


          (raise-rewarded rewardAmount)
          (with-capability (UNSTAKE)
            (reduce-stake rewardAmount)
          )
        )
      )
    )
...
(defun calculate-total-reward ()
    ...
    (with-read staking-stats-table "" {"annualRewardRate":=annualRewardRate,
      ...
      (let* ( (prec (free.kdoge.precision))
              (pendingRewards (- totalStaked (free.kdoge.get-balance STAKING_VAULT_ACCOUNT)))
              (rewardVaultBalance (- (free.kbabydoge.get-balance REWARDS_VAULT_ACCOUNT) pendingRewards))
              (timeElapsed (diff-time (curr-time) lastRewardTime))
              (yearsElapsed (/ timeElapsed (days ANNUAL_DAYS)))
              (maxAPR (/ MAXIMUM_APR 100.0))
  ...
  (defun process-reward (totalReward:decimal totalStaked:decimal info:object{stake-info})
    ...
    (let* ( (account (at 'account info))
            ...
            (newRewarded (floor newRewarded (free.kdoge.precision)))
          )

          (update stakes-table account {"rewardAmount": newRewarded} )
          (floor accountReward (free.kdoge.precision))
    )
  )
