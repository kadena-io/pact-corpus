    ...
    (defun reserve:string (account:string amount-kda:decimal amount-mjc:integer)
      ...
      (with-capability (RESERVE account amount-kda amount-mjc)
        (let
            ...
            (coin.transfer account MJ_BANK amount-kda)
            (if (> mjc-amount 1.0)
                (update mjc-reservations-table account
                    { "account"    : account
                    , "amount-kda" : kda-amount
                    , "amount-mjc" : mjc-amount
                    , "guard"      : g
                    }
                )

                (insert mjc-reservations-table account
                    { "account"    : account
                    , "amount-kda" : kda-amount
                    , "amount-mjc" : mjc-amount
                    , "guard"      : g
          ...
          (format "{} reserved {} MJC with {} KDA" [account, amount-mjc, amount-kda])
        )
      )
    )
