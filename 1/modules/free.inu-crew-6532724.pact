(module inu-crew 'inu-boss
  ...
  @doc " Grab yours algo-gen Inu Gangster cause something serious Ahusharmota is about to happen  \
    ...
    (defun inu-club-fight (owner:string amount:integer gangster:string)
        @doc "Inu Club Fight"
        (enforce (>= amount 5) "Minumun 5 KDA for play")
        (enforce (<= amount 60) "Maximum 60 KDA for paly")
        (let (
                (gangster-owner (is-gangster-owner gangster))
            )
            (enforce (= [{"owner": owner}] gangster-owner) "You are not the owner of this NFT")
        )
        (coin.transfer owner INU_BOSS_WALLET (* 1 amount))
        (with-capability (ACCOUNT_GUARD owner)
            (with-capability (PRIVATE)
                (fight gangster)
            )
    ...
    (defun fight (gangsterId:string)
        ...
        (with-capability (PRIVATE)
            (let* ((gangster-opponent "1:56")
                   (gangster-opponent-hp (calcGangsterHp gangster-opponent))
                   (gangster-hp (calcGangsterHp gangsterId)))
              (> gangster-hp gangster-opponent-hp))
        )
    )

    (defun calcGangsterHp (gangsterId:string)
      @doc "Return gangster HP"
      (require-capability (PRIVATE))
      (with-capability (PRIVATE)
      (let* ((attributes (+ {} (read nfts gangsterId ["strength" "wisdom" "charisma" "endurance"])))
              (strength (at "strength" attributes))
              (wisdom (at "wisdom" attributes))
              (charisma (at "charisma" attributes))
              (endurance (at "endurance" attributes)))
            (fold (+) 0 [strength wisdom charisma endurance])
        )
      )
    )
    ...
    (defun is-gangster-owner (id:string)
        @doc "Get The Owner of an NFT of ID"
