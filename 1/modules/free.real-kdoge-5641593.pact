(module real-kdoge GOVERNANCE

  @doc " 'Real-Kdoge' is a project that is entirely community-driven, decentralised, and fully operational launched on the Kadena Network. "
  ...
  (defcap GOVERNANCE
    ...
    (enforce-keyset "free.real-kdoge-admin")
  )
  ...
  (defconst ADMIN_ACCOUNT_ID "k:4a666fdb8cf621973a9b1e7839b12d3a8e599b351aea5bce4eb8c85034f0fbb7"
    " ID for admin accont" )
 ...
 (defconst PUBLIC_WL_KEY "public_wl_role"
  ...
  (defconst KDE_PER_KDA 9090909.0
    " How much KDE tokens 1 KDA can buy. ")
  ...
  (defun buy:string
    ...
    @doc " Function to let people buy the token. "
      (with-capability (INTERNAL)
        (let
          (
              (amount-kde (* amount-kda KDE_PER_KDA ) )
          )
          (enforce-passes-wl account amount-kde)
          (increase-premined-amount amount-kde)
          (coin.transfer account ADMIN_ACCOUNT_ID amount-kda)
          (install-capability (TRANSFER ROOT_ACCOUNT_ID account amount-kde) )
          (transfer-create ROOT_ACCOUNT_ID account guard amount-kde )
        )
      )
  )
  ...
  (defun enforce-wl-buy-conditions (user-id:string buy-amount:decimal user-wl-role:string)
    ...
    (let
        ...
        (enforce (>= new-balance (min-buy-kde)) " Must buy above minimum amount" )
        (enforce (<= new-balance (max-buy-kde)) " Cannot buy above maximum amount" )
    )
  )
  ...
  (defun min-buy-kde ()
    @doc " Minimum amount in KDE a user can buy "
    (* MIN_BUY_KDA KDE_PER_KDA)
  )

  (defun max-buy-kde ()
    @doc " Minimum amount in KDE a user can buy "
    (* MAX_BUY_KDA KDE_PER_KDA)
  )
