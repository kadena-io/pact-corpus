(module bana-sale GOV
  ...
  (defcap GOV ()
    (enforce-keyset "n_df83905bd42ed92e559616bb707f74979a4010e0.bana")
    (compose-capability (OPS_INTERNAL))
  )

  (defcap OPS ()
    (enforce-keyset "n_df83905bd42ed92e559616bb707f74979a4010e0.bana-ops")
    (compose-capability (OPS_INTERNAL))
  )
  ...
  (defun update-sale-total-supply(sale:string amount:decimal)
    @doc "Update sale total supply"

    (with-capability (OPS)
      (enforce (> amount 0.0) "Token total supply must be positive")
      (update sales sale { "total-supply": amount }) 
    )
  )
