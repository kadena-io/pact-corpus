(module tfm-hype-locker 'mfk-hype-admin-multi-keyset
  ...
  (defun unlock-hype (account:string entries_to_unlock:[object:*])
    @doc
    ...
    (let* (
        ...
        (amount_to_unlock (get-unlockable-amount valid))
      )

      (enforce (> amount_to_unlock 0.0) "Nothing to unlock at this time")

      (install-capability (tfm-hype-coin.TRANSFER LOCK_ACCT account amount_to_unlock))
      (tfm-hype-coin.transfer LOCK_ACCT account amount_to_unlock)
  ...
  (defun get-unlockable-amount (entries_to_unlock:[object:*])
        (let* (
