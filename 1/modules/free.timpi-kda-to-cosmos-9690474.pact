...
;; -----------------------------------------------------------------------------
    ...
    (defcap OPERATOR 
        ...
        (with-read operator-table kda-address 
        {
...
;; -----------------------------------------------------------------------------
    ...
    (defun get-keplr-allocations (account:string)
    (with-capability (OPERATOR account)
    ...
    (defun set-operator
        (account:string)
        (with-capability (GOVERNANCE)
        (insert operator-table account
            {
            "account":account
            ,"operator-guard": (at 'guard (coin.details account))
            })))
