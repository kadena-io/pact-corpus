(module staging11-nft-distribution 'hype-admin-local-test-staging
  ...
  (defun redeem-nft (project:string account:string)
    (with-read allocations-table (allocation-key project account) {"nfts":= nfts}
                ...
                (with-read project-metadata-table project {"nfts_redeemed":= nfts_redeemed}

                        (update project-metadata-table project {"nfts_redeemed": (+ 1 nfts_redeemed)})
                        (update allocations-table (allocation-key project account) {"nfts": (- nfts 1)})
                        ;;transfer the key or something 
                        ;  (hypercent.nft-ledger.ceva token id)

                        (at "key" (read nft-index (nft-index-key project nfts_redeemed)))
                )
    )
  )
  ...
  (defun buy-nfts (project_name:string account:string guard:guard)
        (let* (
            (nfts 1)
            ...
            (with-default-read allocations-table account {"round": 0, "nfts":0} { "round":=last-round, "nfts":=last-nfts}
                ...
                (bind (read nft-index-table (nft-index-key project_name nfts_sold)) {
                  "key":=key
                }
                  (install-capability (free.marmalade-staging11-ledger.MINT project_name account 1.0))
                  (install-capability (NFT_DISTRIBUTION))
                  (free.marmalade-staging11-ledger.mint key account guard 1.0)
                )
            )
        )
    )
