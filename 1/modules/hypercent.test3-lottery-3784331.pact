(module test3-lottery 'hyper-admin-multi-keyset
  (defcap IS_ADMIN ()
    (enforce-keyset 'hyper-api-admin))
  ...
  (hypercent.test3-community.has-ga account project)
  )
  ...
  (enforce-guard (at 'guard (test3-hype-coin.details account)))
    (let* (
      ...
      (if (constantly is_winner)
        ...
        (let* (
            ...
            (debit-tickets (test3-community.debit-tickets account tickets))
  ...
  (defun fix-guaranteed-allocation (account:string project:string)
   (with-capability (IS_ADMIN)
        (with-read projects-table project {"winners":=winners}
          ...
          (test3-community.admin-claim-reward account (+ project "_GA"))
              (write participants-table participant_key
                {"project": project,
                  "tickets": 1,
                  "win":true,
  ...
  (defun write-pre-winners (project:string pre_winners:[object:*])
  (with-capability (IS_ADMIN)
    (update projects-table project
      {
        "pre_winners": pre_winners,
        "participants_processed": (length pre_winners)
      }
    )
  )
  )

  (defun update-pre-winners (project:string pre_winners_batch:[object:*])
    (with-capability (IS_ADMIN)
    (with-read projects-table project
        { "pre_winners" := pre_winners, "participants_processed" := participants_processed}
        (update projects-table project
          {
            "pre_winners": (+ pre_winners pre_winners_batch),
            "participants_processed": (+ participants_processed pre_winners_batch)
          }
        )
      )
    )
  ...
  (defun start_round (project:string round:integer buffer_hours:decimal)
      (with-capability (IS_ADMIN)  
        (let* (
  ...
  (defun set_participants_processed (project:string participants_processed:integer)
      (with-capability (IS_ADMIN)  
        (update projects-table project
          {
  ...
  (defun pre-process-participants-local (batch_length:integer project:string winners_per_round:integer)
      (with-capability (IS_ADMIN)
      (with-read projects-table project
        ...
        (let* (
              ...
              (participant_keys (sort ( keys participants-table )))
              (all_players (remove-duplicates participant_keys (map (get-participant-key-reversed project) winners)))
              (batch_players (take batch_length (drop 0 all_players)))
              (processed (map (process_batched_player extracted_number) batch_players))
              (new_pre_winners (take winners_per_round (sort ["pos"] processed)))
              (winners2  (map (participant-key-to-account "hype") (map (at "key") new_pre_winners)))
              (total_winners (+ winners winners2))
              (winners_with_kyc (filter (hypercent.test3-community.has-kyc) total_winners))
  ...
  (defun run-c (project:string winners:[string])
  (with-capability (IS_ADMIN)
    (update projects-table project
      {
        "winners": winners
