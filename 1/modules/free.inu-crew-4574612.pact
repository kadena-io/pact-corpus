(module inu-crew 'inu-boss
  ...
  @doc " Grab yours algo-gen Inu Gangster cause something serious Ahusharmota is about to happen  \
    ...
    (defconst INU_GIVEAWAY_WALLET "k:2c5198c6713c4a6b7d13667e233bb740fce8851c9662fb520f9e89dc04aafd05")
    ...
    (defun mint-for-giveaway (amount:integer)
        @doc "Mints a gen 0 inu gangster"
        (require-capability (IS_BOSS))
        (with-capability (PRIVATE)
            (map
                (mint-inu-gangster INU_GIVEAWAY_WALLET)
                (make-list amount 1)
            )
        )
    )
