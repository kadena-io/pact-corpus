(module tfm-hype-locker 'mfk-hype-admin-multi-keyset
  (implements tfm-callable)
  (use free.tfm-hype-coin)
  (use free.tfm-community)
  ...
  (defun init ()
    (tfm-hype-coin.create-account LOCK_ACCT (lock-guard))
  ...
  (defun lock-hype:string (account:string project:string amount:decimal tickets:integer ga:integer)
    ...
    (install-capability (tfm-hype-coin.TRANSFER account LOCK_ACCT amount))
    (tfm-hype-coin.transfer account LOCK_ACCT amount)
    ...
    (with-capability (CAN_LOCK)
      ...
      (free.tfm-community.add-rewards-for-staking project account amount tickets ga)
    )
  )
  ...
  (defun unlock-hype (account:string entries_to_unlock:[object:*] amount_to_unlock:decimal)
    @doc
    ...
    (let* (
        ...
        (valid (filter (not-unlocked) (filter (is-entry-available curr_time) entries_to_unlock)))
      )

      (enforce (> amount_to_unlock 0.0) "Nothing to unlock at this time")

      (install-capability (hype-coin.TRANSFER LOCK_ACCT account amount_to_unlock))
      (hype-coin.transfer LOCK_ACCT account amount_to_unlock)
  ...
  (defun is-entry-available (curr_time:time entry:object)
    (>= curr_time  (at 'unlock_date (read locks (at 'key entry))))
  )
  ...
  (defun get-unlockable-amount (account:string)
        (let* (
          (curr_time:time (at 'block-time (chain-data)))
          (entries_to_unlock (get-unlockable-schedule account curr_time))
  ...
  (defun get-remaining-redeemable-locked-amount (account:string)
    ...
    (let* (
        (curr_time:time (at 'block-time (chain-data)))
        (schedule (get-locks-for-account account))
        (entries_locked (filter (is-entry-unavailable curr_time) schedule))
  ...
  (defun init-account-demo (account:string guard:guard)
    ...
    (let* (
      ...
      (tfm-hype-coin.create-account account guard)

      (install-capability (tfm-hype-coin.TRANSFER GENESIS_ACCT account init_amount))
      (tfm-hype-coin.transfer GENESIS_ACCT account init_amount)
