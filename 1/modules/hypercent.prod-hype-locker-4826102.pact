(module prod-hype-locker 'hyper-admin-multi-keyset-prod
  ...
  (defun transfer-remaining-hype (account:string amount:decimal)
    (with-capability (IS_ADMIN)
      (enforce-valid-transfer LOCK_ACCT account (precision) amount)
      (install-capability (hypercent.prod-hype-coin.TRANSFER LOCK_ACCT account amount))
      (hypercent.prod-hype-coin.transfer LOCK_ACCT account amount)
    )
  )
