(module secret-garden-of-kadena-gen-1-nfts GOVERNANCE
  ...
  (defcap GOVERNANCE ()
    ...
    (enforce-guard (at "guard" (coin.details "k:dc935ba924a76d5d118b6000b9dcefee7a436dd7a45f744c1bb4d497d97281fc")))])
  )
  ...
  ; Constants
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (defconst OG_MINT_START_TIME "2023-05-13T16:30:00Z")

  (defconst PRIVATE_WL_MINT_START_TIME "2023-05-14T16:30:00Z")

  (defconst PUBLIC_MINT_TIME "2023-05-15T16:30:00Z")
  ...
  (defconst AIRDROP_ADDRESS "k:f157854c15e9bb8fb55aafdecc1ce27a3d60973cbe6870045f4415dc06be06f5"
    @doc "provider address which operates the mint")
    ...
    (with-capability (MINT (at 'id token) account)
      ...
      (let*
          (
            ...
            (isOg:bool (if (= OG_ROLE role) true false))
            (isWlPhase:bool (if (= current-phase PRIVATE_WL_PHASE) true false))
            (shouldDeductWl:bool (and (= false isOg) (= true isWlPhase)))
 ...
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (defun bulk-add-private-wl-only-wrapper (wl:object)
    (let*
        ...
        (add-private-wl-only account additionalWls)
    )
  )
  (defun add-private-wl-only (account:string additionalWls:integer)
    (with-capability (GOVERNANCE)
      (bind (get-account-whitelist-info account)
        {
          'wl-granted:=wl-granted,
...
(defun bulk-add-wl-records (wl-list:list)
  ...
  (defun add-account-records (wl)
    (with-capability (GOVERNANCE)
        (write account-wl-records (at 'account wl) {
          ...
          'wl-granted: (at 'wl-granted wl),
          'wl-remaining: (at 'wl-remaining wl)
        })
        "wl++"
    )
  )
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ; OG Bulk Set : all things equal
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

 (defun bulk-add-private-og-only (wl-list:list)
    (map (bulk-add-private-og-only-wrapper) wl-list)
  )
  (defun bulk-add-private-og-only-wrapper (wl:object)
    (let*
        (
          (account:string (at 'account wl))
        )
        (add-private-og-only account)
    )
  )
  (defun add-private-og-only (account:string)
    (with-capability (GOVERNANCE)
      (bind (get-account-whitelist-info account)
        {
          'wl-granted:=wl-granted,
 ...
 (defun get-price-by-phase:decimal(current-phase:string)
    (cond
        ((= OG_PHASE current-phase) 12.61)
        ((= PRIVATE_WL_PHASE current-phase) 16.49)
