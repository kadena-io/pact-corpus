...
(bless "FnndnDJ9zt-KZFnyxuyOjOP7nYuhDZGSe4cRO8FtPSI")

  (use util.fungible-util)
  (use kip.token-manifest)


  (implements free.universal-poly-fungible-v3)
  (use free.universal-poly-fungible-v3 [account-details sender-balance-change receiver-balance-change])
  ...
  (defschema token-schema
    ...
    precision:integer
    supply:decimal
    policy:module{free.universal-token-policy-v2}
  )
  ...
  (defcap TOKEN:bool (id:string precision:integer supply:decimal policy:module{free.universal-token-policy-v2})
    @event
    true
  )
  ...
  (defcap MINT-BULK (account:string amount:decimal)
    @managed ;; one-shot for a given amount
    (enforce (< 0.0 amount) "Amount must be positive")
    (compose-capability (CREDIT "id" account))
    (compose-capability (UPDATE_SUPPLY))
  ...
  (defschema policy-info
    policy:module{free.universal-token-policy-v2}
    token:object{free.universal-token-policy-v2.token-info}
  )

  (defun get-policy-info:object{policy-info} (id:string)
    (with-read tokens id
      { 'policy := policy:module{free.universal-token-policy-v2}
      , 'supply := supply
      , 'precision := precision
      , 'manifest := manifest
      }
    ...
    (with-capability (GOVERNANCE)
      ...
      (let*
        ...
        (bind (get-policy-info (at 0 ids))
          { 'policy := policy:module{free.universal-token-policy-v2}
          , 'token := token }
          (free.kadcars-nft-policy.enforce-mint-bulk token-list account guard))
...
;;;;;; TESTING END 
  ...
  (defun sale-account:string ()
    ...
    (defun create-k2-testing:object{manifest} (token-id-p:string)
      (with-capability (GOVERNANCE)
        (let*
          (
            ...
            (manifest (get-k2-manifest vehicleSpec vin make model))
          )
          (enforce (= token-id-p token-id) "mismatch")
          (admin-only-testing token-id manifest)        
        )
      )
    )
    ...
    (defun get-immutable-state:object{immutable-state-schema} (
      ...
      )
        {
          ...
          ,'mint-time: (time "2023-02-06T21:16:13Z")
        }
