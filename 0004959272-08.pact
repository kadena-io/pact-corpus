   ...
   (defun get-first-non-empty-collection-from-weapons ()
        (let* 
            (
                ...
                ;(has-next-non-minted-cols-list-list:list (filter (compose (length-of-non-minted) (< 0)) non-minted-cols-list))
                (equal:list (filter (compose (length-of-non-minted) (= 0)) non-minted-cols-list))
                (lessthan:list (filter (compose (length-of-non-minted) (< 0)) non-minted-cols-list))
                (greaterthan:list (filter (compose (length-of-non-minted) (> 0)) non-minted-cols-list))

            )
            (format "equal {} lessthan {} greaterthan {}" [equal lessthan greaterthan])
    ...
    (defun visible-for-test-get-non-minted-tokens (col-id:string )
        (if (= col-id "battle-heroes-weapons-policy-level-1-col-6")
                {"non-minted-token-ids": ["1" "2"], "collection-id" : "battle-heroes-weapons-policy-level-1-col-6"}
                {"non-minted-token-ids": [], "collection-id" : (+ "battle-heroes-weapons-policy-level-1-col-" col-id)}
        )
    )
