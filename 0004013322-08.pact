...
; ============================================
    ...
    (defun init-miner (token-id:string uri:string special-attributes:string market-price:decimal kda-mined-index:integer nft-id:string for-sale:bool staked:bool) 
        (require-capability (PRIVATE))
    	(insert mledger token-id
    		{ "nft-id": token-id
            , "uri": uri
    		, "hashrate": 3.0
    		, "tied-asic": []
    		...
    		, "updated-at": (time "2023-03-03T21:00:00Z")
    		, "kda-mined-index": kda-mined-index }
    	)
        (insert nft-token-pairs nft-id
            { "token-id": token-id }
        )
        (with-capability (CALL-POLICY-MODULES)
            (kmc-policy.init-miner-status token-id for-sale staked)
        )
    )

    (defun get-token-pairs-id ()
        (keys nft-token-pairs)
    )

    (defun insert-coll-offer-multiple (items:[object])
    	(with-capability (ADMIN)
    		(map (insert-collection-offer) items)
    	)
    )
    
    (defun insert-collection-offer (item:object)
        (require-capability (PRIVATE))
        (insert collection-offer-table (int-to-str 10 (at 'offer-id item))
        	{ "address": (at 'address item)
        	, "valid-until": (time (at 'valid-until item))
        	, "amount": (at 'amount item)
        	, "offer-id": (at 'offer-id item) }
        )
    )
    ...
    (defun init-airdrop-multiple (items:list)
    	(with-capability (ADMIN)
    		(map (init-airdrop) items)
    	)
    )

    (defun init-airdrop (item:object)
    	(require-capability (PRIVATE))
    	(insert airdrop-table (get-token-id (at 'key item))
    		{ "airdrop": (at 'air item) }
    	)
    )
    ...
    (defun accept-nft-offer (offer-id:string)
        ...
        (with-read nft-offer-table offer-id
            ...
            (let 
                (
                  (account (kmc-policy.get-owner nft-id))
                )
                (with-read mledger nft-id 
                    { 'staked := staked }
                    ...
                    (with-capability (CALL-POLICY-MODULES)
                        (kmc-policy.update-for-sale nft-id false)
                    )
                    (marmalade.ledger.transfer-create nft-id account new-address (at 'guard (coin.details new-address)) 1.0)
    ...
    ;     (let ; TODO; update this to work for marmalade for notifications
    ;         (
    ;             (owned-miners (select mledger ["nft-id"] 
    ;                                 (and? (where "owner-address" (= account))
    ;                                       (where "for-sale" (= true) ))))
    ;         )
    ;         (map (select-valid-offers-for-id) owned-miners)
    ;     )
    ; )
    ...
    (defun enforced-get-kda-since-last-claim (account:string latest-kda-mined:decimal nft-id:string)
        ...
        (with-read mledger nft-id 
          { 'kda-mined-index := kda-mined-index }
            (enforce (= (kmc-policy.get-owner nft-id) account) "Account is not owner of the NFT")
            (if (= false (at 'staked (kmc-policy.get-miner-status nft-id))) 
                (- 1 1) ;if staked is false, return 0
                (with-read kda-mined-table (int-to-str 10 kda-mined-index) ;if staked is true, do maths
                  { 'kda-mined := previous-kda-mined }
    ...
    (defun accept-collection-offer (account:string nft-id:string offer-id:string)
        ...
        (with-read collection-offer-table offer-id
            ...
            (enforce (= false (at 'staked (kmc-policy.get-miner-status nft-id))) "Your NFT must be turned off before you can accept a collection offer")
            (update mledger nft-id
                { "for-sale" : false }
            )
            (with-capability (CALL-POLICY-MODULES)
...
; ============================================
    ...
    (defun turn-on-miner1 (kda-mined-index:integer nft-id:string)
        (require-capability (PRIVATE))
        (enforce (= (at 'for-sale (kmc-policy.get-miner-status nft-id)) false) "Your NFT is listed on the marketplace, please de-list before turning on your miner")
...
; ============================================
    ...
    (defun get-user-founders-object-multiple (account:string)
        @doc "Returns all Founder's Passes owned by one address"
        (let 
            (
                (owner-ids (kmc-founders-policy.get-tokens-owned account))
            )
            (map (get-user-founders-object) owner-ids) ;todo make fancier
        )
    )

    (defun get-user-founders-object (id:object)
        {"id": id, "founder": (select fledger (where "nft-id" (= (at 'id id)))), "for-sale": (at 'for-sale (kmc-founders-policy.get-status id))}
    )
...
; ============================================
    ...
    (defun list-miner-on-market (nft-id:string owner:string price:decimal for-sale:bool)
        ...
        (enforce (< price 99999999.1) "You cannot set a price higher than 999,999,999")
        (with-read mledger nft-id
          { 'nft-id := ledger-id }
          (enforce (= (at 'staked (kmc-policy.get-miner-status nft-id)) false) "Your NFT is currently powered ON. Please power it off on the farm dashboard and try again")
          (with-capability (MINER_OWNER (kmc-policy.get-owner nft-id) ledger-id)
            (enforce (= owner (kmc-policy.get-owner nft-id)) "Account Owners dont match")
            ...
            (with-capability (CALL-POLICY-MODULES)
            (kmc-policy.update-for-sale nft-id for-sale))
            (emit-event (LIST_MINER nft-id (kmc-policy.get-owner nft-id) price for-sale))
    ...
    (defun buy-miner-off-market
        ...
        (with-read mledger nft-id
            ...
            (let 
                ...
                (enforce (= (at 'for-sale (kmc-policy.get-miner-status nft-id)) true)  "You can only purchase an NFT that is for sale." )
                (enforce (= price ledger-price) "The price you chose does not match the price of the NFT you're trying to buy.")
                (enforce (!= buyer (kmc-policy.get-owner nft-id)) "You cannot buy your own NFT.")
                (with-capability (ACCOUNT_GUARD buyer) 
                    (coin.transfer buyer account (round (* 0.95 ledger-price) 2))
                    ...
                    (with-capability (CALL-POLICY-MODULES)
                    (kmc-policy.update-for-sale nft-id false))
                    (marmalade.ledger.transfer-create nft-id account buyer (at 'guard (coin.details buyer)) 1.0)
                    (emit-event (BUY_MINER nft-id buyer account price))
...
; ============================================
    ...
    (defun transfer-founder:string (id:string sender:string receiver:string amount:decimal guard:guard)
        @doc "Transfer an NFT to another valid k:address"
        (let ((exist (coin-account-exists receiver)))
            (if (= exist true) "" (coin.create-account receiver guard))
        )
        (enforce (= 1.0 amount) "Only 1 Pass can be transferred at a time")
        (enforce (= (at 'for-sale (kmc-founders-policy.get-status id)) false)  "Your NFT is currently for sale, please de-list and try to transfer again" )
        (marmalade.ledger.transfer-create id sender receiver (at 'guard (coin.details receiver)) 1.0)
    ...
    (defun list-founder-pass-on-market(nft-id:string owner:string price:decimal for-sale:bool)
        ...
        (with-read fledger nft-id
          { 'nft-id := ledger-id }
          (with-capability (FOUNDERS_OWNER (kmc-founders-policy.get-owner nft-id) ledger-id)
            (enforce (= owner (kmc-policy.get-owner nft-id)) "Account Owners dont match")
            ...
            (with-capability (CALL-POLICY-MODULES)
                (kmc-founders-policy.update-for-sale nft-id for-sale))
            (emit-event (LIST_FOUNDER nft-id (kmc-policy.get-owner nft-id) price for-sale))
    ...
    (defun buy-founders-pass-off-market
        ...
        (with-read fledger nft-id
            ...
            (let 
                (
                    (current-owner (kmc-founders-policy.get-owner nft-id))
                    (ledger-for-sale (at 'for-sale (kmc-policy.get-miner-status nft-id)))
                )
                ...
                (with-capability (ACCOUNT_GUARD buyer) 
                    ...
                    (with-capability (CALL-POLICY-MODULES)
                        (kmc-founders-policy.update-for-sale nft-id false))
                    (marmalade.ledger.transfer-create nft-id current-owner buyer (at 'guard (coin.details buyer)) 1.0)
    ...
    (defun get-all-founders-for-sale (index-start:integer index-end:integer)
        ...
        (take (- (- index-end (- index-start 1))) (take index-end (sort (select fledger ["nft-id", "for-sale", "market-price", "free-mints-remaining", "airdrops-remaining"] (where "for-sale" (= true))))))
...
; ============================================
    ...
    (defun transfer-miner-multiple (ids:list sender:string receiver:string)
        ...
        (with-capability (PRIVATE)
            (map 
                (transfer-miner1 sender receiver)
                ids
            )
        )
    ...
    (defun transfer-miner1:string (sender:string receiver:string id:string)
        ...
        (with-read mledger id
            { 'nft-id := ledger-id }
            (enforce (= (at 'for-sale (kmc-policy.get-miner-status id)) false)  "Your NFT is currently for sale, please de-list and try to transfer again" )
            (enforce (= (at 'staked (kmc-policy.get-miner-status id)) false) "Your NFT is currently powered ON. Please power it off on the farm dashboard and try again")
            (enforce (!= sender receiver) "You can't send an NFT to yourself")
            (enforce (= (kmc-policy.get-owner ledger-id) sender) "You are not the owner of this NFT")
    ...
    (defun transfer-miner:string (id:string sender:string receiver:string amount:decimal)
        ...
        (with-read mledger id
            ...
            (enforce (= (at 'for-sale (kmc-policy.get-miner-status id)) false)  "Your NFT is currently for sale, please de-list and try to transfer again" )
            (enforce (= (at 'staked (kmc-policy.get-miner-status id)) false) "Your NFT is currently powered ON. Please power it off on the farm dashboard and try again")
            (marmalade.ledger.transfer-create ledger-id (kmc-policy.get-owner ledger-id) receiver (at 'guard (coin.details receiver)) 1.0)
        )
        (emit-event (TRANSFER_MINER id sender receiver))
        (format "Miner with id #{} has been transferred to {}" [id receiver])
    )

    (defun transfer:string (receiver:string id:string guard:guard 
      uri:string special-attributes:string market-price:decimal 
      kda-mined-index:integer nft-id:string for-sale:bool staked:bool)
        @doc "Transfer a Miner NFT to another valid k:address"
        ; (enforce-coin-account-exists receiver)
        (with-capability (PRIVATE)
            (init-miner id uri special-attributes market-price kda-mined-index nft-id for-sale staked)
        )
        (let ((exist (coin-account-exists receiver)))
            (if (= exist true) "" (coin.create-account receiver guard))
        )
        (emit-event (TRANSFER_MINER id (kmc-policy.get-owner id) receiver))
        (marmalade.ledger.transfer-create id (kmc-policy.get-owner id) receiver (at 'guard (coin.details receiver)) 1.0)
...
; ============================================
    ...
    (defcap MINER_OWNER (account:string id:string)
        ...
        (let
            (
                (nft-owner (marmalade.ledger.get-balance id account))
            )
            (enforce (= nft-owner account) "Account is not owner of the NFT")
                (compose-capability (ACCOUNT_GUARD account))
        )
