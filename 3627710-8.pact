(module prod-nft-distribution GOVERNANCE
  ...
  (defcap BUY_NFTS (project:string account:string amount:integer price:decimal)
    @event
    true
  )
  ...
  (defun set-nfts-reserved (project:string nfts_reserved:[integer])
    (with-capability (IS_ADMIN)
      (update project-metadata-table project {"nfts_reserved":  nfts_reserved})
    )
  )
  ...
  (defun fix-alpha-slayers (id:string k_account:string)
      (let* (
          (balance (marmalade.ledger.get-balance id k_account))
          (policy_info (marmalade.ledger.get-policy-info id))
          (nft_index (drop 1 (p policy_info ["token", "manifest", "data", 0, "datum", "name"])))
          (collection_name (p policy_info ["token", "manifest", "data", 0, "datum", "collection"]))
          (policy_name (format "{}" [(at "policy" policy_info)]))
          (fixed_nft_id (concat ["alpha-slayers-", nft_index]))
        )
        collection_name
        (enforce (= balance 1.0) "Nothing to burn")
        (enforce (= "hypercent.prod-nft-sale-royality-policy" policy_name) "Not our NFT")
        (enforce (= "alpha-slayers" collection_name) "Not from alpha-slayers collection")
        (marmalade.ledger.burn id k_account 1.0)
        (install-capability (marmalade.ledger.MINT fixed_nft_id k_account 1.0))
        (marmalade.ledger.mint fixed_nft_id k_account (at "guard" (coin.details k_account)) 1.0)
      )
  )
  ...
  (defun buy-nfts (project_name:string account:string nfts:integer)
        (let* (
            ...
            (enforce (= false (at "redeem_enabled" project)) "Sale closed")
            ...
            (with-default-read allocations-table (allocation-key project_name account) {"round": 0, "nfts":[]} { "round":=last-round, "nfts":=last-nfts}
                ...
                (emit-event (BUY_NFTS project_name account nfts price))
                nfts
            )
        )
    )

    (defun get-nfts-sold (project_name:string)
      (at "nfts_sold" (read project-metadata-table project_name))
    )
    ...
    (defun get-account-collection-tokens (project:string account:string)
      (filter (lambda (token-manifest) (= project (p token-manifest ["manifest", "data", 0, "datum", "collection"]))) (map 
        (lambda (id) {"id": id, "manifest": (marmalade.ledger.get-manifest id)}) 
        (filter 
          (lambda (id) (> (marmalade.ledger.get-balance id account) 0.0)) 
              (map (at "id") (hypercent.prod-nft-sale-royality-policy.get-account-tokens account)))))
    )

    (defun p (object path)
      (fold (lambda (current next) (at next current)) object path)
    )
