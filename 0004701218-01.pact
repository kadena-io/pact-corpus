(module prod-mok-vesting 'hyper-admin-multi-keyset-prod
  (use mok.token)
  ...
  (defcap IS_ADMIN ()
    ...
    (defconst MOK_BANK:string 'prod-mok-bank)
    (defun mok-bank-guard () (keyset-ref-guard "hyper-admin-multi-keyset-prod" ))

    (defconst MOK_HYPE_ESCROW:string 'prod-mok-hype-escrow)
    (defun mok-hype-escrow-guard () (create-module-guard "MOK_HYPE_ESCROW" ))
  ...
  (defun init ()
    ...
    (mok.token.create-account MOK_HYPE_ESCROW (mok-hype-escrow-guard))
  )
  ...
  (defun create-allocation (kda-amount:decimal account:string)
    ...
    (let* (
        (round (hypercent.prod-community-v2.get-project-round MOK_PROJECT_NAME))
 ...
 ;"******************* Allocation Util Methods *******************"
  ...
  (defun get-min-allocation-amount()
    (let* (
        ...
        (allocation_price (get-allocation-price))
      )
  ...
  (defun get-participant-ga (project:string account:string)
      (hypercent.prod-community-v2.get-participant-ga project account)
  )
  ...
  (defun can-invest (project:string account:string)
    (hypercent.prod-community-v2.is-round-winner project account)
  )
  ...
  (defun redeem (account:string)
    ...
    (let* (
      ...
      (install-capability (mok.token.TRANSFER MOK_HYPE_ESCROW account amount_to_redeem))
      (mok.token.transfer-create MOK_HYPE_ESCROW account (at "guard" (coin.details account)) amount_to_redeem)
