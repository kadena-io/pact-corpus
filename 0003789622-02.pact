...
; ============================================
    ...
    (defun initialize ()
        ...
        (insert values-table MINT_CHAIN_ID {"value": "1"})
...
; ============================================
    ...
    (defun transfer:string (id:string sender:string receiver:string amount:decimal)
    ...
    (defun buy-founders-pass-off-market
        ...
        (with-read fledger nft-id
            ...
            (with-capability (ACCOUNT_GUARD buyer) 
                ...
                (coin.transfer buyer ADMIN_ADDRESS (round (* 0.05 ledger-price) 2))
                ...
                (update fledger nft-id 
                    {
                        "owner-address": buyer,
                        "for-sale": false,
                        "unique-id" : (generate-random-number nft-id)
                    }
                )
...
; ============================================
    ...
    (defun get-unique-founders-id (nft-id:string account:string) ;update so admin can read
        @doc "Only callable by the admin and the owner of an NFT"
        (with-capability (ACCOUNT_GUARD account) 
