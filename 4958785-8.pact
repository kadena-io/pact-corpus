...
(defun get-reserved-tokens (col-id:string)
   ...
   (defun get-next-non-minted-weapon ()
        (let* 
            (
                (collection-suffix-list:list (enumerate 1 32)) 
                (collection-names-list:list (map (lambda (x) (x + "battle-heroes-weapons-policy-")) collection-suffix-list))
    ...
    (defun visible-for-test-get-non-minted-tokens:list (col-id)
        (if (= col-id "battle-heroes-weapons-policy-2")
            ["1" "2"]
            []
        )
    )
