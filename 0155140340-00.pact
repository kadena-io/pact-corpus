(module superconnectors GOVERNANCE
  ...
  (defun add-batch-connection-events:[object{connection-event-schema}]
    (connection-events:[object{connection-event-schema}])
    @doc "Create connection events"
    (with-capability (OPS)
      (map (add-connection-event) connection-events)
    )
  )
  ...
  (defun calculate-connection-reputation:integer (connection-id:string)
    @doc "Calculate the reputation of a connection"
    (let (
        (sum
          (lambda (x:[integer])
            (fold (+) 0 x))
        ...
        (query
          (lambda (k edge)
            (= connection-id (at "connection-id" edge))))
        (filter (lambda (k edge) 1))
      )
      (sum (fold-db connection-events (query) (filter)))
    )
  )
)
